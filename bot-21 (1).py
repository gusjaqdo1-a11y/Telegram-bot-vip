...
def send_d7_sms(phone_number, text):
    """Send SMS through D7 Networks. Returns (ok, detail)."""
    if not D7_TOKEN:
        return False, "D7_TOKEN is not configured"
    ...
    payload = {
        ...
        "message_globals": {
            "originator": os.getenv("D7_ORIGINATOR", "VerifyBot")
        }
    }
    try:
...
def send_waforge_whatsapp(phone_number, user_id):
    """Send a server-generated WhatsApp OTP through WaForge v1.

    The OTP is generated and stored by WaForge, so this bot deliberately does
    not generate or expose the code itself.
    """
    if not WAFORGE_API_KEY:
        return False, "WAFORGE_API_KEY is not configured", None

    headers = {
        "Authorization": f"Bearer {WAFORGE_API_KEY}",
        "Content-Type": "application/json",
        "Accept": "application/json",
    }
    # This matches the working Termux request supplied by the owner.
    payload = {"phone": phone_number}

    try:
        r = requests.post(
            WAFORGE_OTP_SEND_URL,
            json=payload,
            headers=headers,
            timeout=30,
        )
        ...
        if r.status_code in (200, 201, 202):
            request_id = body.get("id") or body.get("request_id") or body.get("uuid")
            return True, body, request_id
...
def admin_menu():
    ...
    kb.add("☎️ CUSTOMER", "💱 MIN CHANGE CURRENCY")
...
def customer_handler(m):
    # For the general admin, this button edits a customer's Telegram username.
    # For normal users it keeps the original customer-support behaviour.
    if is_admin(m.from_user.id):
        msg=bot.send_message(m.chat.id, "☎️ <b>CUSTOMER</b>\n\nSend the customer's current Telegram username.\nExample: <code>@vexdou1</code>")
...
def edit_customer_find(m):
    ...
    for user_id,data in users.items():
        if str(data.get('username','')).strip().lstrip('@').lower()==username.lower():
            uid=str(user_id); break
    if not uid:
        bot.send_message(m.chat.id, f"❌ Customer <code>@{html.escape(username)}</code> not found.")
        return
    msg=bot.send_message(m.chat.id, f"👤 Customer found: <b>@{html.escape(username)}</b>\n\nSend the new username.\nExample: <code>@newusername</code>")
    bot.register_next_step_handler(msg, edit_customer_save, uid)

def edit_customer_save(m, uid):
    if not is_admin(m.from_user.id): return
    new_username=(m.text or '').strip().lstrip('@')
    if not re.fullmatch(r'[A-Za-z0-9_]{5,32}', new_username):
        bot.send_message(m.chat.id, "❌ Invalid Telegram username. Example: <code>@newusername</code>")
        return
    for other_id,data in users.items():
        if str(other_id)!=str(uid) and str(data.get('username','')).strip().lstrip('@').lower()==new_username.lower():
            bot.send_message(m.chat.id, "❌ That username is already assigned to another user in the database.")
            return
    old_username=users[str(uid)].get('username','')
    users[str(uid)]['username']=new_username
    save_user(uid)
    bot.send_message(m.chat.id, f"✅ Customer username updated.\n\nOld: <code>@{html.escape(str(old_username).lstrip('@'))}</code>\nNew: <code>@{html.escape(new_username)}</code>\nUser ID: <code>{uid}</code>")
