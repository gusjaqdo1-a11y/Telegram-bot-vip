...
# Verification method controls are persisted in MongoDB settings.
# SMS and WhatsApp can be opened/closed independently by the admin.
verify_pending = {}
...
def send_smess_whatsapp(phone_number, code, user_id):
    """Send WhatsApp OTP using the current SMESS /api/send contract."""
    if not SMESS_API_KEY:
        return False, "SMESS_API_KEY is not configured"
    payload = {
        "recipient": phone_number,
        "text": "Your Downloader Bot verification code is:",
        "copycode": str(code),
        "copytext": "Tap to Copy",
        "priority": "otp",
        "idempotency_key": f"otp-{user_id}-{code}-{int(time.time() // 300)}"
    }
    headers = {
    ...
    try:
        r = requests.post(SMESS_API_URL, data=payload, headers=headers, timeout=30)
        try:
            body = r.json()
        except Exception:
            body = {"raw": r.text}
        if r.status_code == 202 and body.get("success", True):
            return True, body
...
@bot.message_handler(func=lambda m: m.text in [
    "🟢 Open SMS", "🔴 CLOSE SMS",
    "🟢 OPEN VIA WHATSAPP", "🔴 CLOSE VIA WHATSAPP",
    "♻️ Reset all Verify"
])
def verification_admin_manager(m):
    if not is_admin(m.from_user.id):
        return

    if m.text == "🟢 Open SMS":
        set_setting("sms_enabled", True)
        bot.send_message(m.chat.id, "🟢 SMS verification is OPEN. Users can choose Gmail, SMS and (if enabled) WhatsApp.")
        return

    if m.text == "🔴 CLOSE SMS":
        set_setting("sms_enabled", False)
        bot.send_message(m.chat.id, "🔴 SMS verification is CLOSED.")
        return

    if m.text == "🟢 OPEN VIA WHATSAPP":
        if not SMESS_API_KEY:
            bot.send_message(m.chat.id, "❌ WhatsApp cannot be opened because SMESS_API_KEY is missing.")
            return
        set_setting("whatsapp_verify_enabled", True)
        bot.send_message(m.chat.id, "🟢 WhatsApp verification is now OPEN. Users can request OTP through SMESS.")
        return

    if m.text == "🔴 CLOSE VIA WHATSAPP":
        set_setting("whatsapp_verify_enabled", False)
        bot.send_message(m.chat.id, "🔴 WhatsApp verification is now CLOSED.")
        return

    if m.text == "♻️ Reset all Verify":
        kb = InlineKeyboardMarkup()
        kb.row(
            InlineKeyboardButton("✅ YES, RESET ALL", callback_data="reset_all_verify_confirm"),
            InlineKeyboardButton("❌ CANCEL", callback_data="reset_all_verify_cancel")
        )
        bot.send_message(
            m.chat.id,
            "⚠️ This will mark EVERY user as unverified. They will be able to verify again.\n\n"
            "Existing email/phone information will be kept; only verification status is reset.",
            reply_markup=kb
        )


@bot.callback_query_handler(func=lambda call: call.data in ["reset_all_verify_confirm", "reset_all_verify_cancel"])
def reset_all_verify_callback(call):
    if not is_admin(call.from_user.id):
        return

    if call.data == "reset_all_verify_cancel":
    ...
    for uid, data in users.items():
        if data.get("verified") or data.get("whatsapp_verified"):
            changed += 1
        data["verified"] = False
        data["whatsapp_verified"] = False
        save_user(uid)

    # Kill every in-memory verification session too.
    ...
    try:
        bot.edit_message_text(
            f"♻️ ALL VERIFICATION RESET SUCCESSFULLY.\n\n"
            f"👥 Users reset: {changed}\n"
            f"🔐 Everyone is now Unverified and can verify again.",
            call.message.chat.id,
            call.message.message_id
        )
    except Exception:
        pass
    bot.answer_callback_query(call.id, "All verification statuses reset")
...
def clear_verification_state(uid, chat_id=None):
    """Completely stop a verification session, including pyTelegramBotAPI next-step handlers."""
    uid = str(uid)
    verify_pending.pop(int(uid), None)
    verify_pending.pop(uid, None)
    email_verify_pending.pop(uid, None)
    phone_verify_pending.pop(uid, None)
    whatsapp_verify_pending.pop(uid, None)

    # This is the critical fix for the old bug: cancelling an inline button
    # does not automatically remove a registered next-step handler.
    try:
        bot.clear_step_handler_by_chat_id(int(chat_id if chat_id is not None else uid))
    except Exception:
        pass


def verification_method_keyboard():
    kb = InlineKeyboardMarkup(row_width=2)
    kb.row(InlineKeyboardButton("📧 Gmail", callback_data="verify_choice_gmail"))
    if get_setting("sms_enabled", False):
        kb.add(InlineKeyboardButton("📱 SMS", callback_data="verify_choice_phone"))
    if get_setting("whatsapp_verify_enabled", False) and SMESS_API_KEY:
...
def start_verify_flow(call):
    ...
    if users.get(uid, {}).get("verified", False):
        bot.answer_callback_query(call.id, "✅ Your account is already verified.", show_alert=True)
        return
    ...
    try:
        bot.edit_message_text(
            ...
            call.message.chat.id,
            call.message.message_id,
            reply_markup=verification_method_keyboard()
        )
        bot.answer_callback_query(call.id)
    except Exception as e:
        print("Start verification error:", e)
...
def handle_verify_choice(call):
    ...
    clear_verification_state(uid, call.message.chat.id)
    ...
    try:
        ...
        if choice == "gmail":
            ...
            if not get_setting("sms_enabled", False):
                bot.answer_callback_query(call.id, "SMS verification is currently closed.", show_alert=True)
                return
            msg = bot.send_message(call.message.chat.id, "📱 Send your phone number with country code (e.g. +252... or +251...):", reply_markup=kb)
            bot.register_next_step_handler(msg, process_verification_phone)
        elif choice == "whatsapp":
            if not SMESS_API_KEY or not get_setting("whatsapp_verify_enabled", False):
                bot.answer_callback_query(call.id, "WhatsApp verification is currently closed.", show_alert=True)
                return
            msg = bot.send_message(call.message.chat.id, "🟢 Send your WhatsApp number with country code (e.g. +252... or +251...):", reply_markup=kb)
            bot.register_next_step_handler(msg, process_verification_whatsapp)
        else:
            bot.answer_callback_query(call.id, "Unknown verification method.", show_alert=True)
            return
        bot.answer_callback_query(call.id)
    except Exception as e:
...
def cancel_verify_process(call):
    ...
    try:
        bot.edit_message_text(
            "❌ <b>Verification cancelled.</b>\n\nYou can start again from <b>👤 Profile → Verify</b> whenever you are ready.",
            call.message.chat.id,
            call.message.message_id,
            reply_markup=None
        )
        ...
        try:
            bot.send_message(call.message.chat.id, "❌ Verification cancelled.", reply_markup=user_menu(is_admin(call.from_user.id)))
        except Exception:
            pass


@bot.callback_query_handler(func=lambda call: call.data == "cancel_link_verify")
def cancel_link_verify(call):
    uid = call.from_user.id
    verify_pending.pop(uid, None)
    verify_pending.pop(str(uid), None)
    pending_links.pop(uid, None)
    clear_verification_state(uid, call.message.chat.id)
    try:
        bot.answer_callback_query(call.id, "Verification cancelled")
        bot.edit_message_text("❌ Link verification cancelled. Send another link when you are ready.", call.message.chat.id, call.message.message_id, reply_markup=None)
    except Exception:
        pass


# Helper to auto-expire all verification sessions after one minute.
def delayed_cancel_session(chat_id, message_id, uid):
    time.sleep(60)
    uid = str(uid)
    active = uid in email_verify_pending or uid in phone_verify_pending or uid in whatsapp_verify_pending
    if active:
        clear_verification_state(uid, chat_id)
        try:
            bot.edit_message_text("⌛ Verification session expired. Please start again from Profile.", chat_id, message_id, reply_markup=None)
        except Exception:
            pass
...
def process_verification_email(m):
    ...
    menu_buttons = ["👤 Profile", "👑 ADMIN PANEL", "💰 BALANCE", "💸 WITHDRAWAL", "👥 REFERRAL", "🆔 GET ID", "☎️ CUSTOMER", "🤖CUSTOMER AI", "🔙 BACK MAIN MENU", "💳 PAY", "💳 PAY", "💸 WITHDRAWAL"]
    if (m.text or "").strip() in menu_buttons:
        clear_verification_state(uid, m.chat.id)
        try:
            bot.send_message(m.chat.id, "❌ Verification cancelled.", reply_markup=user_menu(is_admin(m.from_user.id)))
        except Exception:
            pass
        return
...
# ----- WHATSAPP VERIFICATION LOGIC (SMESS) ----- #

def process_verification_whatsapp(m):
    uid = str(m.from_user.id)
    phone_input = (m.text or "").strip().replace(" ", "")
    if phone_input in ["👤 Profile", "💰 BALANCE", "💸 WITHDRAWAL", "👥 REFERRAL", "🆔 GET ID", "☎️ CUSTOMER", "🤖CUSTOMER AI", "💳 PAY", "🔙 BACK MAIN MENU"]:
        clear_verification_state(uid, m.chat.id)
        bot.send_message(m.chat.id, "❌ Verification cancelled.", reply_markup=user_menu(is_admin(m.from_user.id)))
        return
    if not re.fullmatch(r"\+[1-9]\d{9,14}", phone_input):
        msg = bot.send_message(m.chat.id, "❌ Invalid WhatsApp number. Use full international format, e.g. +252xxxxxxxxx")
        bot.register_next_step_handler(msg, process_verification_whatsapp)
        return
    code = str(random.randint(100000, 999999))
    now = time.time()
    whatsapp_verify_pending[uid] = {"phone": phone_input, "code": code, "time": now, "last_resend": now}
    ok, detail = send_smess_whatsapp(phone_input, code, uid)
    if not ok:
        print("SMESS WhatsApp OTP error:", detail)
        whatsapp_verify_pending.pop(uid, None)
        bot.send_message(m.chat.id, "❌ WhatsApp OTP could not be queued. Check SMESS connection/API key and that the sender WhatsApp number is connected in SMESS.")
        return
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("🔄 Resend WhatsApp", callback_data="resend_whatsapp_code"), InlineKeyboardButton("❌ Cancel", callback_data="cancel_verify_process"))
    msg = bot.send_message(m.chat.id, f"📲 WhatsApp OTP sent to {phone_input}. Enter the 6-digit code:", reply_markup=kb)
...
def resend_whatsapp_code_callback(call):
    ...
    if not data:
        bot.answer_callback_query(call.id, "Session expired. Start verification again.", show_alert=True)
        return
    elapsed = time.time() - data.get("last_resend", data.get("time", 0))
    cooldown = 60
    if elapsed < cooldown:
        bot.answer_callback_query(call.id, f"Please wait {int(cooldown-elapsed)}s.", show_alert=True)
        return
    code = str(random.randint(100000, 999999))
    data["code"] = code
    data["last_resend"] = time.time()
    ok, detail = send_smess_whatsapp(data["phone"], code, uid)
    if not ok:
        print("SMESS resend error:", detail)
        bot.answer_callback_query(call.id, "Could not send WhatsApp OTP.", show_alert=True)
        return
    bot.answer_callback_query(call.id, "✅ New WhatsApp OTP sent")
    msg = bot.send_message(call.message.chat.id, "Enter the new 6-digit WhatsApp code:")
    bot.register_next_step_handler(msg, process_whatsapp_code)
...
def process_whatsapp_code(m):
    ...
    data = whatsapp_verify_pending.get(uid)
    if not data:
        bot.send_message(m.chat.id, "❌ WhatsApp verification session expired. Start again from Profile.")
        return
    code_input = (m.text or "").strip()
    if code_input in ["👤 Profile", "💰 BALANCE", "💸 WITHDRAWAL", "👥 REFERRAL", "🆔 GET ID", "☎️ CUSTOMER", "🤖CUSTOMER AI", "💳 PAY", "🔙 BACK MAIN MENU"]:
        clear_verification_state(uid, m.chat.id)
        bot.send_message(m.chat.id, "❌ Verification cancelled.", reply_markup=user_menu(is_admin(m.from_user.id)))
        return
    if not re.fullmatch(r"\d{6}", code_input):
        msg = bot.send_message(m.chat.id, "⚠️ Enter the 6-digit WhatsApp code:")
        bot.register_next_step_handler(msg, process_whatsapp_code)
        return
    if code_input != data["code"]:
        msg = bot.send_message(m.chat.id, "❌ Incorrect WhatsApp code. Try again:")
        bot.register_next_step_handler(msg, process_whatsapp_code)
        return
    users[uid]["verified"] = True
    users[uid]["phone"] = data["phone"]
    ...
    try:
        ...
        pass
    bot.send_message(m.chat.id, "✅ WhatsApp verification successful. Your account is now verified.", reply_markup=user_menu(is_admin(m.from_user.id)))
...
def _safe_send_file(chat_id, path, caption=""):
...
def download_media(chat_id, link, message_id):
    """Download media from all configured platforms using yt-dlp.

    TikTok image/carousel posts are allowed by keeping noplaylist=False and
    collecting both image and video outputs. Single-video platforms still use
    the normal best-video+audio fallback.
    """
    platform = detect_platform(link)
    supported = "TikTok (video/photo), Facebook, Instagram, Pinterest, Snapchat, X/Twitter and YouTube"
    if platform == "unknown":
        try:
            bot.edit_message_text(f"❌ Unsupported or invalid link. Supported: {supported}.", chat_id, message_id)
        except Exception:
            pass
        return
    ...
    base_opts = {
        "quiet": True,
        "no_warnings": True,
        # Required for TikTok carousel/photo posts and other multi-entry posts.
        "noplaylist": False,
        "retries": 5,
        "fragment_retries": 5,
    ...
    try:
        # Only inspect YouTube first because duration limits are enforced there.
        if platform == "youtube":
            with yt_dlp.YoutubeDL({**base_opts, "extract_flat": True, "noplaylist": True}) as ydl:
                info = ydl.extract_info(link, download=False)
            duration = info.get("duration") if info else None
            if duration and duration > max_duration:
                bot.edit_message_text(
                    f"❌ YouTube video is too long. Maximum is {max_duration // 60} minutes.",
                    chat_id,
                    message_id
                )
                return

        # bestvideo+bestaudio for video; /best is the important fallback for
        # photo-only posts such as TikTok/Pinterest/Instagram image posts.
        ydl_opts = {
            **base_opts,
            "outtmpl": os.path.join(tmp_dir, "%(id)s-%(playlist_index)03d.%(ext)s"),
        ...
        media_files = [
            p for p in files
            if _is_image_file(p) or _is_video_file(p) or _is_audio_file(p)
        ]
        if not media_files:
            raise RuntimeError("No downloadable media was produced by yt-dlp.")

        sent = 0
        for path in media_files[:20]:
            _safe_send_file(chat_id, path, f"🎬 {platform.title()} downloaded successfully")
            sent += 1
        ...
        try:
            bot.edit_message_text(
                f"❌ Download failed for {platform.title()}.\n\n{str(e)[:700]}",
                chat_id,
                message_id
            )
        except Exception:
            try:
                bot.send_message(chat_id, f"❌ Download failed: {str(e)[:700]}")
            except Exception:
                pass
    finally:
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            if user_id in pending_links:
                ...
                queue_download_or_verify(user_id, link, user_id)
            else:
...
# ================= LINK HANDLER =================
def queue_download_or_verify(user_id, link, chat_id=None):
    """After channel checks, enforce verification again before starting a download."""
    uid = str(user_id)
    ...
    if VERIFY_ENABLED and not users.get(uid, {}).get("verified", False):
        ...
        kb.add(InlineKeyboardButton("❌ Cancel Verification", callback_data="cancel_link_verify"))
        bot.send_message(chat_id, "🔐 Verification Required\n\nChoose a verification method or cancel.", reply_markup=kb)
        return False

    msg = bot.send_message(chat_id, "⬇️ Processing your video...")
    if is_quick_access(user_id):
        vip_executor.submit(download_media, chat_id, link, msg.message_id)
    else:
        normal_executor.submit(download_media, chat_id, link, msg.message_id)
    return True
...
def handle_links(message):
    ...
    try:
        queue_download_or_verify(user_id, link, message.chat.id)
    except Exception as e:
        print("Download queue error:", e)
    ...
    if joined_all:
        ...
        if user_id in pending_links:
            ...
            try:
                ...
                print("Queue after join error:", e)
        else:
            try:
