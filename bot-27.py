...
def _rapidapi_youtube_details(video_id):
    ...
    if not isinstance(data, dict):
        raise RuntimeError("RapidAPI returned an invalid response.")
    return data
...
def _rapid_formats(data):
    ...
    for d in _rapid_walk(data):
        ...
        if video or audio:
            seen.add(url)
            out.append({"url": url, "height": int(height), "width": int(width), "fps": fps, "audio": audio, "video": video, "text": text})
    return out
...
def _rapid_choose(data, quality):
    ...
    if not fs:
        raise RuntimeError("RapidAPI returned no downloadable video formats.")
...
def _rapid_download(link, tmp_dir, quality, max_seconds, chat_id=None):
    ...
    if not vid:
        raise RuntimeError("Could not extract the YouTube video ID.")
