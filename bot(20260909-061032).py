...
def _ai_help_text(lang="en", admin=False):
    ...
    base=(
        ...
        "/crypto — Crypto access, minimum and fees\n"
        "/downloads — Download system\n"
        "/music — Music/MP3\n"
        "/withdrawal — Your withdrawal status\n"
        "/referral — Referral information\n"
        "/system — User-facing system overview\n"
...
def _ai_user_context(uid):
    ...
    return {
        ...
        "balance_usd":balance_usd_value(uid),"blocked":blocked_amount(uid),
...
def _ai_user_report(uid, admin=False, lang="en"):
    ...
    if lang=="so":
        lines=["🤖 <b>CUSTOMER AI — XOGTA ACCOUNT-KA</b>","",
               ...
               f"🔒 Blocked: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>",f"📊 Portfolio: <b>${c['portfolio_usd']:,.2f}</b>",
...
def _ai_system_overview(lang="en", admin=False):
    ...
    if lang=="so":
        return ("🧠 <b>SYSTEM OVERVIEW</b>\n\n"
                "Waxaan caawin karaa Balance, Blocked/Balance Lock, Premium/Trial, Verification, Gmail, D7 SMS, WaForge WhatsApp, Crypto, Fees/Gas, YouTube, Downloads, Music/MP3, Withdrawal, Referral, Force Join, Profile iyo errors.\n\n"
                f"💎 Premium: <b>{sc['premium']}</b>\n🪙 Crypto: <b>{sc['crypto']}</b>\n⏳ Crypto Hold: <b>{sc['crypto_hold']}</b>\n"
                f"💰 Crypto minimum: <b>${sc['crypto_min_deposit']:.2f}</b>\n💸 Crypto fee: <b>{sc['crypto_fee_percent']:.2f}%</b>\n"
                f"🔐 Premium verification: <b>{sc['premium_verification']}</b>\n📲 SMS: <b>{sc['sms']}</b> | WhatsApp: <b>{sc['whatsapp']}</b>\n"
                f"▶️ YouTube API: <b>{'READY' if sc['rapidapi_youtube'] else 'NOT SET'}</b>\n📢 Force Join: <b>{'OPEN' if sc['force_join'] else 'CLOSED'}</b>\n\n"
                "Haddii aad dhib qabto, sharax waxa dhacay; waxaan kuu tilmaamayaa waxa la hubinayo.")
    text=("🧠 <b>SYSTEM OVERVIEW</b>\n\n"
          "I can help with Balance, Blocked/Balance Lock, Premium/Trial, Verification, Gmail, D7 SMS, WaForge WhatsApp, Crypto, Fees/Gas, YouTube, Downloads, Music/MP3, Withdrawal, Referral, Force Join, Profile and common errors.\n\n"
          f"💎 Premium: <b>{sc['premium']}</b>\n🪙 Crypto: <b>{sc['crypto']}</b>\n⏳ Crypto Hold: <b>{sc['crypto_hold']}</b>\n"
          f"💰 Crypto minimum: <b>${sc['crypto_min_deposit']:.2f}</b>\n💸 Crypto fee: <b>{sc['crypto_fee_percent']:.2f}%</b>\n"
          f"🔐 Premium verification: <b>{sc['premium_verification']}</b>\n📲 SMS: <b>{sc['sms']}</b> | WhatsApp: <b>{sc['whatsapp']}</b>\n"
          f"▶️ YouTube API: <b>{'READY' if sc['rapidapi_youtube'] else 'NOT SET'}</b>\n📢 Force Join: <b>{'OPEN' if sc['force_join'] else 'CLOSED'}</b>\n\n"
          "Tell me what happened and I will guide you through the correct check or fix.")
    return text
...
def _ai_intent_answer(uid, q, admin=False, lang="en"):
    ...
    if any(x in q for x in ["balance","haraag","haraaga","my money","how much money","lacag intee"]):
        if c["balance_locked"]:
            if lang=="so": return _ai_user_report(uid,False,lang)+"\n\n🔒 Balance-kaagu hadda waa xiran yahay; lacagta waxaa lagu hayaa Balance Lock ilaa activation-ka saxda ah."
            return _ai_user_report(uid,False,lang)+"\n\n🔒 Your balance is currently closed by Balance Lock; the saved funds require the correct activation process."
        if lang=="so": return f"💰 <b>HARAAGAAGA</b>\n\nAvailable: <b>{html.escape(format_asset(c['asset'],c['balance']))}</b>\nUSD value: <b>${c['balance_usd']:,.2f}</b>\nBlocked: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>\nPortfolio: <b>${c['portfolio_usd']:,.2f}</b>\nTotal assets: <b>${c['total_assets_usd']:,.2f}</b>"
        return f"💰 <b>YOUR BALANCE</b>\n\nAvailable: <b>{html.escape(format_asset(c['asset'],c['balance']))}</b>\nUSD value: <b>${c['balance_usd']:,.2f}</b>\nBlocked: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>\nPortfolio: <b>${c['portfolio_usd']:,.2f}</b>\nTotal assets: <b>${c['total_assets_usd']:,.2f}</b>"

    if any(x in q for x in ["blocked","block","xiran","locked","hold","lacagta la qabtay"]):
        if lang=="so": return f"🔒 <b>BLOCKED / HOLD</b>\n\nBlocked amount: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>\nBalance Lock: <b>{'YES' if c['balance_locked'] else 'NO'}</b>\n\nBlocked lacag waxay ka imaan kartaa withdrawal pending. Crypto conversion-na waxay yeelan kartaa 1-hour hold haddii Hold system-ku furan yahay. Balance Lock waa xannibaad uu Admin sameeyo."
        return f"🔒 <b>BLOCKED / HOLD</b>\n\nBlocked amount: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>\nBalance Lock: <b>{'YES' if c['balance_locked'] else 'NO'}</b>\n\nPending withdrawals can appear in Blocked Amount. Crypto conversions may use a 1-hour hold when enabled. Balance Lock is an admin-controlled account lock."

    if any(x in q for x in ["premium","vip","trial","tijaabo"]):
        if lang=="so": return f"💎 <b>PREMIUM</b>\n\nStatus: <b>{'ACTIVE' if c['premium'] else 'NOT ACTIVE'}</b>\nExpiry: <b>{html.escape(c['premium_until'])}</b>\nTrial available: <b>{'YES' if c['trial_available'] else 'NO'}</b>\nVerification required: <b>{'YES' if premium_verification_required() else 'NO'}</b>\n\nPremium wuxuu bixiyaa quality sare, priority/high concurrency, full YouTube access, platforms dheeraad ah iyo download speed aan artificial loo xadidin."
        return f"💎 <b>PREMIUM</b>\n\nStatus: <b>{'ACTIVE' if c['premium'] else 'NOT ACTIVE'}</b>\nExpiry: <b>{html.escape(c['premium_until'])}</b>\nTrial available: <b>{'YES' if c['trial_available'] else 'NO'}</b>\nVerification required: <b>{'YES' if premium_verification_required() else 'NO'}</b>\n\nPremium provides higher quality, priority/high concurrency, full YouTube access, extra platforms and no artificial download-speed throttle."

    if any(x in q for x in ["whatsapp","waforge"]):
        if lang=="so": return f"📱 <b>WHATSAPP VERIFICATION</b>\n\nStatus: <b>{'OPEN' if bool(get_setting('whatsapp_verify_enabled',False)) and bool(WAFORGE_API_KEY) else 'CLOSED'}</b>\nProvider: <b>WaForge</b>\nOTP: 6 digits | Expiry: 5 minutes | Resend cooldown: 60 seconds.\n\nD7 looma isticmaalo WhatsApp; D7 waa SMS oo keliya."
        return f"📱 <b>WHATSAPP VERIFICATION</b>\n\nStatus: <b>{'OPEN' if bool(get_setting('whatsapp_verify_enabled',False)) and bool(WAFORGE_API_KEY) else 'CLOSED'}</b>\nProvider: <b>WaForge</b>\nOTP: 6 digits | Expiry: 5 minutes | Resend cooldown: 60 seconds.\n\nD7 is not used for WhatsApp; D7 is SMS-only."

    if any(x in q for x in ["sms","d7","phone verification"]):
        if lang=="so": return f"📲 <b>SMS VERIFICATION</b>\n\nStatus: <b>{'OPEN' if bool(get_setting('sms_enabled',False)) else 'CLOSED'}</b>\nProvider: <b>D7 Networks</b>\nOTP: 6 digits | Local expiry: 10 minutes. WhatsApp waa WaForge oo gooni ah."
        return f"📲 <b>SMS VERIFICATION</b>\n\nStatus: <b>{'OPEN' if bool(get_setting('sms_enabled',False)) else 'CLOSED'}</b>\nProvider: <b>D7 Networks</b>\nOTP: 6 digits | Local expiry: 10 minutes. WhatsApp uses WaForge separately."

    if any(x in q for x in ["gmail","email verification","email code"]):
        if lang=="so": return "📧 <b>GMAIL / EMAIL VERIFICATION</b>\n\nOTP waa 6 digits, local session-kuna 10 daqiiqo ayuu dhacaa. Resend-ku wuxuu bilaabaa timer cusub."
        return "📧 <b>GMAIL / EMAIL VERIFICATION</b>\n\nThe OTP is 6 digits and the local session expires after 10 minutes. Resend starts a fresh timer."

    if any(x in q for x in ["verify","verification","otp","code","xaqiijin"]):
        if lang=="so": return f"🔐 <b>VERIFICATION</b>\n\nXaaladdaada: <b>{'Verified' if c['verified'] else 'Not Verified'}</b>\nMethod: <b>{html.escape(str(c['verification_method']))}</b>\n\nHababka la heli karo waxay ku xiran yihiin Admin settings: Gmail, D7 SMS iyo WaForge WhatsApp."
        return f"🔐 <b>VERIFICATION</b>\n\nYour status: <b>{'Verified' if c['verified'] else 'Not Verified'}</b>\nMethod: <b>{html.escape(str(c['verification_method']))}</b>\n\nAvailable methods depend on admin settings: Gmail, D7 SMS and WaForge WhatsApp."
    ...
    if any(x in q for x in ["crypto","bitcoin","btc","ethereum","eth","usdt","coin"]):
        ...
        if lang=="so": return f"🪙 <b>CRYPTO</b>\n\nStatus: <b>OPEN</b>\nDeposited/credited: <b>${c['deposited']:,.2f}</b>\nMinimum: <b>${c['crypto_minimum']:,.2f}</b>\nAccess: <b>{'ALLOWED' if c['crypto_allowed'] else 'LOCKED'}</b>\nRemaining: <b>${c['crypto_remaining']:,.2f}</b>\nFee: <b>{crypto_fee_percent():.2f}%</b>\n\nCrypto conversion-ka gudaha bot-ka ma aha blockchain transaction toos ah."
        return f"🪙 <b>CRYPTO</b>\n\nStatus: <b>OPEN</b>\nDeposited/credited: <b>${c['deposited']:,.2f}</b>\nMinimum: <b>${c['crypto_minimum']:,.2f}</b>\nAccess: <b>{'ALLOWED' if c['crypto_allowed'] else 'LOCKED'}</b>\nRemaining: <b>${c['crypto_remaining']:,.2f}</b>\nFee: <b>{crypto_fee_percent():.2f}%</b>\n\nThe in-bot crypto conversion is an internal ledger conversion, not a direct blockchain broadcast."

    if any(x in q for x in ["youtube","yt","shorts"]):
        if lang=="so": return "▶️ <b>YOUTUBE</b>\n\nYouTube videos iyo Shorts waa Premium-only, marka laga reebo Admin ama special access. Downloader-ku wuxuu isku dayaa RapidAPI, configured fallback/Cobalt iyo ugu dambayn yt-dlp. Video gaar ah wuu diidi karaa sababo YouTube restrictions, cookies ama provider."
        return "▶️ <b>YOUTUBE</b>\n\nYouTube videos and Shorts are Premium-only, except Admin or special-access users. The downloader tries RapidAPI, configured Cobalt/fallbacks and finally yt-dlp. A specific video may fail because of YouTube restrictions, cookies or provider availability."

    if any(x in q for x in ["download","video","tiktok","instagram","facebook","pinterest","snapchat","twitter","x.com"]):
        if lang=="so": return f"📥 <b>DOWNLOADS</b>\n\nDir public supported link. System-ku wuxuu isku dayaa provider fallbacks haddii kii hore diido. Chat Actions waxay muujiyaan processing/upload. Free limit: <b>{int(get_setting('free_max_minutes',FREE_MAX_MINUTES_DEFAULT) or FREE_MAX_MINUTES_DEFAULT)} min</b>; Premium: <b>{int(get_setting('premium_max_minutes',PREMIUM_MAX_MINUTES_DEFAULT) or PREMIUM_MAX_MINUTES_DEFAULT)} min</b>. File-size limit Admin ayaa maamula."
        return f"📥 <b>DOWNLOADS</b>\n\nSend a public supported link. The system tries provider fallbacks when the first method fails. Chat Actions show processing/upload. Free limit: <b>{int(get_setting('free_max_minutes',FREE_MAX_MINUTES_DEFAULT) or FREE_MAX_MINUTES_DEFAULT)} min</b>; Premium: <b>{int(get_setting('premium_max_minutes',PREMIUM_MAX_MINUTES_DEFAULT) or PREMIUM_MAX_MINUTES_DEFAULT)} min</b>. File-size limits are admin-controlled."
    ...
    if any(x in q for x in ["banned","ban","mamnuuc","mamnuucay"]):
        if lang=="so": return f"🚫 <b>BAN STATUS</b>\n\nAccount-kaaga: <b>{"BANNED" if c['banned'] else "NOT BANNED"}</b>."
        return f"🚫 <b>BAN STATUS</b>\n\nYour account: <b>{"BANNED" if c['banned'] else "NOT BANNED"}</b>."
    ...
    if any(x in q for x in ["fee","gas","commission"]):
        if lang=="so": return f"💸 <b>FEES</b>\n\nCrypto platform fee: <b>{crypto_fee_percent():.2f}%</b>. Gas waa estimate/reserve marka loo baahdo; internal conversion-ku blockchain transaction ma diraayo."
        return f"💸 <b>FEES</b>\n\nCrypto platform fee: <b>{crypto_fee_percent():.2f}%</b>. Gas is an estimate/reserve when applicable; internal conversion does not broadcast a blockchain transaction."
    ...
    if lang=="so":
        return ("🤖 <b>Waan fahmay.</b>\n\nWaxaan kaa caawin karaa dhammaan qaybaha system-ka: Balance, Balance Lock, Blocked/Hold, Premium, Trial, Verification, Gmail, D7 SMS, WaForge WhatsApp, Crypto, Fees, YouTube, Downloads, Music/MP3, Withdrawal, Referral, Force Join, Profile iyo errors.\n\nQor su'aashaada si faahfaahsan; button looma baahna.")
    return ("🤖 <b>I understand.</b>\n\nI can help with the complete system: Balance, Balance Lock, Blocked/Hold, Premium, Trial, Verification, Gmail, D7 SMS, WaForge WhatsApp, Crypto, Fees, YouTube, Downloads, Music/MP3, Withdrawal, Referral, Force Join, Profile and common errors.\n\nDescribe what happened in your own words; no button is required.")
