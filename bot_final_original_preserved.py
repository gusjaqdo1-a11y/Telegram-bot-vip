...
def _storage_report_text():
    ...
    # If a provider quota is known through an environment variable, calculate remaining space.
    try:
        limit_gb=float(os.getenv("MONGO_STORAGE_LIMIT_GB", "0") or 0)
    ...
    if limit_gb>0:
        ...
        lines += ["📊 <b>CONFIGURED MONGO LIMIT</b>",f"• Limit: <b>{_format_bytes(limit)}</b>",f"• Used: <b>{_format_bytes(used)}</b>",f"• Remaining: <b>{_format_bytes(max(0,limit-used))}</b>"]
    else:
        lines += ["ℹ️ <b>MongoDB provider quota:</b> Not exposed by dbStats. Set <code>MONGO_STORAGE_LIMIT_GB</code> if you want the bot to calculate remaining quota."]
    return "\n".join(lines)
