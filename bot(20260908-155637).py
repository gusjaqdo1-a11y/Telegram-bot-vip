...
def process_whatsapp_code(m):
    ...
    users[uid]["verified"]=True; users[uid]["phone"]=data["phone"]; users[uid]["whatsapp_verified"]=True; users[uid]["sticker"]=users[uid].get("sticker") or "🌟"; save_user(uid); whatsapp_verify_pending.pop(uid,None)
    trial_activated=activate_pending_trial(uid,m.chat.id)
...
def _rapidapi_youtube_details(video_id):
    ...
    params = {"videoId": video_id, "urlAccess": "normal", "videos": "true", "audios": "true", "lang": "en-US"}
