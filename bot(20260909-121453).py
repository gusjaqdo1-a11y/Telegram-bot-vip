...
def send_waforge_whatsapp(phone_number, user_id):
    ...
    # Current WaForge contract: {to: "+country..."}. Some older accounts accepted {phone: ...}.
    ...
    for url in _waforge_request_urls(WAFORGE_OTP_SEND_URL, WAFORGE_OTP_SEND_LEGACY_URL):
        try:
            ...
            if r.status_code in (200,201,202):
                rid=body.get("id") or body.get("request_id") or body.get("uuid") or body.get("session_id")
                return True,body,rid
            # If an old endpoint explicitly rejects the new body, try its legacy field once.
            if r.status_code in (400,422) and url != WAFORGE_OTP_SEND_LEGACY_URL:
                r2=requests.post(url,json={"phone":phone_number},headers=headers,timeout=30)
                try: body2=r2.json()
                except Exception: body2={"raw":r2.text}
                last=(r2.status_code,body2)
                if r2.status_code in (200,201,202):
                    rid=body2.get("id") or body2.get("request_id") or body2.get("uuid") or body2.get("session_id")
                    return True,body2,rid
        except Exception as e:
            last=(None,str(e))
    return False,f"WaForge OTP send failed: {last}",None

def verify_waforge_whatsapp(phone_number, code, request_id=None):
    """Verify against the exact WaForge OTP session when the provider returns one."""
    if not WAFORGE_API_KEY:
        return False,"WAFORGE_API_KEY is not configured"
    headers={"Authorization":f"Bearer {WAFORGE_API_KEY}","Content-Type":"application/json","Accept":"application/json"}
    # Current WaForge contract verifies with `to` + `code`. If a session/request id is returned,
    # include it as an extra field without depending on it.
    payload={"to":phone_number,"code":str(code)}
    if request_id:
        payload["request_id"]=str(request_id)
    last=None
    for url in _waforge_request_urls(WAFORGE_OTP_VERIFY_URL, WAFORGE_OTP_VERIFY_LEGACY_URL):
        try:
            r=requests.post(url,json=payload,headers=headers,timeout=30)
            try: body=r.json()
            except Exception: body={"raw":r.text}
            ...
            ok=(r.status_code in (200,201) and (body.get("verified") is True or body.get("success") is True or status in {"verified","approved","success"} or body.get("matched") is True))
            if ok:
                return True,body
            # Compatibility with the older WaForge schema.
            if r.status_code in (400,401,404,422) and url != WAFORGE_OTP_VERIFY_LEGACY_URL:
                legacy_payload={"phone":phone_number,"code":str(code)}
                if request_id: legacy_payload["request_id"]=str(request_id)
                r2=requests.post(url,json=legacy_payload,headers=headers,timeout=30)
                try: body2=r2.json()
                except Exception: body2={"raw":r2.text}
                last=(r2.status_code,body2)
                status2=str(body2.get("status","")).lower() if isinstance(body2,dict) else ""
                ok2=(r2.status_code in (200,201) and (body2.get("verified") is True or body2.get("success") is True or status2 in {"verified","approved","success"} or body2.get("matched") is True))
                if ok2:
                    return True,body2
        except Exception as e:
            last=(None,str(e))
...
def _music_artist_from_info(info):
    """Return the lead/first real artist. Never use the uploader as the artist."""
    info=info or {}

    # Explicit artist metadata is the most reliable source.
    for key in ("artist", "track_artist", "album_artist", "creator"):
        value=info.get(key)
        if isinstance(value,str) and value.strip():
            value=value.strip()
            # If a provider returns multiple artists in one string, keep the lead artist.
    ...
    # Some extractors expose artists as a list. Pick the first/lead artist.
    ...
    # IMPORTANT: uploader/channel is NOT a music artist. If no artist metadata exists,
    # the caller will display "Original" exactly as requested.
    return ""

def _music_itunes_cover(title, artist, tmp_dir):
    """Find real song/album artwork from iTunes instead of using a TikTok/video thumbnail."""
    title=(title or "").strip()
    artist=(artist or "").strip()
    if not title or title.lower() in {"unknown title", "original sound"}:
        return None

    terms=[]
    if artist and artist.lower() != "original":
    ...
    for term in terms:
        try:
            r=requests.get(
                ...
                params={"term":term, "media":"music", "entity":"song", "limit":8, "country":"US"},
            ...
            # Prefer a result whose artist/title closely matches the extracted metadata.
            def score(item):
                st=str(item.get("trackName") or "").lower()
                sa=str(item.get("artistName") or "").lower()
                q=title.lower()
                score=0
                if q and (q in st or st in q): score+=5
                if artist and artist.lower() != "original" and artist.lower() in sa: score+=8
                if item.get("artworkUrl100"): score+=1
                return score
            ...
            img_r=requests.get(artwork,headers={"User-Agent":"Mozilla/5.0"},timeout=10)
            img_r.raise_for_status()
            if len(img_r.content)<1000:
                continue
            raw=os.path.join(tmp_dir,"itunes_cover.jpg")
            with open(raw,"wb") as f: f.write(img_r.content)
            try:
                from PIL import Image
                img=Image.open(raw).convert("RGB")
                jpg=os.path.join(tmp_dir,"cover.jpg")
            ...
            print("iTunes music cover lookup failed:",repr(e))
    return None

def _download_music_cover(info, tmp_dir, title=None, artist=None):
    """Use actual song/album artwork first; video thumbnail is only the final fallback."""
    info=info or {}
    title=title or info.get("track") or info.get("title") or info.get("fulltitle") or ""
    artist=artist or _music_artist_from_info(info) or "Original"

    # 1) Real song cover from iTunes. This prevents TikTok/Instagram video covers
    #    from appearing as the MP3 artwork when the song can be identified.
    cover=_music_itunes_cover(title, artist, tmp_dir)
    if cover:
        return cover

    # 2) Some music platforms already provide album artwork as their thumbnail.
    #    Use it only if no dedicated artwork service result was found.
    urls=[]
    thumb=info.get("thumbnail")
    if isinstance(thumb,str) and thumb.strip(): urls.append(thumb.strip())
    thumbs=info.get("thumbnails") or []
    if isinstance(thumbs,list):
        ranked=[]
        for t in thumbs:
            if isinstance(t,dict) and t.get("url"):
                try: ranked.append((int(t.get("width") or 0)*int(t.get("height") or 0),t["url"]))
                except Exception: ranked.append((0,t["url"]))
    ...
    for idx,url in enumerate(urls[:4]):
        try:
            ...
            raw=os.path.join(tmp_dir,f"cover_{idx}.jpg")
            ...
            print("Music cover download failed:",repr(e))
    return None
...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    try:
        ...
        send_action(chat_id, "upload_audio")
        info = info or {}
        title = info.get("track") or info.get("title") or info.get("fulltitle") or "Unknown title"
        artist = _music_artist_from_info(info) or "Original"
        cover_path = _download_music_cover(info, tmp_dir, title=title, artist=artist)
        caption = f"🎵 {title}\n👤 Artist: {artist}\n\n{DOWNLOAD_CAPTION}"
        with open(path, "rb") as audio:
            if cover_path and os.path.isfile(cover_path):
                with open(cover_path, "rb") as thumb:
                    bot.send_audio(chat_id, audio, caption=caption, title=title, performer=artist, thumb=thumb)
            else:
...
def balance_handler(m):
    ...
    if not crypto_open or not has_crypto:
        ...
        text=(
            ...
            + pending_block +
            "━━━━━━━━━━━━━━━━━━\n"
            "ℹ️ <i>Pending appears only when you have a withdrawal waiting for admin approval.\n"
