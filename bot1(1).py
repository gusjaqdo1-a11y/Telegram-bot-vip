...
def premium_required_for_platform(platform, uid, link=None):
    ...
    if platform in premium_platform_keys():
        return True
    if platform == "youtube":
        # YouTube Shorts are always Free. Normal YouTube videos up to 12 minutes
        # are Free; longer videos require Premium unless an admin opens full access.
        if link and youtube_is_short(link):
            return False
        if youtube_full_free_enabled():
            return False
        duration=_youtube_duration_seconds(link) if link else None
        # Unknown duration is allowed to reach the downloader, whose YouTube
        # duration filter remains the final safety gate.
...
def download_media(chat_id, link, message_id, quality=None):
    ...
    if platform=="youtube" and not priority and not youtube_is_short(link) and not youtube_full_free_enabled():
        ...
        if duration and duration>YOUTUBE_FREE_MAX_MINUTES*60:
            try: bot.edit_message_text(f"💎 <b>Premium Required</b>\n\nThis YouTube video is {duration//60}:{duration%60:02d}. Free users can download YouTube videos up to <b>{YOUTUBE_FREE_MAX_MINUTES} minutes</b>.",chat_id,message_id,parse_mode="HTML")
            except Exception: pass
            return
...
def convert_link_to_mp3(chat_id, link, status_message_id, local_source=None, local_cache_dir=None):
    ...
    try:
        ...
        if rapid_mp3 and os.path.isfile(rapid_mp3):
            info={}
            prepared=None
        elif local_source and os.path.isfile(local_source):
            # Fast path: convert the exact already-downloaded video locally.
            # No provider request, no second YouTube login check, and no duplicate download.
            info={}; prepared=None
        ...
        if not source_title:
            ...
            if not raw_title and local_source:
                raw_title = _music_clean_text(os.path.splitext(os.path.basename(local_source))[0])
            source_title = raw_title if raw_title and not _music_is_original_label(raw_title) else "Unknown title"

        # Recognition is intentionally skipped on the local MUSIC fast path so the
        # MP3 can be sent immediately. Normal URL-to-MP3 conversion keeps recognition.
        recognized = {} if local_source else _music_recognize_acoustid(path)

        if not recognized and AUDD_API_TOKEN and not local_source:
        ...
        catalog_meta = {}
        if not local_source and (not recognized_title or not recognized_artist):
...
def music_callback_handler(call):
    ...
    try:
        ...
        status = bot.send_message(call.message.chat.id, "⚡ Converting the downloaded video to MP3...")
