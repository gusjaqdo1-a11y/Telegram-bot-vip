...
def _jamendo_song_search(query, limit=100):
    ...
    if not query or not JAMENDO_CLIENT_ID: return []
    base={"client_id":JAMENDO_CLIENT_ID,"format":"json","limit":100,"imagesize":600,"audiodlformat":"mp32","type":"single albumtrack","order":"relevance"}
    def call(extra):
        try:
            params=dict(base); params.update(extra); r=requests.get("https://api.jamendo.com/v3.0/tracks/",params=params,timeout=JAMENDO_TIMEOUT); r.raise_for_status(); body=r.json() or {}
            if (body.get("headers") or {}).get("status") not in (None,"success"): print("Jamendo API error:",body.get("headers")); return []
            return body.get("results") or []
        except Exception as e: print("Jamendo API request failed:",repr(e)); return []
    modes=[{"search":query},{"namesearch":query},{"artist_name":query}]
    words=[w for w in query.split() if len(w)>=2]
    if len(words)>1:
        longest=sorted(words,key=len,reverse=True)[0]; modes += [{"namesearch":longest},{"artist_name":longest}]
    raw=[]; seen_raw=set()
    for mode in modes:
        for offset in (0,100,200):
            batch=call({**mode,"offset":offset})
            if not batch: break
            for item in batch:
                iid=str(item.get("id") or "") if isinstance(item,dict) else ""
                if iid and iid not in seen_raw: seen_raw.add(iid); raw.append(item)
            if len(raw)>=max(int(limit)*3,150): break
        if len(raw)>=max(int(limit)*3,150): break
    rows=[]; seen=set()
    for item in raw:
        if not isinstance(item,dict): continue
        iid=str(item.get("id") or ""); title=_music_clean_text(item.get("name")); artist=_music_clean_text(item.get("artist_name")) or "Unknown artist"; download=item.get("audiodownload") or ""; allowed=bool(item.get("audiodownload_allowed"))
        if not iid or iid in seen or not title or not allowed or not download: continue
        seen.add(iid)
        try: duration=int(float(item.get("duration") or 0))
        except Exception: duration=0
        rows.append({"id":iid,"title":title,"artist":artist,"duration":duration,"album":_music_clean_text(item.get("album_name")),"cover":item.get("album_image") or item.get("image"),"download":download,"download_allowed":allowed,"license":item.get("license_ccurl") or "","source":"jamendo"})
...
def _song_caption(song):
    custom=str(get_setting("song_caption","") or "").strip()
    if not custom: return _default_song_caption(song)
    values={"title":_music_clean_text(song.get("title")) or "Unknown title","artist":_music_clean_text(song.get("artist")) or "Unknown artist","album":_music_clean_text(song.get("album")) or "","duration":_song_duration(song.get("duration")),"bot":"@"+_main_bot_username()}
    try: text=custom.format(**values)
    except Exception: text=custom
...
def _song_auto_search_should_handle(m):
    ...
    excluded={"💰 BALANCE","💸 WITHDRAWAL","👥 REFERRAL","🆔 GET ID","💎 PREMIUM","👤 Profile","☎️ CUSTOMER","🤖CUSTOMER AI","🔎 Search Song","👑 ADMIN PANEL","❌ CANCEL","📜 HISTORY"}
...
def song_pick_callback(call):
    ...
    normal_executor.submit(_download_jamendo_song,call.message.chat.id,status.message_id,x,str(call.from_user.id))
...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    try:
        ...
        caption = f"🎵 <b>{html.escape(title)}</b>\n👤 <b>Artist:</b> {html.escape(artist)}\n💿 <b>Album:</b> {html.escape(album or 'Unknown album')}\n\n{DOWNLOAD_CAPTION}"
        send_action(chat_id, "upload_audio")
        with open(path, "rb") as audio:
            if cover_path and os.path.isfile(cover_path):
...
def admin_add_song_caption(m):
    if not is_admin(m.from_user.id): return
    current=str(get_setting("song_caption","") or "")
    msg=bot.send_message(m.chat.id,"➕ <b>ADD SONG CAPTION</b>\n\nCurrent:\n<code>"+html.escape(current or "(using default caption)")+"</code>\n\nSend the caption for every downloaded song.\nPlaceholders: <code>{title}</code> <code>{artist}</code> <code>{album}</code> <code>{duration}</code> <code>{bot}</code>",parse_mode="HTML")
    bot.register_next_step_handler(msg,admin_add_song_caption_step)

def admin_add_song_caption_step(m):
    if not is_admin(m.from_user.id): return
    value=_music_clean_text(m.text)
    if not value:
        bot.send_message(m.chat.id,"❌ Caption cannot be empty.",reply_markup=admin_menu()); return
    set_setting("song_caption",value)
    bot.send_message(m.chat.id,"✅ <b>Song caption saved.</b>",parse_mode="HTML",reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "📝 DEFAULT CAPTION")
def admin_default_song_caption(m):
    if not is_admin(m.from_user.id): return
    set_setting("song_caption","")
    sample=_default_song_caption({"title":"Song Name","artist":"Artist Name"})
    bot.send_message(m.chat.id,"📝 <b>DEFAULT CAPTION ENABLED</b>\n\nEvery song will use:\n\n"+sample,parse_mode="HTML",reply_markup=admin_menu())
