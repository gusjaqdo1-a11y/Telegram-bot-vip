...
def balance_handler(m):
    ...
    if not crypto_open or not has_crypto:
        text=(
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            "ℹ️ <i>Pending = withdrawal requests waiting for admin approval.\n"
            "Blocked = funds explicitly blocked by Admin.</i>"
        )
    ...
    text += "\n<i>Pending withdrawals remain separate until Admin approves, rejects, or blocks them.</i>"
...
def _ai_user_report(uid, admin=False, lang="en"):
    ...
    if lang=="so":
        lines=["🤖 <b>CUSTOMER AI — XOGTA ACCOUNT-KA</b>","",
               ...
               f"🔒 Blocked: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>",f"⏳ Pending: <b>{html.escape(format_asset(c['asset'],c['pending']))}</b>",f"📊 Portfolio: <b>${c['portfolio_usd']:,.2f}</b>",
