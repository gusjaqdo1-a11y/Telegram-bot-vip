...
def get_admin_ids():
    try:
        ...
        pass
    return list(dict.fromkeys([PRIMARY_ADMIN_ID] + ADMIN_IDS))
...
def admin_menu():
    ...
    kb.add("✏️ EDIT CUSTOM", "💱 MIN CHANGE CURRENCY")
    kb.add("👑 ADMIN LIST", "💼 PORTFOLIO STATS")
...
@bot.message_handler(func=lambda m: m.text in ["✏️ EDIT CUSTOMER", "✏️ EDIT CUSTOM"])
def edit_customer_start(m):
    if not is_admin(m.from_user.id): return
    msg=bot.send_message(m.chat.id,"✏️ Send User ID and field/value.\nExamples:\n<code>123456789 username newname</code>\n<code>123456789 sticker VIP</code>\n<code>123456789 language en</code>")
    bot.register_next_step_handler(msg, edit_customer_process)

def edit_customer_process(m):
    if not is_admin(m.from_user.id): return
    parts=(m.text or '').split(maxsplit=2)
    if len(parts)!=3: bot.send_message(m.chat.id,"❌ Format: USER_ID FIELD VALUE"); return
    uid,field,value=parts; uid=resolve_user_input(uid)
    if not uid or uid not in users: bot.send_message(m.chat.id,"❌ User not found."); return
    allowed={'username','first_name','last_name','language','city','gender','sticker'}
    if field not in allowed: bot.send_message(m.chat.id,"❌ Allowed fields: username, first_name, last_name, language, city, gender, sticker"); return
    users[uid][field]=value; save_user(uid)
    bot.send_message(m.chat.id,f"✅ Updated <b>{field}</b> for user <code>{uid}</code>.")
...
def change_currency_button(m):
    ...
    bot.send_message(m.chat.id,
        ...
        f"Current value: <b>${usd_value:,.2f} USD</b>\n"
        f"Rate: <b>{market_rate_text(current)}</b>\n"
        f"Minimum Change: <b>${minimum:,.2f} USD</b>\n\n"
        f"Choose the currency/coin below. The full current balance will be converted after you confirm.\n"
        f"You can build a multi-asset portfolio by changing/investing again after adding more USD balance.",
...
def currency_select_callback(call):
    ...
    minimum=float(get_setting("currency_change_min_usd",5.0) or 0)
    if usd <= 0:
        bot.answer_callback_query(call.id,"❌ Your balance is empty.",show_alert=True); return
    if minimum > 0 and usd < minimum:
        bot.answer_callback_query(call.id,f"❌ Minimum Change Currency is ${minimum:,.2f} USD. Your balance is ${usd:,.2f}.",show_alert=True); return
...
def convert_confirm_callback(call):
    ...
    minimum=float(get_setting("currency_change_min_usd",5.0) or 0)
    if minimum > 0 and usd < minimum:
        bot.answer_callback_query(call.id,f"Minimum is ${minimum:,.2f} USD",show_alert=True); return
    now=datetime.now(timezone.utc)
    # Preserve the previous asset instead of deleting it. This makes repeated
    # investments possible while retaining the existing single-balance system.
    if current > 0 and new != "USD":
        add_portfolio_asset(uid, old, current)
    users[uid]["balance"]=round(new_amt,12); users[uid]["balance_asset"]=new; users[uid]["currency"]=new; users[uid].pop("pending_conversion",None); save_user(uid)
    hold_txt=""
