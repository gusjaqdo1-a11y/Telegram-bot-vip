...
PREMIUM_MAX_MINUTES_DEFAULT = int(os.getenv("PREMIUM_MAX_MINUTES", "1440"))
...
MAX_CONCURRENT_DOWNLOADS = int(os.getenv("MAX_CONCURRENT_DOWNLOADS", "40"))
...
PREMIUM_CONCURRENT_DOWNLOADS = int(os.getenv("PREMIUM_CONCURRENT_DOWNLOADS", "80"))
FREE_CONCURRENT_DOWNLOADS = int(os.getenv("FREE_CONCURRENT_DOWNLOADS", str(min(24, MAX_CONCURRENT_DOWNLOADS))))
QUICK_ACCESS_CONCURRENT_DOWNLOADS = int(os.getenv("QUICK_ACCESS_CONCURRENT_DOWNLOADS", "120"))
...
def promo_user_menu(m):
    ...
    if not bool(get_setting("promo_system_enabled",False)):
        bot.send_message(m.chat.id,"🎟 <b>PROMO CODE</b>\n\nPromo system is currently closed.",reply_markup=localized_user_menu(uid),parse_mode="HTML")
        return
    # Promo codes are secret. Opening the system must NEVER reveal an existing
    # code. Users can only claim a code after the admin broadcasts it with SEND
    # ALL (or otherwise sends the exact code to them).
    ...
    msg=bot.send_message(
        ...
        "Enter promo code:",
        reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("❌ Cancel",callback_data="promo_cancel")),
        parse_mode="HTML"
    )
    bot.register_next_step_handler(msg,promo_user_claim_process)
...
def _parse_duration(value, default_unit="days"):
    """Parse 1 hour / 2 hours / 3 days / 2 weeks / 30 minutes / 0."""
    text=str(value or "0").strip().lower()
    if not text or text in {"0","none","no","off","unlimited"}: return 0
    m=re.fullmatch(r"(\d+(?:\.\d+)?)\s*(minute|minutes|min|m|hour|hours|hr|h|day|days|d|week|weeks|w)",text)
    if not m:
        # A plain number uses the supplied default unit.
        try:
            n=float(text); unit=default_unit
        except Exception: raise ValueError("Duration must look like: 1 hour, 3 days, 2 weeks, or 0")
    else:
        n=float(m.group(1)); unit=m.group(2)
    if n<0: raise ValueError("Duration cannot be negative")
    if unit in {"minute","minutes","min","m"}: return int(n*60)
    if unit in {"hour","hours","hr","h"}: return int(n*3600)
    if unit in {"day","days","d"}: return int(n*86400)
    return int(n*604800)

def promo_user_enabled():
    # The user button is controlled by the system switch, not by whether the
    # latest code was already broadcast. This fixes the old "No promo code"
    # state that hid the button even while the admin had opened the system.
    return bool(get_setting("promo_system_enabled", False))
...
def _promo_public_codes():
    """Only codes explicitly announced with SEND ALL are public to users."""
    return [x for x in _active_promo_codes() if bool(x.get("broadcasted",False))]

def _promo_get(code):
    return _promo_collection().find_one({"_id":str(code).upper().strip()})

def _promo_premium_seconds(x):
    value=int(x.get("premium_seconds",0) or 0)
    if value<=0:
        # Backward compatibility with promo documents created by the previous version.
        value=max(0,int(x.get("premium_days",0) or 0))*86400
    return value

def _promo_format(x):
    code=str(x.get("code") or x.get("_id") or "")
    bal=float(x.get("balance_reward",0) or 0)
    ps=_promo_premium_seconds(x)
    ...
    parts=[]
    if bal>0: parts.append(f"💰 ${bal:.2f}")
    if ps>0: parts.append(f"💎 {_duration_text(ps)} Premium")
    reward=" + ".join(parts) if parts else "No reward"
    return (f"🎟 <b>{html.escape(code)}</b>\n"
            f"🎁 Reward: <b>{html.escape(reward)}</b>\n"
            f"📢 Sent All: <b>{'YES' if x.get('broadcasted') else 'NO'}</b>\n"
            f"🟢 Status: <b>{'OPEN' if x.get('enabled') else 'CLOSED'}</b>\n"
...
def _duration_text(seconds):
    ...
    if sec<=0: return "0"
    if sec%604800==0: return f"{sec//604800} week(s)"
    if sec%86400==0: return f"{sec//86400} day(s)"
    if sec%3600==0: return f"{sec//3600} hour(s)"
...
def _promo_claim(uid, code):
    ...
    if not x or not x.get("enabled",False):
        return False,"❌ This promo code is invalid or closed."
    ...
    per=max(1,int(x.get("per_user_limit",1) or 1)); used_by=x.get("used_by") or {}
    ...
    bal=float(x.get("balance_reward",0) or 0); ps=_promo_premium_seconds(x)
    # A user who already knows a secret code may claim it while the system is
    # OPEN. SEND ALL is only the announcement/broadcast action; it is not a
    # requirement for a privately supplied code.
    filt={"_id":code,"enabled":True}
    if maxu>0: filt["used_count"]={"$lt":maxu}
    filt[f"used_by.{uid}"]={"$lt":per}
    ...
    if ps>0: _grant_premium_seconds(uid,ps,"promo_code")
    log_activity(uid,"promo_claimed",{"code":code,"balance_reward":bal,"premium_seconds":ps})
    reward=[]
    if bal>0: reward.append(f"💰 +${bal:.2f}")
    if ps>0: reward.append(f"💎 +{_duration_text(ps)} Premium")
    if not reward: reward.append("🎁 Reward activated")
...
def promo_open_callback(call):
    ...
    msg=bot.send_message(call.message.chat.id,
        "🎟 <b>ENTER PROMO CODE</b>\n\nEnter promo code:\n\nTap Cancel to stop.",
        reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("❌ Cancel",callback_data="promo_cancel")),parse_mode="HTML")
    bot.register_next_step_handler(msg,promo_user_claim_process)
...
def promo_user_claim_process(m):
    uid=str(m.from_user.id); code=(m.text or "").strip().upper()
    if not code:
        bot.send_message(m.chat.id,"❌ Please enter a promo code."); return
    ok,msg=_promo_claim(uid,code)
    bot.send_message(m.chat.id,msg,parse_mode="HTML",reply_markup=localized_user_menu(uid))


def _promo_admin_menu():
    kb=InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("➕ CREATE",callback_data="promo_admin:create"),InlineKeyboardButton("📋 SEE ALL PROMO CODES",callback_data="promo_admin:list"))
...
def promo_admin_start(m):
    ...
    bot.send_message(m.chat.id,"🎟 <b>PROMO CODE CONTROL</b>\n\nCreate unlimited promo codes, choose money and/or Premium rewards, set hours/days/weeks, usage limits, expiry, broadcast and close/open controls.",reply_markup=_promo_admin_menu(),parse_mode="HTML")
...
def promo_admin_callback(call):
    ...
    if action=="open":
        # OPEN only enables the private promo-entry system. It must not publish
        # or mark any code as broadcasted. SEND ALL is the only action that makes
        # a code public/claimable.
        set_setting("promo_system_enabled",True)
        _refresh_all_user_menus("🔄 Promo system opened.")
        bot.answer_callback_query(call.id,"Promo system OPEN")
        try:
            bot.send_message(call.message.chat.id,"🟢 <b>PROMO SYSTEM OPEN</b>\n\nPromo codes remain <b>secret</b> until you use <b>SEND ALL</b>. Users will never see stored codes from the Promo button.",parse_mode="HTML",reply_markup=_promo_admin_menu())
        except Exception: pass
    elif action=="close":
        set_setting("promo_system_enabled",False); _refresh_all_user_menus("🔄 Promo system closed."); bot.answer_callback_query(call.id,"Promo system CLOSED")
    elif action=="list":
        rows=list(_promo_collection().find({}).sort("created_at",-1).limit(100))
        if not rows: text="🎟 <b>SEE ALL PROMO CODES</b>\n\nNo promo codes yet."
        else:
            text="🎟 <b>SEE ALL PROMO CODES</b>\n\n"+"\n\n".join(_promo_format(x) for x in rows)
        bot.send_message(call.message.chat.id,text,parse_mode="HTML")
    elif action=="stats":
        rows=list(_promo_collection().find({})); total=sum(int(x.get("used_count",0) or 0) for x in rows)
        open_n=sum(1 for x in rows if x.get("enabled")); sent_n=sum(1 for x in rows if x.get("broadcasted"))
        bot.send_message(call.message.chat.id,f"📊 <b>PROMO STATS</b>\n\nAll codes: <b>{len(rows)}</b>\nOpen codes: <b>{open_n}</b>\nSent campaigns: <b>{sent_n}</b>\nTotal claims: <b>{total}</b>",parse_mode="HTML")
    elif action=="create":
        msg=bot.send_message(call.message.chat.id,
            "➕ <b>CREATE PROMO CODE</b>\n\n"
            "Send exactly:\n<code>CODE | reward_type | money_usd | premium_duration | max_uses | per_user | expiry | new_users_only</code>\n\n"
            "reward_type: <b>money</b>, <b>premium</b>, <b>both</b>, or <b>none</b>.\n"
            "Premium duration examples: <code>1 hour</code>, <code>3 days</code>, <code>2 weeks</code>.\n"
            "Expiry examples: <code>24 hours</code>, <code>7 days</code>, <code>2 weeks</code>, or <code>0</code> for no expiry.\n\n"
            "Money-only: <code>WELCOME | money | 0.50 | 0 | 100 | 1 | 7 days | no</code>\n"
            "Premium-only: <code>VIP1 | premium | 0 | 1 hour | 100 | 1 | 7 days | no</code>\n"
            "Both: <code>BOTH1 | both | 1.00 | 3 days | 50 | 1 | 30 days | no</code>",parse_mode="HTML")
        bot.register_next_step_handler(msg,promo_create_process)
    elif action=="send":
        msg=bot.send_message(call.message.chat.id,"📢 <b>SEND ALL PROMO</b>\n\nSend the exact promo code. Users will see the code plus <b>OPEN</b> only after this broadcast.",parse_mode="HTML")
        bot.register_next_step_handler(msg,promo_send_process)
    elif action=="edit":
        msg=bot.send_message(call.message.chat.id,"✏️ Send:\n<code>CODE | reward_type | money_usd | premium_duration | max_uses | per_user | expiry | new_users_only | open/close</code>",parse_mode="HTML")
...
def promo_create_process(m):
    ...
    try:
        ...
        kind=parts[1].lower(); bal=float(parts[2] or 0); ps=_parse_duration(parts[3]); maxu=int(parts[4] or 0); per=max(1,int(parts[5] or 1)); expiry_seconds=_parse_duration(parts[6]); newonly=parts[7].lower() in {"yes","y","true","1","on"}
        if kind not in {"money","premium","both","none"}: raise ValueError("reward_type must be money, premium, both or none")
        if kind=="money": ps=0
        elif kind=="premium": bal=0
        elif kind=="none": bal=0; ps=0
        if bal<0 or maxu<0: raise ValueError("Invalid money or usage limit")
        now=datetime.now(timezone.utc); exp=now+timedelta(seconds=expiry_seconds) if expiry_seconds>0 else None
        doc={"_id":code,"code":code,"reward_type":kind,"balance_reward":bal,"premium_seconds":ps,"max_uses":maxu,"per_user_limit":per,"new_users_only":newonly,"enabled":True,"broadcasted":False,"used_count":0,"used_by":{},"created_at":now,"expires_at":exp,"created_by":str(m.from_user.id)}
        _promo_collection().replace_one({"_id":code},doc,upsert=True)
        bot.send_message(m.chat.id,"✅ <b>Promo created.</b>\n\nIt is NOT visible to users yet. Use <b>SEND ALL</b> when you want to announce it.",reply_markup=_promo_admin_menu(),parse_mode="HTML")
...
def promo_send_process(m):
    ...
    if not x.get("enabled",False): bot.send_message(m.chat.id,"❌ This promo is closed."); return
    _promo_collection().update_one({"_id":code},{"$set":{"broadcasted":True,"broadcasted_at":datetime.now(timezone.utc)}})
    set_setting("promo_system_enabled",True)
    ...
    text=("🎁 <b>New Promo Code For Free</b>\n\n"
          f"🎟 Promo Code: <code>{html.escape(code)}</code>\n"
          + ("🎁 Reward: <b>"+html.escape(" + ".join(reward))+"</b>\n\n" if reward else "\n")
          +"Tap <b>OPEN</b>, then enter the code to claim your reward.")
    sent,failed=broadcast_users(lambda uid: bot.send_message(int(uid),text,reply_markup=kb,parse_mode="HTML"), list(users.keys()))
    # The promo broadcast itself already contains the OPEN button, so do not
    # send a second menu-refresh message to every user.
    bot.send_message(m.chat.id,f"✅ Promo <code>{html.escape(code)}</code> broadcast sent to <b>{sent}</b> users.\n❌ Failed: <b>{failed}</b>",parse_mode="HTML",reply_markup=_promo_admin_menu())

def promo_edit_process(m):
    if not is_admin(m.from_user.id): return
    try:
        parts=[x.strip() for x in (m.text or "").split("|")]
        if len(parts)<9: raise ValueError("9 fields required")
        code=parts[0].upper(); old=_promo_get(code)
        if not old: raise ValueError("Promo not found")
        kind=parts[1].lower(); bal=float(parts[2] or 0); ps=_parse_duration(parts[3]); maxu=int(parts[4] or 0); per=max(1,int(parts[5] or 1)); expiry_seconds=_parse_duration(parts[6]); newonly=parts[7].lower() in {"yes","y","true","1","on"}; enabled=parts[8].lower() in {"on","yes","true","1","open"}
        if kind not in {"money","premium","both","none"}: raise ValueError("reward_type must be money, premium, both or none")
        if kind=="money": ps=0
        elif kind=="premium": bal=0
        elif kind=="none": bal=0; ps=0
        exp=datetime.now(timezone.utc)+timedelta(seconds=expiry_seconds) if expiry_seconds>0 else None
        _promo_collection().update_one({"_id":code},{"$set":{"reward_type":kind,"balance_reward":bal,"premium_seconds":ps,"max_uses":maxu,"per_user_limit":per,"new_users_only":newonly,"enabled":enabled,"expires_at":exp}})
        if not enabled: _refresh_all_user_menus("🔄 Promo code closed.")
...
def _refresh_all_user_menus(prefix="🔄 Menu updated."):
    # Menu refreshes are broadcasts too; fan them out concurrently instead of
    # blocking on one user at a time.
    return broadcast_users(lambda uid: bot.send_message(int(uid),prefix,reply_markup=localized_user_menu(uid)), list(users.keys()))
...
def _download_limit_seconds(uid):
    ...
    if is_premium(uid) or _is_trial_active(uid):
        # Premium and Trial use the Premium duration ceiling; default is 24h
        # (effectively unlimited for a Telegram media download).
...
def _rapid_download(link,tmp_dir,quality,max_seconds,chat_id=None):
    ...
    # Video and audio are independent URLs. Download them concurrently for
    # Premium/Quick users instead of waiting for one stream to finish first.
    if audio:
        audio_path=os.path.join(tmp_dir,f"youtube_{vid}_audio.m4a")
        with ThreadPoolExecutor(max_workers=2) as pool:
            fv=pool.submit(_rapid_download_file,video["url"],video_path,max_bytes)
            fa=pool.submit(_rapid_download_file,audio["url"],audio_path,max_bytes)
            fv.result(); fa.result()
    else:
    ...
    ffmpeg=_ffmpeg_bin()
    if not ffmpeg:
        raise RuntimeError("FFmpeg is required to merge RapidAPI video + audio streams. Install FFmpeg on Railway.")
    merged=os.path.join(tmp_dir,f"youtube_{vid}.mp4")
    cmd=[ffmpeg,"-y","-hide_banner","-loglevel","error","-i",video_path,"-i",audio_path,"-c:v","copy","-c:a","aac","-shortest","-threads","0",merged]
    ...
    if proc.returncode!=0 or not os.path.isfile(merged):
        ...
        raise RuntimeError(f"RapidAPI video/audio merge failed: {err}")
...
def _ensure_download_limit_defaults_v2():
    ...
    try:
        ...
        if old_pm == 120: set_setting("premium_max_minutes",1440)
    ...
    try:
        ...
        if not media:
            ...
            opts={
                ...
                "concurrent_fragment_downloads":40 if is_quick_access(uid) else (24 if premium or _is_trial_active(uid) else 8),
                "http_chunk_size": 16 * 1024 * 1024 if is_quick_access(uid) else (12 * 1024 * 1024 if premium or _is_trial_active(uid) else 6 * 1024 * 1024),
                "buffersize": 8 * 1024 * 1024 if is_quick_access(uid) else (4 * 1024 * 1024 if premium or _is_trial_active(uid) else 2 * 1024 * 1024),
                "ratelimit": None,
                **cookie_args
            }
            ffmpeg=_ffmpeg_bin()
        ...
        for path in media[:50 if platform=="tiktok" else 20]:
            ...
            try:
                ...
                upload_stop.set()
            if sent < len(media[:50 if platform=="tiktok" else 20]):
...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    if music_platform=="youtube" and premium_required_for_platform("youtube",uid,link) and not music_priority and not users.get(uid,{}).get("youtube_30m",False):
        try: bot.edit_message_text("💎 <b>Premium/Trial is required for full YouTube MP3.</b>",chat_id,status_message_id,parse_mode="HTML")
        except Exception: pass
        return
    quick=is_quick_access(uid)
    ...
    try:
        ...
        # Universal fast-provider MP3 path. This is important for TikTok,
        # Instagram, Facebook, Snapchat, Pinterest, X, Likee and Threads: the
        # video may have downloaded successfully through SaveAPI/Cobalt, while
        # yt-dlp may fail when the MUSIC button tries to fetch the same URL again.
        ...
        path = mp3_files[0]
        max_music_mb=_download_max_mb(chat_id,platform=music_platform,link=link)
        if max_music_mb > 0 and os.path.getsize(path) > max_music_mb * 1024 * 1024:
            raise RuntimeError(f"The MP3 is too large for the configured {int(max_music_mb)} MB limit.")

        info = info or {}
        if not info:
            try:
                meta_opts={"quiet":True,"no_warnings":True,"skip_download":True,"noplaylist":True,"socket_timeout":15,"http_headers":{"User-Agent":"Mozilla/5.0 Chrome/131 Safari/537.36"}}
...
def admin_referral_broadcast(m):
    ...
    def _send_referral(uid):
        ...
        kb=InlineKeyboardMarkup(); kb.add(InlineKeyboardButton("📤 Share", switch_inline_query=link))
