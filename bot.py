...
def _destination_admin_rights(dtype):
    """Return the smallest native-request rights that Telegram can filter on.

    The previous version requested a long list of explicit False/None rights.
    That can make Telegram clients reject or hide the request-chat picker on
    some versions. For groups the bot only needs to be an administrator; for
    channels it additionally needs permission to post. Telegram will grant the
    requested rights when the selected chat allows it.
    """
    if dtype == "channel":
...
def _destination_request_keyboard(uid, dtype):
    ...
    if dtype == "channel":
        ...
        label="➕ Add Channel"
        prompt="📢 <b>Select a channel</b> from Telegram below. The bot must be allowed to post there."
    else:
        req=KeyboardButtonRequestChat(
            request_id=request_id,
        ...
        label="➕ Add Group"
        prompt="👥 <b>Select a group</b> from Telegram below. You must be a group admin and the bot will request admin access."
