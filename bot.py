...
def _managed_premium_active(bot_id=None, owner_id=None):
    """Per-created-bot Premium state, with legacy owner Premium as fallback."""
    bid=str(bot_id or "").strip()
    if bid:
        d=managed_bots_col.find_one({"bot_id":bid},{"premium_until":1,"owner_id":1}) or {}
        ...
        owner_id=owner_id or d.get("owner_id")
    return bool(owner_id and is_premium(str(owner_id)))
...
def _active_powered_text():
    ...
    meta=_ACTIVE_MANAGED_META.get() or {}
    if _managed_premium_active(meta.get("bot_id"),meta.get("owner_id")):
        return ""
...
def _safe_send_file(chat_id, path, caption="", reply_markup=None, platform=None, link=None):
    ...
    if _ACTIVE_MANAGED_META.get():
        ...
        if not _managed_premium_active(meta.get("bot_id"),meta.get("owner_id")):
            try:
...
def _managed_wallet_start(m, bid):
...
def _managed_premium_callback(call):
    ...
    if data.startswith('mprem:'):
        ...
        rows.append([{"text":wallet_text,"callback_data":f"mpremwallet:{bid}:{months}"}])
        stars_rate=max(1,int(get_setting("stars_per_usd",100) or 100)); stars=max(1,int(round(price*stars_rate)))
        rows.append([{"text":f"⭐ Telegram Stars — {stars} Stars","callback_data":f"mpremstars:{bid}:{months}:{stars}"}])
        rows.append([{"text":"❌ Cancel","callback_data":f"mpremcancel:{bid}"}])
        bot.answer_callback_query(call.id)
        ...
        try:
            ...
            def _mwallet(call):
                ...
                if data.startswith("mwalletrevoke:"): _managed_wallet_revoke_callback(call,data.split(":",1)[1])
                    ...
                    _managed_wallet_start(call.message,bid)
