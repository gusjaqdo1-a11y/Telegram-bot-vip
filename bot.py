...
def _jamendo_song_search(query, limit=100):
    """Search Jamendo's licensed/open catalog. Only tracks explicitly marked
    audiodownload_allowed are offered as full downloads."""
    query=_music_clean_text(query)
    if not query or not JAMENDO_CLIENT_ID:
        return []
    try:
        params={"client_id":JAMENDO_CLIENT_ID,"format":"json","search":query,
                "limit":min(int(limit),200),"imagesize":600,"audiodlformat":"mp32",
        ...
        for item in body.get("results") or []:
            ...
            title=_music_clean_text(item.get("name")); artist=_music_clean_text(item.get("artist_name"))
            if not title or not artist: continue
            rows.append({
                "id":item.get("id"),"title":title,"artist":artist,
                ...
                "download_allowed":bool(item.get("audiodownload_allowed")),
                "license":item.get("license_ccurl") or "",
                "source":"jamendo"
            })
        return rows[:limit]
    except Exception as e:
        print("Jamendo song search failed:",repr(e)); return []
...
@bot.message_handler(func=lambda m: bool(m.text and not m.text.startswith("/") and get_setting("song_auto_search_enabled",False) and _song_search_is_open() and not is_admin(m.from_user.id)))
def auto_song_search_handler(m):
    ...
    if not rows:
        bot.send_message(m.chat.id,"❌ No downloadable songs found. Try another title or artist.")
        return
    _send_song_results(m.chat.id,token,0)
...
def _download_jamendo_song(chat_id,status_id,song,uid):
    tmp=None
    try:
        title=_music_clean_text(song.get("title")) or "Unknown title"
        artist=_music_clean_text(song.get("artist")) or "Unknown artist"
        url=song.get("download")
        if not url: raise RuntimeError("No downloadable file")
        r=requests.get(url,headers={"User-Agent":"Downloadvedioytibot/1.0"},timeout=60,stream=True)
        r.raise_for_status()
        tmp=os.path.join("downloads","jamendo_"+uuid.uuid4().hex); os.makedirs(tmp,exist_ok=True)
        safe=_music_safe_filename(title,artist)
        path=os.path.join(tmp,safe)
        with open(path,"wb") as f:
            for chunk in r.iter_content(1024*256):
                if chunk: f.write(chunk)
        # Use mutagen/ffmpeg metadata embedding helper when possible.
        cover=None
        if song.get("cover"): cover=_music_download_image(song["cover"],tmp,"cover.jpg")
        _embed_music_metadata(path,title,artist,cover_path=cover,album=song.get("album"))
        with open(path,"rb") as audio:
            caption=(f"🎵 <b>{html.escape(title)}</b>\n"
                     f"👤 <b>Artist:</b> {html.escape(artist)}\n"
                     f"💿 <b>Album:</b> {html.escape(song.get('album') or 'Unknown album')}\n"
                     f"⏱️ <b>Duration:</b> {_song_duration(song.get('duration'))}\n"
                     f"📚 <b>Source:</b> Jamendo\n\n"
                     "Full-length track available for download according to the track's Jamendo setting.")
            bot.send_audio(chat_id,audio,caption=caption,parse_mode="HTML",title=title,performer=artist,duration=int(song.get("duration") or 0))
        _record_song_download(uid,song)
        try: bot.delete_message(chat_id,status_id)
        except Exception: pass
    except Exception as e:
        print("Jamendo download error:",repr(e))
        try: bot.edit_message_text("❌ Full song download failed. Please try another track.",chat_id,status_id)
        except Exception: pass
    finally:
