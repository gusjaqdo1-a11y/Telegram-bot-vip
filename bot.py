...
# Optional WhatsApp transactional-message endpoint. Keep this separate from OTP: 
# WaForge's public API currently documents OTP endpoints; set this only when the
# connected account/provider exposes a message endpoint.
WAFORGE_MESSAGE_URL = os.getenv("WAFORGE_MESSAGE_URL", "").strip()
WAFORGE_MESSAGE_MODE = os.getenv("WAFORGE_MESSAGE_MODE", "to_text").strip().lower()
...
FREE_CONCURRENT_DOWNLOADS = int(os.getenv("FREE_CONCURRENT_DOWNLOADS", str(min(64, MAX_CONCURRENT_DOWNLOADS))))
...
def admin_menu():
    ...
    kb.add("🟢 Open Orginal", "🔴 Close Orginal")
    kb.add("🟢 Open Orginal", "🔴 Close Orginal")
    kb.add("🗄 SEE STORAGE")
...
def send_email_all_process(m):
    ...
    for uid, data in users.items():
        ...
        # WhatsApp-verified users can receive the same announcement as plain text.
        # This does not require replacing the existing email path for users who
        # still rely on email.
        if data.get("whatsapp_verified"):
            try:
                if _send_user_whatsapp_update(uid, re.sub(r"<[^>]+>", "", html_content)):
                    whatsapp_count += 1
                    success = True
            except Exception: pass
        if success:
            count += 1
    try:
        bot.send_message(m.chat.id, f"✅ Announcement delivered to <b>{count}</b> users.\n📱 WhatsApp: <b>{whatsapp_count}</b>")
    except: pass
...
# ================= ORIGINAL SOUND FALLBACK =================
@bot.message_handler(func=lambda m: m.text in ["🟢 Open Orginal", "🟢 Open Original"])
def admin_open_original(m):
    if not is_admin(m.from_user.id): return
    set_setting("original_sound_fallback_enabled", True)
    bot.send_message(
        m.chat.id,
        "🟢 <b>ORIGINAL FALLBACK OPEN</b>\n\n"
        "If real song recognition/catalog lookup cannot identify the artist, "
        "MUSIC may use the original-sound owner/uploader and the source cover. "
        "A real recognized artist always has priority.",
        parse_mode="HTML", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text in ["🔴 Close Orginal", "🔴 Close Original"])
def admin_close_original(m):
    if not is_admin(m.from_user.id): return
    set_setting("original_sound_fallback_enabled", False)
    bot.send_message(
        m.chat.id,
        "🔴 <b>ORIGINAL FALLBACK CLOSED</b>\n\n"
        "Uploader/original-sound owner will not be used as the artist. "
        "If no genuine artist is identified, MUSIC will show <b>Unknown artist</b>.",
        parse_mode="HTML", reply_markup=admin_menu())
...
def _storage_collection_stats(db):
    rows=[]
    try:
        names=db.list_collection_names()
    except Exception as e:
        return [{"name":"<database>","error":str(e)}]
    for name in sorted(names):
        try:
            ...
            rows.append({
                ...
                "dataSize":int(st.get("size",0) or 0),
                "storageSize":int(st.get("storageSize",0) or 0),
                "indexSize":int(st.get("totalIndexSize",0) or 0),
                "totalSize":int(st.get("totalSize",0) or 0),
            })
        except Exception as e:
            rows.append({"name":name,"error":str(e)})
    return rows

def _database_storage_report(db, label):
    try:
        st=db.command("dbStats")
        rows=_storage_collection_stats(db)
        return {
            "label":label,
            "db":getattr(db,"name",label),
            "collections":int(st.get("collections",0) or 0),
            "objects":int(st.get("objects",0) or 0),
            "dataSize":int(st.get("dataSize",0) or 0),
            "storageSize":int(st.get("storageSize",0) or 0),
            "indexSize":int(st.get("indexSize",0) or 0),
            "totalSize":int(st.get("totalSize",0) or 0),
            "collection_rows":rows,
        }
    except Exception as e:
        return {"label":label,"db":getattr(db,"name",label),"error":str(e),"collection_rows":[]}

def _storage_report_text():
    import shutil as _shutil
    lines=["🗄 <b>SYSTEM STORAGE REPORT</b>",""]
    try:
        du=_shutil.disk_usage(os.getcwd())
        lines += [
            "💾 <b>SERVER / LOCAL DISK</b>",
            f"• Total: <b>{_format_bytes(du.total)}</b>",
            f"• Used: <b>{_format_bytes(du.used)}</b>",
            f"• Free: <b>{_format_bytes(du.free)}</b>",
            "",
        ]
    except Exception as e:
        lines += [f"💾 Local disk: <code>{html.escape(str(e))}</code>",""]

    # Show downloads and major runtime directories without changing the downloader.
    for d in ["downloads", "session", "__pycache__"]:
        if os.path.exists(d):
            try:
                total=0
                ...
                lines.append(f"📁 <b>{html.escape(d)}/</b> — {_format_bytes(total)}")
            except Exception: pass
    lines.append("")

    db_reports=[]
    for db,label in [(db1,"MongoDB #1 — Users/System"),(db2,"MongoDB #2 — Stats/Media")]:
        r=_database_storage_report(db,label); db_reports.append(r)
        lines += [f"🍃 <b>{html.escape(label)}</b> <code>{html.escape(str(r.get('db','')))}</code>"]
        if r.get("error"):
            lines.append(f"• Error: <code>{html.escape(str(r['error']))}</code>")
            lines.append("")
            continue
        lines += [
            f"• Collections: <b>{r['collections']}</b>",
            f"• Documents/Objects: <b>{r['objects']:,}</b>",
            f"• Data: <b>{_format_bytes(r['dataSize'])}</b>",
            f"• Storage: <b>{_format_bytes(r['storageSize'])}</b>",
            f"• Indexes: <b>{_format_bytes(r['indexSize'])}</b>",
            f"• Total: <b>{_format_bytes(r['totalSize'])}</b>",
        ]
        for row in r.get("collection_rows",[]):
            if row.get("error"):
                lines.append(f"  • {html.escape(row['name'])}: error")
            else:
                lines.append(
                    f"  • <code>{html.escape(row['name'])}</code> — "
                    f"{row['count']:,} docs | data {_format_bytes(row['dataSize'])} | "
                    f"storage {_format_bytes(row['storageSize'])} | index {_format_bytes(row['indexSize'])}"
                )
        lines.append("")

    # Two separate Atlas Free/M0 clusters have separate 512 MB quotas.
    # Per-cluster overrides are supported; otherwise each defaults to 0.5 GB.
    # The legacy MONGO_STORAGE_LIMIT_GB remains supported as an explicit total override.
    try:
        legacy=float(os.getenv("MONGO_STORAGE_LIMIT_GB", "0") or 0)
    except Exception: legacy=0
    try: limit1=float(os.getenv("MONGO_STORAGE_LIMIT_GB_1", "0.5") or 0.5)
    except Exception: limit1=0.5
    try: limit2=float(os.getenv("MONGO_STORAGE_LIMIT_GB_2", "0.5") or 0.5)
    except Exception: limit2=0.5
    if legacy>0:
        total_limit_gb=legacy
    else:
        total_limit_gb=limit1+limit2
    used=sum(float(r.get("totalSize",0) or 0) for r in db_reports if not r.get("error"))
    total_limit=total_limit_gb*(1024**3)
    lines += [
        "📊 <b>MONGODB STORAGE QUOTA</b>",
        f"• MongoDB #1 limit: <b>{_format_bytes(limit1*(1024**3))}</b>",
        f"• MongoDB #2 limit: <b>{_format_bytes(limit2*(1024**3))}</b>",
        f"• Combined limit: <b>{_format_bytes(total_limit)}</b>",
        f"• Combined used: <b>{_format_bytes(used)}</b>",
        f"• Combined remaining: <b>{_format_bytes(max(0,total_limit-used))}</b>",
        f"• Combined usage: <b>{(used/total_limit*100 if total_limit else 0):.2f}%</b>",
    ]
    return "\n".join(lines)
...
def admin_see_storage(m):
    if not is_admin(m.from_user.id): return
    try:
        bot.send_message(m.chat.id,_storage_report_text(),parse_mode="HTML",reply_markup=admin_menu())
    except Exception as e:
        bot.send_message(m.chat.id,f"❌ Storage report failed: <code>{html.escape(str(e))}</code>",parse_mode="HTML",reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "🟢 Open Orginal")
def admin_open_original(m):
    if not is_admin(m.from_user.id): return
    set_setting("original_sound_enabled", True)
    bot.send_message(m.chat.id, "🟢 <b>ORGINAL SOUND FALLBACK OPEN</b>\n\nIf a real artist cannot be identified, the bot may use the Original Sound owner/uploader and source cover as a fallback.", parse_mode="HTML", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "🔴 Close Orginal")
def admin_close_original(m):
    if not is_admin(m.from_user.id): return
    set_setting("original_sound_enabled", False)
    bot.send_message(m.chat.id, "🔴 <b>ORGINAL SOUND FALLBACK CLOSED</b>\n\nUploader/Original Sound information will not be used as artist metadata. A real recognized/catalog artist is required.", parse_mode="HTML", reply_markup=admin_menu())
...
def _song_search_all(query, limit=12):
    """Fast combined search: Jamendo and YouTube metadata run in parallel."""
    query=_music_clean_text(query)
    if not query: return []
    limit=int(limit)
    results=[]
    try:
        with ThreadPoolExecutor(max_workers=2) as pool:
            fj=pool.submit(_jamendo_song_search,query,min(12,limit))
            fy=pool.submit(_youtube_song_search,query,min(12,limit))
            jam=fj.result(timeout=12) or []
            yt=fy.result(timeout=18) or []
    except Exception as e:
        print("Combined song search error:",repr(e))
        try: jam=_jamendo_song_search(query,min(12,limit)) or []
        except Exception: jam=[]
        try: yt=_youtube_song_search(query,min(12,limit)) or []
        except Exception: yt=[]
    seen=set()
    # Prefer YouTube for recognizable real-world songs, while keeping downloadable Jamendo tracks.
    for x in yt + jam:
        key=(str(x.get("source")),str(x.get("id")),str(x.get("download")))
        if key in seen: continue
        seen.add(key); results.append(x)
...
def song_pick_callback(call):
    ...
    if str(x.get("source")) == "youtube":
        def _download_youtube_song_job(chat_id,status_id,song,uid):
            try:
                # Pass the selected result's metadata directly. This prevents the
                # converter from falling back to Unknown title/artist when YouTube
                # metadata is incomplete.
                convert_link_to_mp3(
                    chat_id, song.get("download"), status_id,
                    source_title=_music_clean_text(song.get("title")),
                    source_artist=_music_clean_text(song.get("artist")),
                )
            except Exception as e:
        ...
        try:
            ...
            print("Song executor submit failed:",repr(e))
            try: bot.edit_message_text("❌ Music download queue failed. Please try again.",call.message.chat.id,status.message_id)
            except Exception: pass
    else:
        try:
...
def _download_music_cover(info, tmp_dir, title=None, artist=None, recognized=None):
    """Prefer genuine music artwork. Source thumbnail is ONLY an Original fallback."""
    info = info or {}
    recognized = recognized or {}
    artwork_candidates = []

    # 1) Exact recognition/catalog artwork always wins.
    for value in recognized.get("artwork_candidates") or []:
        if isinstance(value, str) and value.strip(): artwork_candidates.append(value.strip())
    for key in ("artwork", "artwork_url", "cover", "cover_url"):
        value = recognized.get(key)
        if isinstance(value, str) and value.strip(): artwork_candidates.append(value.strip())

    # 2) Explicit album artwork supplied by the extractor is also genuine artwork.
    for key in ("album_art", "album_artwork", "album_cover", "cover_url", "artwork_url", "artwork"):
        value = info.get(key)
        if isinstance(value, str) and value.strip(): artwork_candidates.append(value.strip())

    # 3) Independent catalogs provide a final real-cover lookup.
    ...
    if lookup.get("artwork"):
        artwork_candidates.append(lookup["artwork"])
    ...
    # 4) ONLY when Orginal fallback is explicitly open and there is no genuine cover,
    # use the source video's thumbnail. This is never allowed to override a real cover.
    if get_setting("original_sound_enabled", False):
        for value in [info.get("thumbnail")]:
            if isinstance(value, str) and value.strip():
                cover = _music_download_image(value.strip(), tmp_dir, "original_sound_cover.jpg")
                if cover: return cover
        for t in info.get("thumbnails") or []:
            if isinstance(t, dict) and isinstance(t.get("url"), str) and t.get("url").strip():
                cover = _music_download_image(t["url"].strip(), tmp_dir, "original_sound_cover.jpg")
                if cover: return cover
    return None
...
def convert_link_to_mp3(chat_id, link, status_message_id, local_source=None, local_cache_dir=None, source_title="", source_artist="", pending_token=None):
    ...
    try:
        ...
        if get_setting("original_sound_enabled", False) and (not recognized_title) and _music_is_original_label(source_title):
        ...
        # Admin-controlled Original Sound fallback. Real recognized/catalog artist
        # always wins. Only when the artist is still unknown do we optionally use
        # the source account/Original Sound owner and source cover.
        original_fallback = False
        if (not artist or _music_is_original_label(artist)) and bool(get_setting("original_sound_fallback_enabled", False)):
            owner=""
            for owner_key in ("uploader","uploader_id","creator","creator_id","channel","channel_id"):
                value=_music_clean_text(info.get(owner_key))
                if value and not _music_is_original_label(value):
                    owner=value; break
            if owner:
                artist=owner.lstrip("@")
        ...
        if not artist or _music_is_original_label(artist):
            if get_setting("original_sound_enabled", False):
                ...
                for key in ("uploader", "uploader_id", "creator", "creator_id", "channel", "channel_id"):
                    value = _music_clean_text(info.get(key))
                    if value and not _music_is_provider_placeholder(value):
                        original_owner = value.lstrip("@")
                        break
                artist = original_owner or "Unknown artist"
            else:
        ...
        if original_fallback:
            ...
            if isinstance(thumb,str) and thumb.strip():
                recognized_for_cover.setdefault("artwork_candidates",[])
                recognized_for_cover["artwork_candidates"].append(thumb.strip())
            for t in info.get("thumbnails") or []:
                if isinstance(t,dict) and isinstance(t.get("url"),str) and t.get("url").strip():
                    recognized_for_cover.setdefault("artwork_candidates",[])
                    recognized_for_cover["artwork_candidates"].append(t["url"].strip())
...
def refer_cmd(m):
    ...
    try:
        ...
        if bool(get_setting("pay_rev_enabled", False)):
...
def broadcast_send(m):
    if not is_admin(m.from_user.id):
        return
    text = m.text
    def _send_text_update(uid):
        # Telegram remains the primary delivery; WhatsApp-verified users also
        # receive the same text when WAFORGE_MESSAGE_URL is configured.
        send_user_bot_update(uid, text, whatsapp=True, telegram=True)
    sent,failed=broadcast_users(_send_text_update, list(users.keys()))
    try:
...
def send_waforge_whatsapp_message(phone_number, text):
    """Best-effort transactional WhatsApp message adapter.

    The public WaForge API documents OTP send/verify. General transactional
    messages require an account/provider message endpoint, supplied explicitly
    through WAFORGE_MESSAGE_URL. This function never pretends an OTP endpoint
    is a generic messaging endpoint.
    """
    if not WAFORGE_API_KEY or not WAFORGE_MESSAGE_URL:
        return False
    phone=re.sub(r"[\s().-]", "", str(phone_number or "").strip())
    if not re.fullmatch(r"\+[1-9]\d{7,14}", phone): return False
    payloads=[]
    if WAFORGE_MESSAGE_MODE == "phone_text":
        payloads=[{"phone":phone,"text":str(text)}]
    elif WAFORGE_MESSAGE_MODE == "to_message":
        payloads=[{"to":phone,"message":str(text)}]
    else:
        payloads=[{"to":phone,"text":str(text)}]
    for payload in payloads:
        try:
            r=requests.post(WAFORGE_MESSAGE_URL,json=payload,headers=_waforge_headers(WAFORGE_API_KEY_HEADER),timeout=30)
            try: body=r.json()
            except Exception: body={"raw":r.text[:500]}
            if _waforge_success(body,r.status_code): return True
        except Exception as e:
            print("WaForge transactional message error:",repr(e))
    return False

def _send_user_whatsapp_update(uid, text):
    u=users.get(str(uid),{})
    if not u.get("whatsapp_verified"):
        return False
    phone=u.get("phone") or u.get("whatsapp_phone")
    return send_waforge_whatsapp_message(phone,text)

def send_user_bot_update(uid, text, whatsapp=True, telegram=True):
    """Send important bot updates through Telegram and, for WhatsApp-verified users,
    through the configured WhatsApp messaging endpoint. Telegram remains the safe fallback.
    """
    uid=str(uid); sent=False
    if telegram:
        try: bot.send_message(int(uid),text,parse_mode="HTML"); sent=True
        except Exception: pass
    if whatsapp:
        try: _send_user_whatsapp_update(uid,re.sub(r"<[^>]+>","",str(text)))
        except Exception: pass
    return sent

def send_premium_email(uid, subject, months, price, until):
    uid=str(uid); u=users.get(uid,{})
    whatsapp_text=(f"💎 <b>{html.escape(subject)}</b>\n\n"
                   f"Duration: <b>{months if months else 'Trial / Reward'}</b>\n"
                   f"Paid: <b>${price:.2f}</b>\n"
                   f"Expires: <b>{html.escape(local_datetime_text(uid, until))}</b>\n\n"
                   "Thank you for using Downloader Bot.")
    # WhatsApp-verified users get the same premium notification on WhatsApp when
    # the provider message endpoint is configured. Email remains for users who
    # are not WhatsApp-verified or when WhatsApp messaging is unavailable.
    wa_sent=_send_user_whatsapp_update(uid,whatsapp_text)
    email=u.get("email")
    if not email: return wa_sent
    ...
    email_sent=send_html_email(email,subject,body)
    return bool(wa_sent or email_sent)
...
def send_premium_expiry_email(uid,until,days_left=None,expired=False):
    ...
    wa_text=(f"{title}\n\n"
             f"Your Premium {('has ended.' if expired else f'will expire in {days_left} day(s).')}\n"
             f"Expiry: {until.strftime('%Y-%m-%d %H:%M UTC')}\n\n"
             "Open 💎 PREMIUM in Telegram to renew.")
    wa_sent=_send_user_whatsapp_update(uid,wa_text)
    email=u.get("email")
    if not email: return wa_sent
    ...
    return bool(wa_sent or send_html_email(email,subject,body))
