...
def send_smess_whatsapp(phone_number, code, user_id):
    ...
    payload = {
        ...
        "text": f"Your Downloader Bot verification code is: Code: {code}",
        "copycode": str(code),
        "copytext": "Copy verification code",
        "priority": "otp",
...
def admin_menu():
    ...
    kb.add("👑 PREMIUM USERS", "✏️ EDIT START MESSAGE")
...
def sms_admin_manager(m):
    ...
    if m.text == "🟢 Open SMS":
        ...
        try:
            bot.send_message(m.chat.id, "🟢 WhatsApp verification is now OPEN. Users can request OTP via SMESS.")
        except: pass
...
def start_verify_flow(call):
    try:
        ...
        whatsapp_enabled = get_setting("whatsapp_verify_enabled", False) and bool(SMESS_API_KEY)
        if sms_enabled:
            kb = InlineKeyboardMarkup()
            kb.row(
...
def handle_verify_choice(call):
    ...
    try:
        ...
        if choice == "gmail":
            ...
            if not get_setting("whatsapp_verify_enabled", False) or not SMESS_API_KEY:
...
def process_verification_whatsapp(m):
    uid = str(m.from_user.id)
    phone_input = (m.text or "").strip().replace(" ", "")
    if phone_input in ["👤 Profile", "💰 BALANCE", "💸 WITHDRAWAL", "👥 REFERRAL", "🆔 GET ID", "☎️ CUSTOMER", "🤖CUSTOMER AI", "💳 PAY"] or not re.fullmatch(r"\+[1-9]\d{9,14}", phone_input):
...
def start_handler(message):
    ...
    if str(uid) not in users:
        ...
        users[str(uid)] = {
            "username": message.from_user.username or "", "balance": 0.0, "blocked": 0.0,
            "ref": random_ref(), "bot_id": random_botid(), "invited": 0, "banned": False,
            "verified": False, "quick_access": False, "youtube_30m": False,
            "premium_until": None, "premium_warning_sent": False,
            "joined_date": datetime.now().strftime("%Y-%m-%d"), "month": now_month()
        }
        if ref:
...
def show_premium_menu(chat_id):
    ...
    if is_premium(uid):
        ...
        if not users.get(uid, {}).get("verified", False):
            ...
            return
        bot.send_message(chat_id, premium_features_text() + "\n\n💰 <b>Choose a plan:</b>", reply_markup=premium_plan_keyboard())
...
def send_premium_email(uid, subject, months, price, until):
    ...
    if not email: return
    body = f"""<h2>💎 Premium Activated</h2><p>Your Premium subscription is active.</p><p><b>Duration:</b> {months} month(s)<br><b>Paid:</b> ${price:.2f}<br><b>Expires:</b> {until.strftime('%Y-%m-%d %H:%M UTC')}</p><p>Thank you for using our service.</p>"""
    ...
    while True:
        try:
            ...
            for uid, data in list(users.items()):
                ...
                if dt <= now:
                    if data.get("premium_until") is not None:
                        data["premium_until"] = None; data["premium_warning_sent"] = False; save_user(uid)
                        try: bot.send_message(int(uid), "⏰ Your Premium has expired. Open 💎 PREMIUM to renew.")
                        except Exception: pass
                elif dt - now <= timedelta(days=PREMIUM_WARNING_SENT_DAYS) and not data.get("premium_warning_sent", False):
                    data["premium_warning_sent"] = True; save_user(uid)
                    days_left = max(1, int((dt - now).total_seconds() // 86400))
                    msg = f"⚠️ Your Premium expires in {days_left} day(s).\n\n💎 Renew now to keep 4K quality, faster downloads and extra platforms."
                    try: bot.send_message(int(uid), msg)
                    except Exception: pass
                    email = data.get("email")
                    if email: send_html_email(email, "Your Premium expires soon", f"<h2>⚠️ Premium Expiry Reminder</h2><p>Your Premium expires on <b>{dt.strftime('%Y-%m-%d %H:%M UTC')}</b>.</p><p>Renew now to keep your Premium features.</p>")
        except Exception as e:
            print("Premium worker error:", e)
