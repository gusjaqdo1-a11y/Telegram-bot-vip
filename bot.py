...
def promo_user_menu(m):
    ...
    msg=bot.send_message(
        m.chat.id,
        "🎟 <b>PROMO CODE</b>\n\n"
        "🔐 Promo codes are private.\n"
        "Send the promo code you received from the admin.",
...
def _promo_claim(uid, code):
    ...
    x=_promo_get(code)
    if not x or not x.get("enabled",False) or not x.get("broadcasted",False):
        return False,"❌ This promo code is closed, invalid, or has not been announced."
    ...
    filt={"_id":code,"enabled":True,"broadcasted":True}
...
def promo_open_callback(call):
    ...
    msg=bot.send_message(call.message.chat.id,
        "🎟 <b>ENTER PROMO CODE</b>\n\nSend the promo code below to claim your reward.\n\nTap Cancel to stop.",
...
def promo_send_process(m):
    ...
    _refresh_all_user_menus("🎟 A new promo code is available.")
    bot.send_message(m.chat.id,f"✅ Promo <code>{html.escape(code)}</code> broadcast sent to <b>{sent}</b> users.",parse_mode="HTML",reply_markup=_promo_admin_menu())
...
def _song_auto_search_should_handle(m):
    ...
    if not text or text.startswith("/") or not _song_search_is_open() or not bool(get_setting("song_auto_search_enabled",True)) or is_admin(m.from_user.id): return False
    ...
    try:
        ...
        if source_platform == "pinterest":
            try:
                ...
                if pin_video and shutil.which("ffmpeg"):
                    direct_source=pin_video
                    rapid_mp3=os.path.join(tmp_dir,"pinterest_direct.mp3")
                    proc=subprocess.run(["ffmpeg","-y","-i",pin_video,"-vn","-codec:a","libmp3lame","-b:a","192k",rapid_mp3],stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=120)
                    if proc.returncode!=0 or not os.path.isfile(rapid_mp3):
                        rapid_mp3=None
            except Exception as e:
