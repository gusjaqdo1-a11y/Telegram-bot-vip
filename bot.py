...
def _safe_send_file(chat_id, path, caption="", reply_markup=None, platform=None, link=None):
    ...
    if _ACTIVE_MANAGED_META.get():
        ...
        if powered:
            try:
                bot.send_message(chat_id, powered, parse_mode="HTML")
            except Exception as e:
                print("Managed bot powered-by message failed:", repr(e))
        if _create_caption_open():
            try:
                kb=InlineKeyboardMarkup()
                url=_creator_bot_url()
                if url:
                    kb.add(InlineKeyboardButton("✨ Create Your Own Bot", url=url))
                bot.send_message(chat_id, _create_caption_text(), parse_mode="HTML", reply_markup=kb if url else None)
            except Exception as e:
                print("Managed bot creation caption failed:", repr(e))
    return sent
    ...
    try:
        ...
        for path in media[:50 if platform=="tiktok" else 20]:
            ...
            try:
                ...
                powered=_active_powered_text()
                if _ACTIVE_MANAGED_META.get() and powered:
                    try: bot.send_message(chat_id,powered,parse_mode="HTML")
                    except Exception: pass
            finally:
                upload_stop.set()
...
def _creator_admin_keyboard():
    return {"keyboard":[
        ...
        [{"text":"🟢 Open Create Button"},{"text":"🔴 Close Create Button"}],
...
def _creator_admin_text(uid, chat_id, text):
    ...
    if text=="🟢 Open Create Button":
        set_setting("managed_create_button_enabled",True); _creator_set_commands(); _refresh_all_user_menus("🟢 Create button is now OPEN."); _creator_send(chat_id,"🟢 <b>Create Button OPEN</b>",reply_markup=_creator_admin_keyboard()); return
    if text=="🔴 Close Create Button":
        set_setting("managed_create_button_enabled",False); _creator_set_commands(); _refresh_all_user_menus("🔴 Create button is now CLOSED."); _creator_send(chat_id,"🔴 <b>Create Button CLOSED</b>\nExisting downloader bots continue working.",reply_markup=_creator_admin_keyboard()); return
    ...
    with managed_bot_lock:
        ...
        try:
            ...
            def _start(m):
                ...
                owner=(str(d.get("owner_id") or "")==str(m.from_user.id))
...
def admin_open_create_button(m):
    ...
    _refresh_all_user_menus("🟢 Create button is now OPEN.")
...
def admin_close_create_button(m):
    ...
    _refresh_all_user_menus("🔴 Create button is now CLOSED.")
    ...
    if payload.startswith("managedprem:"):
        try:
            ...
            if len(parts)>=5:
                ...
                premium_logs_col.insert_one({"user_id":uid,"bot_id":bid,"months":int(months),"price":price,"stars":stars,"usd_equivalent":usd,"until":until.isoformat(),"time":now.isoformat(),"type":"managed_bot_purchase_stars"})
