...
def _youtube_song_search(query, limit=8):
    """Search YouTube with typo-tolerant variants and rank exact title/artist matches."""
    query=_music_clean_text(query)
    if not query: return []
    try:
            ...
            for fut in futures:
                try:
                    for item in fut.result(timeout=15) or []:
                        ...
                        if not url or url in seen: continue
                        seen.add(url)
                        item["_score"]=_song_similarity(query,item.get("title",""),item.get("artist",""))
                        rows.append(item)
                except Exception as e:
                    print("YouTube song variant failed:",repr(e))
        rows.sort(key=lambda x:x.get("_score",0),reverse=True)
        for x in rows: x.pop("_score",None)
        return rows[:int(limit)]
    except Exception as e:
        print("YouTube song search failed:",repr(e))
        return []
