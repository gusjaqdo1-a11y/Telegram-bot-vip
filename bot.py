...
def _deezer_song_search(query, limit=100):
    """Search songs and, when the query matches an artist, include that artist's songs."""
    ...
    def add(rows):
        for item in rows or []:
            ...
            artist=_music_clean_text((item.get("artist") or {}).get("name"))
            ...
            found.append({
                ...
                "album": _music_clean_text(album.get("title")),
                "cover": album.get("cover_xl") or album.get("cover_big") or album.get("cover_medium"),
                "preview": item.get("preview") or "",
            })
    try:
        r=requests.get("https://api.deezer.com/search",params={"q":query,"limit":50},headers={"User-Agent":"Mozilla/5.0"},timeout=12)
        r.raise_for_status(); add((r.json() or {}).get("data"))
    except Exception as e:
        print("Song search failed:",repr(e))
    # If query resembles an artist, include that artist's top tracks too.
    try:
        r=requests.get("https://api.deezer.com/search/artist",params={"q":query,"limit":5},headers={"User-Agent":"Mozilla/5.0"},timeout=12)
        r.raise_for_status(); artists=(r.json() or {}).get("data") or []
        for a in artists[:3]:
            aid=a.get("id") if isinstance(a,dict) else None
            if not aid: continue
            rr=requests.get(f"https://api.deezer.com/artist/{aid}/top",params={"limit":100},headers={"User-Agent":"Mozilla/5.0"},timeout=12)
            if rr.ok: add((rr.json() or {}).get("data"))
    except Exception as e:
        print("Artist song search failed:",repr(e))
    # Exact title/artist matches first, then the rest.
    q=query.casefold()
    found.sort(key=lambda x:(0 if x["title"].casefold()==q or x["artist"].casefold()==q else 1, x["title"].casefold()))
    return found[:limit]
...
def search_song_query_step(m):
    ...
    rows=_deezer_song_search(query,100)
...
def song_pick_callback(call):
    ...
    x=rows[idx]
    bot.answer_callback_query(call.id,"🎵 Song selected")
    caption=(f"🎵 <b>{html.escape(x['title'])}</b>\n"
             f"👤 <b>Artist:</b> {html.escape(x['artist'])}\n"
             f"💿 <b>Album:</b> {html.escape(x['album'] or 'Unknown album')}\n"
             f"⏱️ <b>Duration:</b> {_song_duration(x['duration'])}\n\n"
             "▶️ Preview")
    try:
        if x.get("cover"):
            rr=requests.get(x["cover"],headers={"User-Agent":"Mozilla/5.0"},timeout=10)
            if rr.ok and len(rr.content)>1000:
                import io
                bot.send_photo(call.message.chat.id,io.BytesIO(rr.content),caption=caption,parse_mode="HTML")
        if x.get("preview"):
            bot.send_audio(call.message.chat.id,x["preview"],caption=f"🎵 {x['title']} — {x['artist']}\n⏱️ {_song_duration(x['duration'])}",title=x["title"],performer=x["artist"])
        else:
            bot.send_message(call.message.chat.id,"⚠️ No preview is available for this song.")
    except Exception as e:
        print("Song selection send error:",repr(e))
        bot.send_message(call.message.chat.id,"❌ Could not send this song preview. Please try another result.")
