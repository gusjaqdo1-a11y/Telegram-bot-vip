...
# Optional WaForge transactional/broadcast message endpoint. The public WaForge
# OTP docs currently document /otp/send and /otp/verify; keep general WhatsApp
# notifications disabled unless the provider/account supplies a message endpoint.
...
# When OPEN, an unidentified "Original Sound" may use the source/uploader name
# and source thumbnail as the fallback. When CLOSED, only a real music artist
# discovered by recognition/catalog lookup is allowed.
...
def send_waforge_message(phone_number, text, subject="Downloader Bot"):
    """Send a normal WhatsApp notification when a message endpoint is configured."""
...
def whatsapp_notification_enabled(uid):
    u = users.get(str(uid), {})
    return bool(u.get("whatsapp_verified") and u.get("phone") and WAFORGE_API_KEY and WAFORGE_MESSAGE_URL)


def notify_user_email_or_whatsapp(uid, subject, plain_text, html_body=None):
    """Prefer WhatsApp for WhatsApp-verified users; otherwise use verified Gmail."""
    uid = str(uid)
    u = users.get(uid, {})
    if u.get("whatsapp_verified") and u.get("phone"):
        ...
        # Do not silently switch a WhatsApp-verified user back to email.
        return "none"
    if u.get("email_verified") and u.get("email"):
        ok = send_html_email(
            u.get("email"), subject,
            html_body or html.escape(plain_text).replace("\n", "<br>")
        )
    ...
    for uid, data in users.items():
        try:
            if data.get("whatsapp_verified") and data.get("phone"):
                ok, detail = send_waforge_message(
                    data.get("phone"),
                    re.sub(r"<[^>]+>", "", str(html_content)),
                    "Announcement from Video Downloader Bot"
                )
                if ok:
                    whatsapp_count += 1
                else:
            ...
            if email and data.get("email_verified", True):
            ...
            failed += 1
            print("Notification broadcast error:", uid, e)

    try:
        bot.send_message(
            m.chat.id,
            f"✅ <b>Notification broadcast completed.</b>\n\n"
            f"📧 Gmail: <b>{email_count}</b>\n"
            f"📱 WhatsApp: <b>{whatsapp_count}</b>\n"
            f"❌ Failed: <b>{failed}</b>",
            parse_mode="HTML"
        )
    except: pass
    ...
    try:
        ...
        for path in media[:50 if platform=="tiktok" else 20]:
            ...
            if _is_video_file(path):
                ...
                music_pending[token]={"uid":uid,"link":link,"local_source":path,"cache_dir":tmp,"created":time.time(),
                                       "source_title":source_title,"source_artist":source_artist,
                                       "source_uploader":_music_clean_text(info.get("uploader") or info.get("creator") or info.get("channel")) if isinstance(info,dict) else "",
                                       "source_thumbnail":_music_clean_text(info.get("thumbnail")) if isinstance(info,dict) else "",
                                       "source_is_original":bool(_music_is_original_label(info.get("title") or info.get("fulltitle") or source_title)) if isinstance(info,dict) else _music_is_original_label(source_title),
                                       "in_progress":False}
...
def admin_open_original(m):
    ...
    bot.send_message(
        ...
        "If real song recognition/catalog search cannot find an artist, the bot may use "
        "the Original Sound owner/uploader name and the source cover as the fallback.",
        parse_mode="HTML", reply_markup=admin_menu()
    )
...
def admin_close_original(m):
    ...
    bot.send_message(
        ...
        "The bot will NOT use the Original Sound owner/uploader as an artist. "
        "It will use a real recognized/catalog artist only; otherwise it stays Unknown artist.",
        parse_mode="HTML", reply_markup=admin_menu()
    )
...
def _download_music_cover(info, tmp_dir, title=None, artist=None, recognized=None, source_thumbnail=None, source_is_original=False):
    """Prefer real song artwork; use source thumbnail only as an explicit fallback."""
    info = info or {}
    ...
    for value in recognized.get("artwork_candidates") or []:
        if isinstance(value, str) and value.strip():
            genuine_candidates.append(value.strip())
    ...
    # Explicit album artwork fields from extractors are genuine music metadata.
    ...
    lookup = _music_itunes_lookup(
        title or _music_title_from_info(info),
        artist or _music_artist_from_info(info)
    )
    if lookup.get("artwork"):
        genuine_candidates.append(lookup["artwork"])

    for idx, url in enumerate(dict.fromkeys(genuine_candidates)):
        cover = _music_download_image(url, tmp_dir, f"music_cover_{idx}.jpg")
        if cover:
            return cover

    # Only after genuine artwork failed, allow the admin-controlled source/Original
    # fallback. This prevents a TikTok video frame from replacing a real album cover.
    allow_source_thumbnail = bool(
        (get_setting("mp3_original_fallback_enabled", MP3_ORIGINAL_FALLBACK_DEFAULT) if source_is_original
         else get_setting("mp3_cover_enabled", MP3_COVER_DEFAULT))
    )
    if allow_source_thumbnail:
        fallback_candidates = []
        thumb = _music_clean_text(source_thumbnail) or _music_clean_text(info.get("thumbnail"))
        if thumb:
            fallback_candidates.append(thumb)
        for t in info.get("thumbnails") or []:
            if isinstance(t, dict) and isinstance(t.get("url"), str) and t.get("url").strip():
                fallback_candidates.append(t["url"].strip())
        for idx, url in enumerate(dict.fromkeys(fallback_candidates), start=100):
            cover = _music_download_image(url, tmp_dir, f"source_fallback_{idx}.jpg")
            if cover:
                return cover
...
def convert_link_to_mp3(chat_id, link, status_message_id, local_source=None, local_cache_dir=None,
                         source_title="", source_artist="", pending_token=None,
                         source_uploader="", source_thumbnail="", source_is_original=False):
    uid=str(chat_id)
    ...
    try:
        ...
        source_uploader = _music_clean_text(source_uploader) or _music_clean_text(
            info.get("uploader") or info.get("creator") or info.get("channel")
        )
        source_thumbnail = _music_clean_text(source_thumbnail) or _music_clean_text(info.get("thumbnail"))

        original_owner_hint = ""
        original_label = raw_info_title or raw_source_title
        if source_is_original:
            original_label = original_label or "Original Sound"
        m_original = re.match(
            r"^original\s+(?:sound|audio|music)\s*(?:[-:|]\s*)?(.*)$",
            original_label, flags=re.I
        )
        if m_original:
            original_owner_hint = _music_clean_text(m_original.group(1))
        ...
        # Strongest path: fingerprint the actual audio, then use independent
        # catalogs to repair/confirm artist, title and album artwork.
        ...
        for candidate in (title,):
            candidate = _music_clean_text(candidate)
            if candidate and not _music_is_original_label(candidate) and not _music_is_provider_placeholder(candidate):
                if candidate.casefold() not in {x.casefold() for x in search_terms}:
                    search_terms.append(candidate)

        for search_title in search_terms:
            for lookup_fn in (_music_musicbrainz_lookup, _music_deezer_lookup, _music_itunes_lookup):
                try:
                    catalog_meta = lookup_fn(search_title, recognized_artist or source_artist or "")
                except Exception:
                    catalog_meta = {}
        ...
        original_fallback_open = bool(get_setting("mp3_original_fallback_enabled", MP3_ORIGINAL_FALLBACK_DEFAULT))
        used_original_fallback = False
        if not artist or _music_is_original_label(artist):
            if original_fallback_open and (source_uploader or original_owner_hint):
                artist = source_uploader or original_owner_hint
        ...
        cover_path = _download_music_cover(
            ...
            source_is_original=bool(source_is_original or m_original or _music_is_original_label(raw_info_title) or _music_is_original_label(raw_source_title)),
        )
...
def send_premium_email(uid, subject, months, price, until):
    """Legacy function name: route Premium notifications to WhatsApp or Gmail."""
    uid = str(uid)
    duration = months if months else "Trial / Reward"
    plain = (
        f"💎 Downloader Bot — {subject}\n\n"
        f"Duration: {duration}\n"
        f"Paid: ${price:.2f}\n"
        f"Expires: {local_datetime_text(uid, until)}\n"
        f"Platforms: {len(premium_platform_names())}\n"
        "⚡ Priority downloads\n🎥 Higher quality\n▶️ Full YouTube access"
    )
    body = (
        "<html><body style='margin:0;background:#f3f8f5;font-family:Arial;color:#10231d'>"
        "<div style='max-width:620px;margin:30px auto;background:white;border-radius:22px;overflow:hidden'>"
        "<div style='padding:28px;background:linear-gradient(135deg,#087f5b,#19b77d);color:white'>"
        "<h1 style='margin:0'>💎 Downloader Bot</h1><p>Premium account notification</p></div>"
        f"<div style='padding:30px'><h2>{html.escape(subject)}</h2>"
        f"<div style='background:#f0faf6;padding:18px;border-radius:14px'>"
        f"<b>Duration:</b> {html.escape(str(duration))}<br>"
        f"<b>Paid:</b> ${price:.2f}<br>"
        f"<b>Expires:</b> {html.escape(local_datetime_text(uid, until))}<br>"
        f"<b>Platforms:</b> {len(premium_platform_names())}</div>"
        "<p>Thank you for using our service.</p></div></div></body></html>"
    )
    channel = notify_user_email_or_whatsapp(uid, subject, plain, body)
    return channel != "none"


def send_premium_expiry_email(uid, until, days_left=None, expired=False):
    """Legacy function name: route Premium expiry notifications to WhatsApp or Gmail."""
    uid = str(uid)
    subject = (
        "Your Downloader Bot Premium has expired"
        if expired
        else f"Your Downloader Bot Premium expires in {days_left} day(s)"
    )
    plain = (
        f"⏰ {subject}\n\n"
        f"Expiry: {until.strftime('%Y-%m-%d %H:%M UTC')}\n"
        + ("Open 💎 PREMIUM in Telegram to renew." if expired else "Renew now to keep Premium features.")
    )
    title = "⏰ Premium Expired" if expired else "⚠️ Premium Expiry Reminder"
    body = (
        "<html><body style='margin:0;background:#f4f7f6;font-family:Arial'>"
        "<div style='max-width:620px;margin:30px auto;background:#fff;border-radius:20px;padding:30px'>"
        f"<h1>{title}</h1>"
        f"<p>Your Premium {('has ended.' if expired else f'will expire in {days_left} day(s).')}</p>"
        f"<p><b>Expiry:</b> {until.strftime('%Y-%m-%d %H:%M UTC')}</p>"
        "<p>Open <b>💎 PREMIUM</b> in Telegram to renew.</p>"
        "</div></body></html>"
    )
    channel = notify_user_email_or_whatsapp(uid, subject, plain, body)
    return channel != "none"
...
def _safe_collection_stats(db, collection_name):
    try:
        return db.command("collStats", collection_name)
    except Exception as e:
        return {"ns": f"{db.name}.{collection_name}", "_error": str(e)}


def _database_storage_report(db):
    rows = []
    try:
        names = sorted(db.list_collection_names())
    except Exception as e:
        return {"db": db.name, "error": str(e), "collections": []}

    for name in names:
        stats = _safe_collection_stats(db, name)
        rows.append({
            "name": name,
            "documents": int(stats.get("count", 0) or 0),
            ...
            "total_size": int(
                (stats.get("storageSize", 0) or 0) +
                (stats.get("totalIndexSize", 0) or 0)
            ),
            "error": stats.get("_error"),
        })

    try:
        dbstats = db.command("dbStats")
    except Exception as e:
        dbstats = {"_error": str(e)}
...
def _path_size(path):
    ...
    for root, dirs, files in os.walk(path):
        # Avoid huge virtual/system trees if a symlink is present.
...
def _admin_storage_report_text():
    lines = ["🗄 <b>SYSTEM STORAGE & DATABASE REPORT</b>", ""]

    # Local filesystem / Railway container storage.
    try:
        ...
        lines += [
            "💾 <b>LOCAL DISK</b>",
            f"📍 Path: <code>{html.escape(root_path)}</code>",
            f"📦 Total: <b>{_human_bytes(disk.total)}</b>",
            f"🟠 Used: <b>{_human_bytes(used)}</b>",
            f"🟢 Free: <b>{_human_bytes(disk.free)}</b>",
            f"📊 Used: <b>{(used / disk.total * 100):.2f}%</b>" if disk.total else "📊 Used: N/A",
            "",
        ]
        for label, path in [
            ("downloads/", os.path.join(root_path, "downloads")),
            ("session", os.path.join(root_path, "session")),
            ("current working directory", root_path),
        ]:
            if os.path.exists(path):
                size = _path_size(path)
                lines.append(f"   • {html.escape(label)}: <b>{_human_bytes(size)}</b>")
        lines.append("")
    except Exception as e:
        lines += [f"💾 <b>LOCAL DISK:</b> error — {html.escape(str(e)[:250])}", ""]

    # MongoDB databases and every collection.
    for db_label, db in (("MongoDB #1 — Users/Settings", db1), ("MongoDB #2 — Stats/Videos", db2)):
        report = _database_storage_report(db)
        lines += [f"🍃 <b>{html.escape(db_label)}</b>", f"🗃 Database: <code>{html.escape(db.name)}</code>"]
        dbstats = report.get("dbstats") or {}
        if report.get("error") or dbstats.get("_error"):
            lines.append(f"❌ dbStats error: {html.escape(str(report.get('error') or dbstats.get('_error'))[:300])}")
        else:
            lines += [
                f"📄 Logical data: <b>{_human_bytes(dbstats.get('dataSize', 0))}</b>",
                f"💽 Storage allocated: <b>{_human_bytes(dbstats.get('storageSize', 0))}</b>",
                f"📚 Index size: <b>{_human_bytes(dbstats.get('indexSize', 0))}</b>",
                f"📦 Collections: <b>{len(report.get('collections', []))}</b>",
            ]

        rows = sorted(report.get("collections", []), key=lambda x: x.get("total_size", 0), reverse=True)
        if rows:
            lines.append("📌 <b>Collections:</b>")
            for row in rows:
                err = row.get("error")
                if err:
                    lines.append(f"   • <code>{html.escape(row['name'])}</code> — ERROR")
                    continue
                lines.append(
                    f"   • <code>{html.escape(row['name'])}</code> | "
                    f"docs: <b>{row['documents']:,}</b> | "
                    f"data: {_human_bytes(row['data_size'])} | "
                    f"storage: {_human_bytes(row['storage_size'])} | "
                    f"indexes: {_human_bytes(row['index_size'])}"
                )
        lines.append("")

    # Optional Mongo capacity configured by the owner. MongoDB does not expose
    # an Atlas/managed-cluster plan quota through dbStats, so never invent one.
    try:
        mongo_limit_gb = float(os.getenv("MONGO_STORAGE_LIMIT_GB", "0") or 0)
    except Exception:
        mongo_limit_gb = 0
    if mongo_limit_gb > 0:
        used_mongo = 0
        for db in (db1, db2):
            try:
                ...
                used_mongo += float(st.get("storageSize", 0) or 0) + float(st.get("indexSize", 0) or 0)
            except Exception:
                pass
        limit_bytes = mongo_limit_gb * 1024**3
        free_bytes = max(0, limit_bytes - used_mongo)
        lines += [
            "☁️ <b>CONFIGURED MONGODB CAPACITY</b>",
            f"📦 Limit: <b>{_human_bytes(limit_bytes)}</b>",
            f"🟠 Used: <b>{_human_bytes(used_mongo)}</b>",
            f"🟢 Remaining: <b>{_human_bytes(free_bytes)}</b>",
            "",
        ]
    else:
        lines += [
            "☁️ <b>MONGODB REMAINING CAPACITY</b>",
            "ℹ️ Provider quota is not exposed by MongoDB dbStats.",
            "Set <code>MONGO_STORAGE_LIMIT_GB</code> in Railway/hosting variables if you want the bot to calculate the configured remaining quota.",
            "",
        ]

    lines += [
        "🔐 <b>Configured storage sources</b>",
        f"MongoDB #1: <code>{html.escape(str(MONGO_URI_1).split('@')[-1])}</code>",
        f"MongoDB #2: <code>{html.escape(str(MONGO_URI_2).split('@')[-1])}</code>",
        f"WhatsApp notifications endpoint: <b>{'CONFIGURED' if WAFORGE_MESSAGE_URL else 'NOT CONFIGURED'}</b>",
        "",
        "ℹ️ Collection 'storage' is the Mongo allocated collection space; 'data' is logical document data; 'indexes' is index space.",
    ]
    return "\n".join(lines)
...
def admin_see_storage(m):
    if not is_admin(m.from_user.id):
        return
    try:
        text = _admin_storage_report_text()
        # Telegram messages have a finite length; split without cutting too aggressively.
        for i in range(0, len(text), 3800):
            bot.send_message(m.chat.id, text[i:i+3800], parse_mode="HTML")
    except Exception as e:
        bot.send_message(m.chat.id, f"❌ Storage report failed: <code>{html.escape(str(e)[:500])}</code>", parse_mode="HTML")
