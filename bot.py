...
def _music_artist_from_info(info):
    """Pick the best artist metadata; prefer an explicit artist/lead artist over uploader."""
    info=info or {}
    ...
    if isinstance(artists,list):
        names=[]
        for item in artists:
            if isinstance(item,dict): name=item.get("name") or item.get("artist")
            else: name=item
            if isinstance(name,str) and name.strip(): names.append(name.strip())
        if names:
            return names[0]
    for key in ("uploader","channel","uploader_id"):
        value=info.get(key)
        if isinstance(value,str) and value.strip():
            return value.strip()
    return ""

def _download_music_cover(info, tmp_dir):
    """Download the track's own thumbnail/cover, not the original video file."""
    info=info or {}
    urls=[]
    ...
    for idx,url in enumerate(urls[:4]):
        try:
            ...
            # Telegram audio thumbnails should be JPEG. Convert when Pillow is available.
            try:
...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    try:
        ...
        cover_path = _download_music_cover(info, tmp_dir)
        caption = f"🎵 {title}\n👤 Artist: {artist}\n\n{DOWNLOAD_CAPTION}"
