...
# WaForge's documented public OTP API is /api/otp/{send,verify}.
# Keep a custom base URL option for self-hosted/legacy deployments.
...
def verify_waforge_whatsapp(phone_number, code):
    ...
    payload = {"to": phone_number, "code": str(code)}
...
def delayed_cancel_session(chat_id, message_id, uid):
    # OTP sessions stay active long enough for D7 Verify's configured 10-minute TTL.
    time.sleep(600)
...
def process_verification_phone(m):
    ...
    msg=bot.send_message(m.chat.id,f"📲 A 6-digit OTP has been sent via SMS to <code>{phone_input}</code>.\n\nEnter the code below.\n⏳ Valid for 10 minutes.",reply_markup=kb)
...
def resend_phone_code_callback(call):
    ...
    kb=InlineKeyboardMarkup(); kb.row(InlineKeyboardButton("🔄 Resend SMS",callback_data="resend_phone_code"),InlineKeyboardButton("❌ Cancel",callback_data="cancel_verify_process"))
    msg=bot.send_message(call.message.chat.id,"📲 <b>New SMS OTP sent.</b>\n\nEnter the new 6-digit code.",reply_markup=kb)
    bot.register_next_step_handler(msg,process_phone_code)
...
def process_phone_code(m):
    ...
    if ok:
        users[uid]["verified"]=True; users[uid]["phone"]=data["phone"]; users[uid]["sticker"]=users[uid].get("sticker") or "🌟"; save_user(uid); phone_verify_pending.pop(uid,None)
        trial_activated=activate_pending_trial(uid,m.chat.id)
