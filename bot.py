...
def sms_admin_manager(m):
    ...
    if m.text == "🟢 Open SMS":
        ...
        try:
            bot.send_message(m.chat.id, "🟢 WhatsApp verification is now OPEN. Users can request OTP via WaForge.")
        except: pass
...
def process_verification_phone(m):
    ...
    success, detail = send_d7_sms(phone_input, sms_text)
    if success:
        try:
            kb = InlineKeyboardMarkup()
            kb.row(
...
def resend_phone_code_callback(call):
    ...
    success, detail = send_d7_sms(phone, sms_text)
    if success:
...
def process_verification_whatsapp(m):
    uid = str(m.from_user.id)
    phone_input = (m.text or "").strip().replace(" ", "")
    if not re.fullmatch(r"\+[1-9]\d{9,14}", phone_input):
    ...
    if not ok:
        bot.send_message(
            m.chat.id,
            "❌ <b>WaForge could not send the WhatsApp OTP.</b>\n\n"
    ...
    msg = bot.send_message(
        m.chat.id,
        f"📲 <b>WhatsApp OTP sent</b>\n\n"
        f"Number: <code>{phone_input}</code>\n"
        "Enter the 6-digit code sent to your WhatsApp.\n\n"
        "⏳ <b>Expires in 5 minutes.</b>\n"
        "📋 In WhatsApp, use the Copy Code button to copy the OTP if it is shown.",
        reply_markup=whatsapp_otp_keyboard(),
    )
...
def resend_whatsapp_code_callback(call):
    uid = str(call.from_user.id)
    data = whatsapp_verify_pending.get(uid)
    if not data:
    ...
    if not ok:
        print("WaForge resend error:", detail)
        bot.answer_callback_query(call.id, "Could not send WaForge OTP.", show_alert=True)
        return

    now = time.time()
    data["request_id"] = request_id
    data["last_resend"] = now
    data["time"] = now
    bot.answer_callback_query(call.id, "✅ New WaForge OTP sent")
    msg = bot.send_message(
        call.message.chat.id,
        "📲 <b>New WhatsApp OTP sent.</b>\n\n"
        "Enter the new 6-digit code.\n"
        "⏳ <b>This code expires in 5 minutes.</b>\n"
        "📋 Use the Copy Code button in WhatsApp if available.",
        reply_markup=whatsapp_otp_keyboard(),
    )
    bot.register_next_step_handler(msg, process_whatsapp_code)
    threading.Thread(target=expire_whatsapp_session, args=(uid, now), daemon=True).start()
...
def process_whatsapp_code(m):
    ...
    data = whatsapp_verify_pending.get(uid)
    if not data:
        bot.send_message(m.chat.id, "⏰ <b>This code has expired.</b>\n\nPlease request a new WhatsApp OTP.")
        return

    # Enforce the same 5-minute lifetime locally as a second safety check.
    if time.time() - data.get("time", 0) >= WAFORGE_OTP_TTL:
        whatsapp_verify_pending.pop(uid, None)
        bot.send_message(m.chat.id, "⏰ <b>This code has expired.</b>\n\nPlease request a new WhatsApp OTP.", reply_markup=whatsapp_otp_keyboard())
        return

    code = (m.text or "").strip()
    if not re.fullmatch(r"\d{6}", code):
        msg = bot.send_message(m.chat.id, "⚠️ Please enter the 6-digit WhatsApp code:", reply_markup=whatsapp_otp_keyboard())
        bot.register_next_step_handler(msg, process_whatsapp_code)
        return
    ...
    if not ok:
        ...
        if "expired" in detail_text or "expire" in detail_text or "410" in detail_text:
            ...
            bot.send_message(m.chat.id, "⏰ <b>This code has expired.</b>\n\nPlease request a new WhatsApp OTP.", reply_markup=whatsapp_otp_keyboard())
            return
        msg = bot.send_message(m.chat.id, "❌ Incorrect WhatsApp code. Please try again:", reply_markup=whatsapp_otp_keyboard())
        bot.register_next_step_handler(msg, process_whatsapp_code)
        return
    ...
    bot.send_message(
        m.chat.id,
        "✅ <b>WhatsApp verification successful.</b> Your account is now verified."
        + ("\n\n🎁 Your pending 1-Day Premium Trial was activated automatically." if trial_activated else ""),
        reply_markup=localized_user_menu(uid),
    )
...
def check_membership(user_id):
    """Check the configured force-join channel without replacing the old UI.
    If Telegram cannot verify membership, keep the JOIN + CONFIRM buttons visible
    instead of sending a bare warning.
    """
    touch_user(user_id)
    try:
        ...
        if status in ("member", "administrator", "creator"):
            bot.send_message(user_id, render_start_message(str(user_id)),
                             reply_markup=localized_user_menu(str(user_id)), parse_mode="HTML")
            return True
...
def balance_handler(m):
    ...
    text=f"💰 <b>Current Balance:</b> {format_asset(code,available)}\n"
    text += f"💵 <b>Total Portfolio Value:</b> ${total_assets_usd(uid):,.2f} USD\n"
...
@bot.message_handler(func=lambda m: m.text in ["🟢 OPEN HOLD","🔴 CLOSE HOLD","🟢 OPEN CRYPTO","🔴 CLOSE CRYPTO","💸 CRYPTO FEE","⛽ GAS FEE","📈 CRYPTO FEE STATS"])
def crypto_admin_controls(m):
    ...
    if action=="🔴 CLOSE CRYPTO":
        ...
        converted=0; fees=0.0
        # Safely liquidate portfolio/current crypto to USD using the live market feed.
        for uid,u in list(users.items()):
            with CRYPTO_TX_LOCK:
                total_usd=0.0
                cur=cur_code(uid); cur_amt=balance_amount(uid)
                if is_crypto(cur) and cur_amt>0:
                    gross=asset_to_usd(cur,cur_amt); fee,gas,total=crypto_fee_usd(gross,cur); net=max(0,gross-total); total_usd+=net
                    crypto_fee_ledger_col.insert_one({"user_id":uid,"type":"crypto_close_fee","from_asset":cur,"to_asset":"USD","gross_usd":gross,"fee_usd":fee,"gas_fee_usd":gas,"net_usd":net,"time":datetime.now(timezone.utc)})
                    fees+=fee+gas; u["balance"]=0.0
                p=u.get("portfolio") if isinstance(u.get("portfolio"),dict) else {}
                for code,amt in list(p.items()):
                    if is_crypto(code) and float(amt or 0)>0:
                        gross=asset_to_usd(code,float(amt)); fee,gas,total=crypto_fee_usd(gross,code); total_usd+=max(0,gross-total); fees+=fee+gas
                        crypto_fee_ledger_col.insert_one({"user_id":uid,"type":"crypto_close_fee","from_asset":code,"to_asset":"USD","gross_usd":gross,"fee_usd":fee,"gas_fee_usd":gas,"net_usd":max(0,gross-total),"time":datetime.now(timezone.utc)})
                        p[code]=0.0
                if total_usd>0:
                    u["portfolio"]=p; u["balance_asset"]="USD"; u["currency"]="USD"; u["balance"]=round(float(u.get("balance",0) or 0)+total_usd,12); save_user(uid); converted+=1
                    try: bot.send_message(int(uid),f"🔴 <b>Crypto service closed by admin.</b>\nYour crypto holdings were returned to your USD balance at the current live market price, after applicable fees.")
                    except: pass
        bot.send_message(m.chat.id,f"🔴 <b>Crypto CLOSED.</b>\nUsers affected: {converted}\nFees + gas recorded: <b>${fees:,.2f}</b>"); return
...
def premium_features_text():
    return (
        ...
        "▶️ Full YouTube videos\n"
        "📥 10 extra popular platforms\n"
        "🎵 Better music metadata\n"
        "⚡ Premium download queue\n\n"
        "🔐 Premium purchase requires account verification."
    )
...
def show_premium_menu(chat_id):
    ...
    if is_premium(uid):
        ...
        if not users.get(uid, {}).get("verified", False):
            ...
            bot.send_message(chat_id, premium_features_text() + "\n\n⚠️ Verify your account first, then return to Premium.", reply_markup=kb)
            return
...
def premium_confirm_callback(call):
    ...
    bot.edit_message_text(f"🎉 <b>Premium Activated!</b>\n\n📅 Duration: {months} months\n💵 Paid: ${price:.2f}\n⏰ Expires: {until.strftime('%Y-%m-%d %H:%M UTC')}\n\n🚀 Faster speed and high-quality downloads are now enabled.", call.message.chat.id, call.message.message_id)
...
def send_premium_email(uid, subject, months, price, until):
    ...
    body=("<html><body style='margin:0;background:#f3f8f5;font-family:Arial;color:#10231d'>"
          ...
          f"<div style='padding:30px'><h2>{html.escape(subject)}</h2><p>Your Premium access is active.</p><div style='background:#f0faf6;padding:18px;border-radius:14px'><b>Duration:</b> {months if months else 'Trial / Reward'}<br><b>Paid:</b> ${price:.2f}<br><b>Expires:</b> {until.strftime('%Y-%m-%d %H:%M UTC')}</div><p>Thank you for using our service.</p></div></div></body></html>")
        ...
        try:
            ...
            for uid,data in list(users.items()):
                ...
                if dt<=now:
                    ...
                    try: bot.send_message(int(uid),"⏰ <b>Your Downloader Bot Premium has expired.</b>\n\n💎 Open Premium to renew.")
                    except Exception: pass
...
def handle_links(message):
    ...
    if VERIFY_ENABLED and not users.get(uid, {}).get("verified", False):
    ...
    platform = detect_platform(link)
    # Normal YouTube videos are Premium-only; Shorts remain available to free users.
    if platform == "youtube" and not youtube_is_short(link) and not is_premium(uid):
        kb = InlineKeyboardMarkup()
...
def add_new_admin_start(m):
    if not is_admin(m.from_user.id): return
    msg=bot.send_message(m.chat.id,"👑 Send the Telegram User ID of the new admin:")
...
def add_new_admin_process(m):
    ...
    try:
        ...
        ids=get_admin_ids()
        if uid in ids: bot.send_message(m.chat.id,"⚠️ This user is already an admin."); return
        ids.append(uid); save_admin_ids(ids)
        bot.send_message(m.chat.id,f"✅ Admin added: <code>{uid}</code>")
        try: bot.send_message(uid,"👑 You have been added as an administrator.")
        except: pass
    except Exception: bot.send_message(m.chat.id,"❌ Invalid Telegram ID.")

@bot.message_handler(func=lambda m: m.text == "➖ REMOVE ADMIN")
def remove_admin_start(m):
    if not is_admin(m.from_user.id): return
    msg=bot.send_message(m.chat.id,"👑 Send the Telegram User ID of the admin to remove:")
...
def remove_admin_process(m):
    ...
    try:
        ...
        if uid==PRIMARY_ADMIN_ID:
            bot.send_message(m.chat.id,"🔒 The main/general admin cannot be removed."); return
        ids=[x for x in get_admin_ids() if x!=uid]; save_admin_ids(ids)
        bot.send_message(m.chat.id,f"✅ Admin removed: <code>{uid}</code>")
    except Exception: bot.send_message(m.chat.id,"❌ Invalid Telegram ID.")
...
def see_all_balance(m):
    ...
    for uid,u in users.items():
        total=total_assets_usd(uid)
        username=('@'+u.get('username')) if u.get('username') else 'No username'
        rows.append((total,uid,username))
    rows.sort(reverse=True)
    lines=["📊 <b>ALL USER BALANCES</b>",f"👥 Users: <b>{len(rows)}</b>",""]
    for total,uid,username in rows[:500]:
        lines.append(f"• <code>{uid}</code> | {username} | <b>${total:,.2f} USD</b>")
    if not rows: lines.append("No users found.")
    bot.send_message(m.chat.id,"\n".join(lines))

@bot.message_handler(func=lambda m: m.text == "💰 SEE BALANCE")
def see_balance_start(m):
    if not is_admin(m.from_user.id): return
    msg=bot.send_message(m.chat.id,"📝 Send the message you want to send together with each user's balance.\nExample: You have $100 balance Come use your balance")
...
def see_balance_process(m):
    ...
    for uid in list(users):
        try:
            total=total_assets_usd(uid)
            code=cur_code(uid); current=balance_amount(uid)
            text=f"💰 <b>Your Balance</b>\n\n{custom}\n\n💵 Total Value: <b>${total:,.2f} USD</b>\n💼 Current Holding: <b>{format_asset(code,current)}</b>"
            bot.send_message(int(uid),text); sent+=1
        except Exception: pass
    bot.send_message(m.chat.id,f"✅ Balance message sent to <b>{sent}</b> users.")

@bot.message_handler(func=lambda m: m.text in ["💱 MIN CHANGE CURRENCY","📉 CHANGE MINIMUM"])
def min_change_currency_start(m):
    if not is_admin(m.from_user.id): return
    current=float(get_setting("currency_change_min_usd",5.0) or 0)
    msg=bot.send_message(m.chat.id,f"💱 <b>Minimum Change Currency</b>\n\nCurrent minimum: <b>${current:,.2f} USD</b>\n\nSend new minimum in USD. Send <b>0</b> to allow any amount.")
    bot.register_next_step_handler(msg,min_change_currency_process)

def min_change_currency_process(m):
    if not is_admin(m.from_user.id): return
    try:
        value=float((m.text or '').replace('$','').replace(',','').strip())
        if value<0 or value>100000000: raise ValueError
        set_setting("currency_change_min_usd",value)
        bot.send_message(m.chat.id,f"✅ Minimum Change Currency set to <b>${value:,.2f} USD</b>." if value>0 else "✅ Minimum Change Currency disabled. Users can change from $0 minimum (subject to having a positive balance).")
    except Exception: bot.send_message(m.chat.id,"❌ Invalid amount. Example: 5 or 0")

@bot.message_handler(func=lambda m: m.text == "☎️ CUSTOMER")
def edit_customer_start(m):
    if not is_admin(m.from_user.id): return
    msg=bot.send_message(m.chat.id,
        "☎️ <b>CUSTOMER</b>\n\nSend the customer Telegram username.\nExample: <code>@vexdou1</code>")
    bot.register_next_step_handler(msg, edit_customer_find)

def edit_customer_find(m):
    if not is_admin(m.from_user.id): return
    username=(m.text or '').strip().lstrip('@')
    if not username:
        bot.send_message(m.chat.id,"❌ Please send a valid username, e.g. <code>@vexdou1</code>")
        return
    uid=None
    for user_id, data in users.items():
        old=str(data.get('username','')).strip().lstrip('@')
        if old.lower()==username.lower():
            uid=str(user_id)
            break
    if not uid:
        bot.send_message(m.chat.id,f"❌ Customer <code>@{username}</code> not found.")
        return
    msg=bot.send_message(m.chat.id,
        f"👤 Customer found: <b>@{username}</b>\n\nSend the new username.\nExample: <code>@newusername</code>")
    bot.register_next_step_handler(msg, edit_customer_save, uid)

def edit_customer_save(m, uid):
    if not is_admin(m.from_user.id): return
    new_username=(m.text or '').strip().lstrip('@')
    if not new_username or ' ' in new_username:
        bot.send_message(m.chat.id,"❌ Invalid username. Example: <code>@vexdou1</code>")
        return
    for other_uid, data in users.items():
        if str(other_uid) != str(uid) and str(data.get('username','')).strip().lstrip('@').lower()==new_username.lower():
            bot.send_message(m.chat.id,"❌ That username is already assigned to another customer.")
            return
    old_username=str(users[str(uid)].get('username','')).strip().lstrip('@')
    users[str(uid)]['username']=new_username
    save_user(str(uid))
    bot.send_message(m.chat.id,
        f"✅ Customer username updated successfully.\n\n👤 Old: <code>@{old_username}</code>\n👤 New: <code>@{new_username}</code>")
...
def activate_pending_trial(uid, chat_id=None):
    uid=str(uid); u=users.get(uid,{})
    if not u.get("trial_pending") or not u.get("verified") or not trial_enabled():
        return False
    ...
    if chat_id is not None:
        try: bot.send_message(chat_id,f"🎁 <b>{days} DAY PREMIUM TRIAL ACTIVATED!</b>\n\n⏰ Expires: {until.strftime('%Y-%m-%d %H:%M UTC')}",reply_markup=localized_user_menu(uid))
        except Exception: pass
...
def premium_trial_callback(call):
    ...
    days=trial_days()
    if not users.get(uid,{}).get("verified"):
...
def change_currency_button(m):
    uid=str(m.from_user.id)
    if not crypto_system_open():
        bot.send_message(m.chat.id,"⚠️ <b>This Version is Not Available Now!</b>\n\nCrypto Currency system is currently closed by admin.",reply_markup=localized_user_menu(uid)); return
    current=cur_code(uid); minimum=float(get_setting("currency_change_min_usd",5.0) or 0); usd_value=asset_to_usd(current,available_asset_amount(uid))
    bot.send_message(m.chat.id,
        f"💱 <b>CHANGE CURRENCY</b>\n\nCurrent: <b>{current}</b>\nAvailable: <b>{format_asset(current,available_asset_amount(uid))}</b>\nUSD value: <b>${usd_value:,.2f}</b>\nMinimum: <b>${minimum:,.2f} USD</b>\n\n✍️ Choose a currency, then enter the exact amount you want to convert.\nYou will see the live rate, fee and gas fee before CONFIRM.", reply_markup=currency_kb())
...
def currency_select_callback(call):
    ...
    if new in CRYPTO_CURRENCIES and not crypto_system_open():
    ...
    if available<=0: bot.answer_callback_query(call.id,"❌ No available balance",show_alert=True); return
    refresh_market_rates(False)
    if asset_usd_price(new)<=0: bot.answer_callback_query(call.id,"❌ Live market price unavailable",show_alert=True); return
    if blocked_amount(uid)>0 or hold_amount_usd(uid)>0:
        bot.answer_callback_query(call.id,"❌ Clear active hold/blocked funds first.",show_alert=True); return
    minimum=float(get_setting("currency_change_min_usd",5.0) or 0)
    msg=bot.send_message(call.message.chat.id,
        f"✍️ <b>ENTER AMOUNT</b>\n\nCurrent balance: <b>{format_asset(old,available)}</b>\nCurrent USD value: <b>${asset_to_usd(old,available):,.2f}</b>\nTarget: <b>{new}</b>\nMinimum conversion: <b>${minimum:,.2f} USD</b>\n\nSend the amount in <b>{old}</b> that you want to convert.\nExample: <code>5</code>", reply_markup=ReplyKeyboardMarkup(resize_keyboard=True).add("❌ CANCEL CONVERSION"))
    users[uid]["pending_conversion_amount"]={"to":new,"from":old}
    save_user(uid)
    bot.answer_callback_query(call.id)
    bot.register_next_step_handler(msg, conversion_amount_step)
...
def conversion_amount_step(m):
    ...
    try:
        ...
        msg=bot.send_message(m.chat.id,"❌ Invalid amount. Send a number, e.g. <code>5</code>.")
        bot.register_next_step_handler(msg,conversion_amount_step); return
    old=p.get("from"); new=p.get("to"); available=available_asset_amount(uid)
    if cur_code(uid)!=old or amount>available+1e-12:
        bot.send_message(m.chat.id,f"❌ Insufficient available balance. Available: <b>{format_asset(old,available)}</b>",reply_markup=localized_user_menu(uid)); return
    gross=asset_to_usd(old,amount); minimum=float(get_setting("currency_change_min_usd",5.0) or 0)
    if minimum>0 and gross<minimum:
        msg=bot.send_message(m.chat.id,f"❌ Minimum is <b>${minimum:,.2f} USD</b>. Your amount is <b>${gross:,.2f}</b>. Send a larger amount.")
        bot.register_next_step_handler(msg,conversion_amount_step); return
    fee,gas,total=crypto_fee_usd(gross,new); net=max(0,gross-total); new_amt=usd_to_asset(new,net)
    if new_amt<=0 or net<=0:
        bot.send_message(m.chat.id,"❌ Amount is too small after fees.",reply_markup=localized_user_menu(uid)); return
    nonce=uuid.uuid4().hex[:12]
    users[uid]["pending_conversion"]={"nonce":nonce,"from":old,"to":new,"amount":amount,"gross_usd":gross,"fee_usd":fee,"gas_usd":gas,"net_usd":net,"new_amount":new_amt,"created_at":datetime.now(timezone.utc).isoformat()}
    ...
    bot.send_message(m.chat.id,
        f"🧾 <b>CONVERSION PREVIEW</b>\n\nFrom: <b>{format_asset(old,amount)}</b>\nGross value: <b>${gross:,.2f} USD</b>\nTo: <b>{new}</b>\nRate: <b>{market_rate_text(new)}</b>\nEstimated receive: <b>{format_asset(new,new_amt)}</b>\n\n💸 Trading fee ({crypto_fee_percent():.4f}%): <b>-${fee:,.2f}</b>\n⛽ Network/Gas reserve: <b>-${gas:,.2f}</b>\n💰 Total fees: <b>-${total:,.2f} USD</b>\n📥 Net value: <b>${net:,.2f} USD</b>\n\n⚠️ Market price can move before confirmation.\nPress <b>CONFIRM</b> to execute.",reply_markup=kb)
...
def convert_confirm_callback(call):
    ...
    with CRYPTO_TX_LOCK:
        if p.get("to") in CRYPTO_CURRENCIES and not crypto_system_open():
            users[uid].pop("pending_conversion",None); save_user(uid); bot.answer_callback_query(call.id,"Crypto is currently closed",show_alert=True); return
        old=p.get("from"); new=p.get("to"); amount=float(p.get("amount",0)); available=available_asset_amount(uid)
        ...
        refresh_market_rates(True); gross=asset_to_usd(old,amount); fee,gas,total=crypto_fee_usd(gross,new); net=gross-total; new_amt=usd_to_asset(new,net)
        minimum=float(get_setting("currency_change_min_usd",5.0) or 0)
        if minimum>0 and gross<minimum or net<=0 or new_amt<=0:
            bot.answer_callback_query(call.id,"Conversion no longer valid",show_alert=True); return
        # Deduct only the amount the user selected; remainder stays in the current asset.
        users[uid]["balance"]=round(max(0.0,balance_amount(uid)-amount),12)
        if is_crypto(new): add_portfolio_asset(uid,new,new_amt)
        elif new=="USD":
            users[uid]["balance"]=round(balance_amount(uid)+net,12)
        else:
            # Non-crypto target is represented as a portfolio holding so cash is not overwritten.
    ...
    bot.send_message(call.message.chat.id,
        f"✅ <b>CONVERSION COMPLETE</b>\n\n💰 Used: <b>{format_asset(old,amount)}</b>\n📥 Received: <b>{format_asset(new,new_amt)}</b>\n💸 Fee: <b>${fee:,.2f}</b>\n⛽ Gas fee: <b>${gas:,.2f}</b>\n💵 Net value: <b>${net:,.2f} USD</b>\n📊 Live rate: <b>{market_rate_text(new)}</b>{hold_txt}\n\nYour remaining balance was not converted.",reply_markup=localized_user_menu(uid))
