...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    try:
        ...
        # 2) Cross-check with independent music catalogs when fingerprinting did not
        # already identify the song. This is what fixes Original Sound metadata that
        # yt-dlp exposes only as a video/uploader name.
        catalog_meta = {}
        if not recognized_title or not recognized_artist:
            for lookup_fn in (_music_musicbrainz_lookup, _music_deezer_lookup, _music_itunes_lookup):
                try:
                    catalog_meta = lookup_fn(title, artist if artist != "Unknown artist" else "")
                except Exception:
                    catalog_meta = {}
            ...
            if catalog_meta:
                # Catalog title/artist wins over uploader-derived metadata.
