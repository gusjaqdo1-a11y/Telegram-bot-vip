...
SAVEAPI_KEY = os.getenv("SAVEAPI_KEY", "").strip()
SAVEAPI_URL = os.getenv("SAVEAPI_URL", "https://api.saveapi.org/v1/download").strip()
...
def promo_user_menu(m):
    ...
    if not bool(get_setting("promo_system_enabled",False)):
        ...
        return
    # Prefer announced codes, but if the admin opened the system before SEND ALL,
    # show the newest active code instead of incorrectly reporting no code.
    public=_promo_public_codes()
    active=_active_promo_codes()
    x=(public or active)
    if not x:
        bot.send_message(m.chat.id,"🎟 <b>PROMO CODE</b>\n\nThe promo system is open, but there is no active promo code yet.",reply_markup=localized_user_menu(uid),parse_mode="HTML")
        return
    x=x[0]
    code=str(x.get("code") or x.get("_id") or "")
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("OPEN",callback_data=f"promo_open:{code}"))
    bot.send_message(m.chat.id,
        "🎟 <b>PROMO CODE</b>\n\n"
        f"Promo Code: <code>{html.escape(code)}</code>\n\n"
        "Tap <b>OPEN</b>, then enter the code to receive your reward.",
...
def promo_admin_callback(call):
    ...
    if action=="open":
        ...
        _promo_collection().update_many({"enabled":True,"broadcasted":False},{"$set":{"broadcasted":True,"broadcasted_at":now}})
        set_setting("promo_system_enabled",True)
        _refresh_all_user_menus("🔄 Promo system updated.")
        bot.answer_callback_query(call.id,"Promo system OPEN")
        try:
            bot.send_message(call.message.chat.id,"🟢 <b>PROMO SYSTEM OPEN</b>\n\nActive promo codes are now visible to users. Use <b>SEND ALL</b> if you also want to broadcast the code immediately.",parse_mode="HTML",reply_markup=_promo_admin_menu())
        except Exception: pass
    elif action=="close":
...
def _saveapi_download(link, tmp_dir, uid=None, platform=None):
    ...
    r=requests.get(SAVEAPI_URL,params={"url":link},headers=headers,timeout=SAVEAPI_TIMEOUT)
    if r.status_code in (401,403):
    ...
    saved=[]
    for i,item in enumerate(medias[:20]):
...
def download_media(chat_id, link, message_id, quality=None):
    ...
    try:
        ...
        if not media and platform in SAVEAPI_SUPPORTED_PLATFORMS and _saveapi_enabled():
            try:
                media=_saveapi_download(link,tmp,uid=uid,platform=platform); provider="saveapi"
            except Exception as e:
...
def _youtube_song_search(query, limit=8):
    """Fast song search fallback using yt-dlp's public YouTube search metadata.

    Jamendo remains the first source for downloadable licensed tracks. When a
    popular/commercial song is not present there, this returns YouTube results
    so the user can still find the exact title/artist. Downloading content is
    subject to the source's terms and the user's rights.
    """
    query=_music_clean_text(query)
    if not query: return []
    try:
        ...
        for item in entries:
            ...
            if not url and vid and len(vid)==11:
            ...
            rows.append({
                "id":vid or hashlib.sha1(url.encode()).hexdigest()[:16],
                "title":title,"artist":artist,"duration":dur,"album":"",
                "cover":item.get("thumbnail"),"download":url,"download_allowed":True,
                "license":"","source":"youtube","webpage_url":url,
            })
        rows.sort(key=lambda x:_song_similarity(query,x.get("title",""),x.get("artist","")),reverse=True)
        return rows[:int(limit)]
    except Exception as e:
