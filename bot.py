...
PREMIUM_CONCURRENT_DOWNLOADS = int(os.getenv("PREMIUM_CONCURRENT_DOWNLOADS", "20"))
...
def send_waforge_whatsapp(phone_number, user_id):
    ...
    headers={"Authorization":f"Bearer {WAFORGE_API_KEY}","Content-Type":"application/json","Accept":"application/json"}
    try:
        r=requests.post(WAFORGE_OTP_SEND_URL,json={"phone":phone_number},headers=headers,timeout=30)
        try: body=r.json()
        except Exception: body={"raw":r.text}
        if r.status_code in (200,201,202):
            rid=body.get("id") or body.get("request_id") or body.get("uuid")
...
def verify_waforge_whatsapp(phone_number, code):
    ...
    headers={"Authorization":f"Bearer {WAFORGE_API_KEY}","Content-Type":"application/json","Accept":"application/json"}
    try:
        r=requests.post(WAFORGE_OTP_VERIFY_URL,json={"phone":phone_number,"code":str(code)},headers=headers,timeout=30)
        try: body=r.json()
        except Exception: body={"raw":r.text}
        ok=r.status_code==200 and (body.get("verified") is True or body.get("success") is True)
        return ok,body
    except Exception as e:
        return False,str(e)
...
def process_verification_whatsapp(m):
    ...
    if not ok:
        print("WaForge OTP error:",detail)
        bot.send_message(m.chat.id,"❌ <b>WaForge could not send the WhatsApp OTP.</b>\n\nPlease check WAFORGE_API_KEY/phone number and try again.")
        return
...
def detect_platform(link):
    ...
    for platform, patterns in PLATFORM_PATTERNS.items():
        if any(p in link_l for p in patterns):
            return platform
    return "unknown"
...
def _run_ytdlp_download(link, tmp_dir, platform, fmt, base_opts, max_duration, uid=None, quality=None):
    ...
    if platform=="youtube":
        ...
        common["format"]=fmt
        if platform=="instagram": common=_instagram_opts(common)
        attempts=[common]
        if platform=="instagram" and common.get("cookiefile"):
            public_opts=dict(common); public_opts.pop("cookiefile",None); attempts.append(public_opts)
    last_error=None
    for idx,opts in enumerate(attempts):
        try:
            with yt_dlp.YoutubeDL(opts) as ydl: return ydl.extract_info(link,download=True)
        except Exception as exc:
            last_error=exc; print(f"yt-dlp attempt {idx+1}/{len(attempts)} failed:",repr(exc))
            if platform!="youtube" and not (platform=="instagram" and idx+1<len(attempts) and _is_retryable_instagram_error(exc)): raise
...
def download_media(chat_id, link, message_id, quality=None):
    ...
    try:
        ...
        if not media and platform in {"youtube","instagram"} and _cobalt_enabled():
            try:
        ...
        if not media:
            ...
            opts={
                ...
                "concurrent_fragment_downloads":8 if premium else 1,
                "ratelimit": None,
                **cookie_args
            }
...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    try:
        ...
        if not mp3_files and prepared:
            ...
            if os.path.isfile(candidate): mp3_files = [candidate]
        if not mp3_files:
        ...
        try: bot.edit_message_text(friendly, chat_id, status_message_id)
        except Exception: pass
    finally:
...
def balance_handler(m):
    ...
    code=cur_code(uid); available=available_asset_amount(uid); hold=hold_amount_usd(uid); blocked=blocked_amount(uid)
    text=f"💰 <b>Balance:</b> {format_asset(code,available)}\n"
    text += f"🔒 <b>Blocked Amount:</b> {format_asset(code,blocked)}"
    # New/non-crypto users only see their available and blocked balance.
    crypto_assets=any(is_crypto(c) and float(a or 0)>1e-12 for c,a in get_portfolio(uid).items()) or is_crypto(code) and balance_amount(uid)>1e-12
    allowed,_,_=crypto_access_status(uid)
    if not crypto_assets and not allowed:
        bot.send_message(m.chat.id,text,reply_markup=localized_user_menu(uid)); return
    text += f"\n💵 <b>Total Portfolio Value:</b> ${total_assets_usd(uid):,.2f} USD"
    if hold > 0.0000000001: text += f"\n🔒 <b>Amount on Hold:</b> ${hold:,.2f} USD"
    rows=portfolio_text(uid)
    if rows:
        text += "\n\n💼 <b>INVESTMENTS</b>"
        for usd, c, a in rows[:30]: text += f"\n• {format_asset(c,a)} ≈ ${usd:,.2f}"
    kb=InlineKeyboardMarkup()
    kb.add(InlineKeyboardButton("💼 VIEW PORTFOLIO", callback_data="view_portfolio"))
    if hold > 0.0000000001: kb.add(InlineKeyboardButton("❓ What Means Hold",callback_data="what_hold"))
    bot.send_message(m.chat.id,text,reply_markup=kb)
...
# ================= CUSTOMER AI — FULL SYSTEM SUPPORT BOT =================
# CUSTOMER_AI_BOT_TOKEN is a separate BotFather token. BOT2_TOKEN remains the
# existing verification bot token so the verification flow is not broken.
AI_SUPPORT_PENDING=set()

CUSTOMER_AI_SYSTEM_PROMPT = """
You are the Customer AI for this Telegram Downloader system. You are a smart,
calm, accurate support assistant. Explain the system clearly in the user's
language. You can use the user's live account data from MongoDB and the live
system settings available to this bot. Never invent balances, Premium expiry,
verification state, crypto prices, withdrawal status, or admin settings.
If data is unavailable, say that it is unavailable.

You understand: account/profile, balance and blocked money, currency exchange,
crypto access/minimum deposit, crypto fees and 1-hour holds, Premium/trial,
verification (Gmail, D7 SMS, WaForge WhatsApp), YouTube Premium-only access,
RapidAPI/Cobalt/yt-dlp download flow, music/MP3, supported platforms, Force
Join, referrals, withdrawals, feedback, language, customer support, bot ID,
Telegram ID, download limits, admin controls, and common errors.
"""


def _ai_user_context(uid):
    """Build a live, privacy-aware account snapshot for the Customer AI."""
    uid=str(uid)
    u=users.get(uid,{})
    try:
    ...
    try:
        ...
        downloads=0
    verified=user_is_verified(uid) if uid in users else False
    premium=is_premium(uid)
    locked=balance_is_locked(uid)
    return {
        "uid":uid,
        "username":u.get("username") or "",
        "name":u.get("first_name") or "User",
        "bot_id":u.get("bot_id") or "N/A",
        "balance":balance_amount(uid),
        "asset":cur_code(uid),
        "balance_usd":balance_usd_value(uid),
        "blocked":blocked_amount(uid),
        "portfolio_usd":portfolio_usd_value(uid),
        "total_assets_usd":total_assets_usd(uid),
        "verified":verified,
        "verification_method":u.get("verification_method") or "N/A",
        "email":u.get("email") or "N/A",
        "phone":u.get("phone") or "N/A",
        "premium":premium,
        "premium_until":premium_until_text(uid) if premium else "Not active",
        "trial_available":trial_available(uid) if uid in users else False,
        "downloads":downloads,
        "language":lang_of(uid),
        "crypto_open":crypto_system_open(),
        "crypto_allowed":allowed,
        "crypto_remaining":remaining,
        "crypto_minimum":minimum,
        "deposited":deposited,
        "balance_locked":locked,
        "youtube_special":bool(u.get("youtube_30m")),
        "referrals":int(u.get("invited",0) or 0),
        "city":u.get("city") or "N/A",
        "gender":u.get("gender") or "N/A",
    }


def _ai_system_context():
    """Current configuration/status, suitable for explanations and admin AI."""
    return {
        "crypto": "OPEN" if crypto_system_open() else "CLOSED",
        "crypto_hold": "OPEN" if crypto_hold_open() else "CLOSED",
        "crypto_min_deposit": crypto_min_deposit_usd(),
        "crypto_fee_percent": crypto_fee_percent(),
        ...
        "free_mb": int(get_setting("free_max_mb",49) or 49),
        "premium_mb": int(get_setting("premium_max_mb",49) or 49),
        "rapidapi_youtube": bool(RAPIDAPI_YT_KEY),
        "cobalt": bool(COBALT_API_URL),
        "d7_sms": bool(D7_TOKEN),
        "waforge_whatsapp": bool(WAFORGE_API_KEY),
        "resend_email": bool(RESEND_API_KEY),
        "force_join": bool(CHANNEL_WINDOW_OPEN),
        "users": len(users),
...
def _ai_find_user(query):
    ...
    if found:
        return found
    for uid,data in users.items():
        if str(data.get("username","")).lstrip("@").lower()==q:
            return uid
    return None


def _ai_user_report(uid, admin=False):
    c=_ai_user_context(uid); u=users.get(str(uid),{})
    ...
    crypto=("✅ Allowed" if c["crypto_allowed"] else f"⏳ ${c['crypto_remaining']:.2f} remaining") if c["crypto_open"] else "🔴 Crypto closed"
    lines=[
        "🤖 <b>CUSTOMER AI — ACCOUNT REPORT</b>",
        "",
        f"👤 Name: <b>{html.escape(c['name'])}</b>",
        f"🔗 Username: <b>{html.escape(username)}</b>",
        f"🆔 Telegram ID: <code>{c['uid']}</code>",
        f"🤖 Bot ID: <code>{html.escape(str(c['bot_id']))}</code>",
        f"🌍 Language: <b>{html.escape(c['language'])}</b>",
        f"🔐 Verification: <b>{verify}</b>",
        f"💎 Premium: <b>{premium}</b>" + (f" until <b>{html.escape(c['premium_until'])}</b>" if c['premium'] else ""),
        f"💰 Balance: <b>{html.escape(format_asset(c['asset'],c['balance']))}</b>",
        f"💵 Balance USD value: <b>${c['balance_usd']:,.2f}</b>",
        f"🔒 Blocked: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>",
        f"📊 Portfolio: <b>${c['portfolio_usd']:,.2f} USD</b>",
        f"💼 Total assets: <b>${c['total_assets_usd']:,.2f} USD</b>",
        f"🔐 Balance status: <b>{status}</b>",
        f"🪙 Crypto: <b>{crypto}</b>",
        f"💳 Account credited/deposited: <b>${c['deposited']:,.2f} USD</b>",
        f"📥 Downloads: <b>{c['downloads']}</b>",
        f"👥 Referrals: <b>{c['referrals']}</b>",
    ]
    if admin:
        lines += [f"📧 Email: <code>{html.escape(str(c['email']))}</code>",f"📱 Phone: <code>{html.escape(str(c['phone']))}</code>",f"🏙 City: <b>{html.escape(str(c['city']))}</b>"]
    return "\n".join(lines)


def _ai_help_text():
    return ("🤖 <b>CUSTOMER AI</b>\n\n"
            "Waxaad si dabiici ah ii weydiin kartaa wax kasta oo ku saabsan bot-ka. Tusaale:\n\n"
            "• <b>Haraagaygu waa imisa?</b>\n"
            "• <b>Maxaa lacagta loo block gareeyay?</b>\n"
            "• <b>Premium maxaan ka helayaa?</b>\n"
            "• <b>Sidee WhatsApp OTP u shaqeeyaa?</b>\n"
            "• <b>Crypto maxaa ii xiran?</b>\n"
            "• <b>Maxaa YouTube u shaqayn waayay?</b>\n"
            "• <b>Sidee MP3 looga sameeyaa?</b>\n"
            "• <b>Withdrawal-kaygu xaggee marayaa?</b>\n"
            "• <b>Maxay tahay 1-hour hold?</b>\n\n"
            "Waxaan akhriyaa xogta account-kaaga marka ay suurtagal tahay, mana sameeyo xog aan jirin.")


def _ai_intent_answer(uid, q, admin=False):
    """High-coverage deterministic AI: intent + live account/system context."""
    ...
    if any(x in q for x in ["balance", "haraag", "haraaga", "lacag intee", "my money", "how much money"]):
        if c["balance_locked"]:
            return _ai_user_report(uid,admin=False) + "\n\n🔒 <b>Fiiro gaar ah:</b> Balance-ka hadda lama isticmaali karo sababtoo ah waxaa saaran Balance Lock."
        return (f"💰 <b>Haraagaaga hadda</b>\n\n"
                f"Available: <b>{html.escape(format_asset(c['asset'],c['balance']))}</b>\n"
                f"USD value: <b>${c['balance_usd']:,.2f}</b>\n"
                f"Blocked: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>\n"
                f"Portfolio: <b>${c['portfolio_usd']:,.2f} USD</b>\n"
                f"Total assets: <b>${c['total_assets_usd']:,.2f} USD</b>")
    if any(x in q for x in ["blocked", "block", "xiran", "locked", "hold", "lacagta la qabtay"]):
        return (f"🔒 <b>Blocked / Hold</b>\n\nBlocked amount: <b>{html.escape(format_asset(c['asset'],c['blocked']))}</b>\n"
                f"Balance lock: <b>{'YES' if c['balance_locked'] else 'NO'}</b>\n\n"
                "Withdrawal pending lacagta waxay geli kartaa Blocked Amount. Crypto conversion-na haddii Hold uu furan yahay wuxuu yeelan karaa 1-hour hold. "
                "Balance Lock-na waa maamulka gaar ah oo admin sameeyo.")
    if any(x in q for x in ["premium", "vip", "trial", "tijaabo"]):
        return (f"💎 <b>Premium</b>\n\nStatus: <b>{'ACTIVE' if c['premium'] else 'NOT ACTIVE'}</b>\n"
                f"Expiry: <b>{html.escape(c['premium_until'])}</b>\n\n"
                "Premium wuxuu bixiyaa quality sare, priority/high-concurrency downloads, full YouTube access, platforms dheeraad ah iyo download speed aan artificial ahaan loo xadidin. "
                f"Premium verification: <b>{'REQUIRED' if premium_verification_required() else 'NOT REQUIRED'}</b>.")
    if any(x in q for x in ["whatsapp", "waforge"]):
        return ("📱 <b>WhatsApp Verification</b>\n\n"
                f"Status: <b>{'OPEN' if bool(get_setting('whatsapp_verify_enabled',False)) and bool(WAFORGE_API_KEY) else 'CLOSED'}</b>\n"
                "WhatsApp OTP waxaa si gaar ah u dira <b>WaForge</b>; D7 looma isticmaalo WhatsApp. Code-ku wuxuu leeyahay 5 daqiiqo expiry iyo resend cooldown. "
                "Haddii aad hesho 'expired', request cusub bilow.")
    if any(x in q for x in ["sms", "d7", "phone verification"]):
        return ("📲 <b>SMS Verification</b>\n\n"
                f"Status: <b>{'OPEN' if bool(get_setting('sms_enabled',False)) else 'CLOSED'}</b>\n"
                "SMS OTP waxaa dira D7 Networks. Code-ku waa 6-digit waxaana local ahaan loo hayaa 10 daqiiqo. WhatsApp iyo SMS waa laba nidaam oo kala duwan.")
    if any(x in q for x in ["gmail", "email verification", "email code"]):
        return ("📧 <b>Gmail Verification</b>\n\n"
                "Bot-ku wuxuu Gmail OTP ku diraa email provider-ka la configured-gareeyay. Code-ku waa 6-digit, local session-kuna wuxuu dhacaa 10 daqiiqo kadib. Resend-ku timer-ka wuu cusboonaysiiyaa.")
    if any(x in q for x in ["verify", "verification", "otp", "code", "xaqiijin"]):
        return ("🔐 <b>Verification</b>\n\n"
                f"Your status: <b>{'Verified' if c['verified'] else 'Not Verified'}</b>\n"
                "Hababka la heli karo waxay ku xiran yihiin Admin settings: Gmail, D7 SMS, iyo WaForge WhatsApp. Marka hal hab si sax ah loo xaqiijiyo, account-ka waxaa loo aqoonsanayaa verified Premium/trial checks.")
    if any(x in q for x in ["crypto", "bitcoin", "btc", "ethereum", "eth", "usdt", "coin"]):
        if not c["crypto_open"]:
            return "🪙 <b>Crypto</b>\n\n🔴 Crypto system-ka hadda Admin ayaa xiray. Marka la xiro, crypto holdings waxaa lagu liquidate-gareeyaa USD, waxaana la ilaaliyaa USD cash-ka hore u jiray."
        return (f"🪙 <b>Crypto</b>\n\nStatus: <b>OPEN</b>\n"
                f"Your credited/deposited total: <b>${c['deposited']:,.2f}</b>\n"
                f"Minimum: <b>${c['crypto_minimum']:,.2f}</b>\n"
                f"Access: <b>{'ALLOWED' if c['crypto_allowed'] else 'LOCKED'}</b>\n"
                f"Remaining: <b>${c['crypto_remaining']:,.2f}</b>\n\n"
                "Crypto conversion waxay leedahay platform fee iyo haddii asset-ku crypto yahay gas estimate/reserve. Conversion-ka gudaha bot-ka ma sameeyo blockchain broadcast toos ah.")
    if any(x in q for x in ["youtube", "yt", "shorts"]):
        return ("▶️ <b>YouTube</b>\n\n"
                "YouTube videos iyo Shorts waa Premium-only (marka laga reebo Admin ama user special access leh). Downloader-ku wuxuu isku dayaa RapidAPI marka key la hayo, kadib configured fallbacks/Cobalt iyo ugu dambayn yt-dlp. "
                "Haddii video gaar ahi diido, YouTube restriction/cookies/provider ayaa sababi kara.")
    if any(x in q for x in ["download", "video", "tiktok", "instagram", "facebook", "pinterest", "snapchat", "twitter", "x.com"]):
        return ("📥 <b>Downloads</b>\n\n"
                "Send a supported public link. Bot-ku wuxuu sameeyaa provider fallback marka adeegga koowaad diido. Telegram Chat Actions waxay muujinayaan processing/upload status. "
                f"Free limit: <b>{int(get_setting('free_max_minutes',FREE_MAX_MINUTES_DEFAULT) or FREE_MAX_MINUTES_DEFAULT)} min</b>; Premium limit: <b>{int(get_setting('premium_max_minutes',PREMIUM_MAX_MINUTES_DEFAULT) or PREMIUM_MAX_MINUTES_DEFAULT)} min</b>. "
                "File-size limit-na Admin ayaa maamula.")
    if any(x in q for x in ["music", "mp3", "song", "hees"]):
        return ("🎵 <b>MUSIC / MP3</b>\n\n"
                "Video la soo dejiyey hoostiisa taabo 🎵 MUSIC. Bot-ku wuxuu isticmaalaa audio extraction + FFmpeg si MP3 loo sameeyo. Title/artist metadata haddii la helo waa la muujin karaa; haddii artist-ka maqan yahay waxaa loo sheegi karaa Unknown artist.")
    if any(x in q for x in ["withdraw", "withdrawal", "bixid", "bixi", "cash out"]):
        return ("💸 <b>Withdrawal</b>\n\n"
                "Dooro withdrawal method-ka la taageero, geli address-ka iyo amount-ka. Lacagta pending-ka ah waxaa loo tixgelin karaa Blocked Amount ilaa Admin ka xaqiijiyo. Haddii Balance Lock saaran yahay, withdrawal lama bilaabi karo.")
    if any(x in q for x in ["referral", "refer", "casuum", "invite"]):
        return (f"👥 <b>Referral</b>\n\nInvited users: <b>{c['referrals']}</b>.\nReferral reward-ka hadda waa <b>${referral_reward_amount():.2f}</b> qofkiiba, haddii system-ku sidaas u configured yahay."
                " Network commission waa nidaam gaar ah oo admin settings ku xiran.")
    if any(x in q for x in ["profile", "my id", "telegram id", "bot id", "username"]):
        return _ai_user_report(uid,admin=False)
    if any(x in q for x in ["language", "luqad", "afaan"]):
        return f"🌍 <b>Language</b>\n\nLuqadda account-kaaga hadda: <b>{html.escape(c['language'])}</b>. Waxaad ka beddeli kartaa CHANGE LANGUAGE menu-ga."
    if any(x in q for x in ["force join", "channel", "join channel"]):
        return (f"📢 <b>Force Join</b>\n\nSystem status: <b>{'OPEN' if CHANNEL_WINDOW_OPEN else 'CLOSED'}</b>. "
                "Haddii channels la maamulo, user-ka waa inuu ku biiro channels-ka loo baahan yahay kadibna CONFIRM sameeyo.")
    if any(x in q for x in ["fee", "gas", "commission"]):
        return (f"💸 <b>Fees</b>\n\nCrypto platform fee: <b>{crypto_fee_percent():.2f}%</b>. "
                "Gas-ka crypto waxaa lagu qiyaasi karaa live network data marka la heli karo ama admin reserve ayaa fallback ahaan loo isticmaalaa."
                " Internal conversion-ku ma diraayo transaction blockchain ah.")
    if any(x in q for x in ["help", "caawi", "sidee", "what can you do", "maxaad qaban"]):
        return _ai_help_text()

    if admin and any(x in q for x in ["system status", "system", "dashboard", "settings", "xaaladda bot"]):
        sc=_ai_system_context()
        return ("🧠 <b>ADMIN AI — SYSTEM DASHBOARD</b>\n\n"
                ...
                f"🪙 Crypto: <b>{sc['crypto']}</b>\n⏳ Crypto Hold: <b>{sc['crypto_hold']}</b>\n"
                f"💰 Crypto minimum: <b>${sc['crypto_min_deposit']:.2f}</b>\n💸 Crypto fee: <b>{sc['crypto_fee_percent']:.2f}%</b>\n"
                f"📲 SMS: <b>{sc['sms']}</b> | WhatsApp: <b>{sc['whatsapp']}</b> | Verify: <b>{sc['verify']}</b>\n"
                f"▶️ RapidAPI YouTube: <b>{'READY' if sc['rapidapi_youtube'] else 'NOT SET'}</b> | Cobalt: <b>{'READY' if sc['cobalt'] else 'NOT SET'}</b>\n"
                f"📧 Email API: <b>{'READY' if sc['resend_email'] else 'NOT SET'}</b>\n"
                f"📢 Force Join: <b>{'OPEN' if sc['force_join'] else 'CLOSED'}</b>")

    # Admin can ask for a specific user using an ID/Bot ID/@username in the message.
    if admin:
        candidates=re.findall(r"(?:@?[A-Za-z0-9_]{5,32}|\d{6,20})", q)
        for cand in candidates:
            found=_ai_find_user(cand)
            if found:
                return _ai_user_report(found,admin=True)

    return ("🤖 <b>Waan fahmay su'aashaada, laakiin jawaab sax ah ayaan rabaa inaan ku siiyo.</b>\n\n"
            "Waxaan ka jawaabi karaa Balance, Blocked/Hold, Premium, Trial, Verification, WhatsApp/WaForge, SMS/D7, Gmail, Crypto, Fees/Gas, YouTube, Downloads, Music/MP3, Withdrawal, Referral, Profile, Force Join iyo system status.\n\n"
            "Tusaale qor: <b>‘Maxaa crypto ii xiran?’</b> ama <b>‘Haraagaygu waa imisa?’</b>.")
...
def customer_ai_handler(m):
    ...
    if bot_locked_guard(m) or banned_guard(m): return
    if is_admin(m.from_user.id):
        username=str(get_setting("customer_ai_username","Aidownloaderbot") or "Aidownloaderbot").lstrip("@")
        msg=bot.send_message(m.chat.id,
            "🤖 <b>CUSTOMER AI ADMIN</b>\n\n"
            f"AI Bot: <b>@{html.escape(username)}</b>\n\n"
            "Dedicated Customer AI bot-ka wuxuu leeyahay account-aware support iyo admin system dashboard.\n"
            "Token-ka bot-kan waa <code>CUSTOMER_AI_BOT_TOKEN</code> gudaha Railway Variables.")
        return
    username=str(get_setting("customer_ai_username","Aidownloaderbot") or "Aidownloaderbot").lstrip("@")
    if CUSTOMER_AI_BOT_TOKEN:
        kb=InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🤖 OPEN CUSTOMER AI",url=f"https://t.me/{username}"))
        bot.send_message(m.chat.id,
            "🤖 <b>CUSTOMER AI</b>\n\n"
            "Waxaan hayaa xogta account-kaaga, waxaana kuu sharxi karaa system-ka oo dhan: Balance, Premium, Verification, WhatsApp/SMS, Crypto, YouTube, Downloads, MP3, Withdrawal, Referral iyo wax kasta oo bot-ka ku saabsan.\n\n"
            "Taabo button-ka hoose oo su'aashaada si dabiici ah u qor.",reply_markup=kb)
    else:
        bot.send_message(m.chat.id,
            "⚠️ <b>Customer AI Bot lama shidin.</b>\n\n"
            "Admin-ku waa inuu Railway Variables ku daraa <code>CUSTOMER_AI_BOT_TOKEN</code> oo ah token-ka BotFather ee Customer AI bot-ka.")
...
if customer_ai_bot:
    ...
    def customer_ai_start(message):
        ...
        if uid not in users:
            users[uid]={"username":message.from_user.username or "","first_name":message.from_user.first_name or "there",
                        "balance":0.0,"blocked":0.0,"ref":random_ref(),"bot_id":random_botid(),"invited":0,"banned":False,
                        "verified":False,"quick_access":False,"youtube_30m":False,"premium_until":None,"premium_warning_sent":False,
                        "trial_used":False,"trial_pending":False,"trial_version_used":None,"trial_pending_version":None,
                        "referral_50_rewarded":False,"referred_by":None,"joined_date":datetime.now().strftime("%Y-%m-%d"),
                        "last_seen_at":datetime.now(timezone.utc).isoformat(),"month":now_month(),"language":"so","currency":"USD",
                        "balance_asset":"USD","gender":None,"city":None}
            save_user(uid)
        else:
            users[uid]["username"]=message.from_user.username or users[uid].get("username","")
            users[uid]["first_name"]=message.from_user.first_name or users[uid].get("first_name","there")
            touch_user(uid)
        if users[uid].get("banned"):
            customer_ai_bot.send_message(message.chat.id,"🚫 Your account is banned from this service."); return
        kb=ReplyKeyboardMarkup(resize_keyboard=True)
        kb.row("💰 MY BALANCE","👤 MY PROFILE")
        kb.row("💎 PREMIUM","🔐 VERIFICATION")
        kb.row("🪙 CRYPTO","📥 DOWNLOADS")
        kb.row("💸 WITHDRAWAL","📚 ALL SYSTEM")
        customer_ai_bot.send_message(message.chat.id,
            "🤖 <b>WELCOME TO CUSTOMER AI</b>\n\n"
            "Waxaan ahay Customer AI-ga system-kan. Waxaan ku fahmi karaa Somali ama English, waxaanan kuu sharxi karaa system-ka oo dhan anigoo eegaya account-kaaga live marka xogtu jirto.\n\n"
            "Qor su'aashaada ama isticmaal button-ka hoose.",reply_markup=kb)

    @customer_ai_bot.message_handler(commands=['help'])
    def customer_ai_help(message):
        customer_ai_bot.send_message(message.chat.id,_ai_help_text())

    @customer_ai_bot.message_handler(func=lambda m: m.text in {"💰 MY BALANCE","👤 MY PROFILE","💎 PREMIUM","🔐 VERIFICATION","🪙 CRYPTO","📥 DOWNLOADS","💸 WITHDRAWAL","📚 ALL SYSTEM"})
    def customer_ai_menu(message):
        uid=str(message.from_user.id); touch_user(uid)
        mapping={
            "💰 MY BALANCE":"balance haraaga my money",
            "👤 MY PROFILE":"my profile telegram id bot id username",
            "💎 PREMIUM":"premium trial",
            "🔐 VERIFICATION":"verification otp",
            "🪙 CRYPTO":"crypto btc usdt",
            "📥 DOWNLOADS":"download video tiktok youtube instagram",
            "💸 WITHDRAWAL":"withdrawal bixid",
            "📚 ALL SYSTEM":"help what can you do"
        }
        customer_ai_bot.send_chat_action(message.chat.id,"typing")
        customer_ai_bot.send_message(message.chat.id,_ai_intent_answer(uid,mapping[message.text],is_admin(uid)))

    @customer_ai_bot.message_handler(commands=['system'])
    def customer_ai_system(message):
        uid=str(message.from_user.id)
        if not is_admin(uid):
            customer_ai_bot.send_message(message.chat.id,"ℹ️ System dashboard is available to admins only. I can still explain all user-facing features to you."); return
        sc=_ai_system_context()
        customer_ai_bot.send_message(message.chat.id,_ai_intent_answer(uid,"system status",True))
    ...
    def customer_ai_user(message):
        ...
        customer_ai_bot.send_message(message.chat.id,_ai_user_report(found,True) if found else "❌ User not found.")
    ...
    def customer_ai_message(message):
        ...
        touch_user(uid)
        customer_ai_bot.send_chat_action(message.chat.id,"typing")
        answer=_ai_intent_answer(uid,message.text or "",is_admin(uid))
...
def premium_features_text():
    ...
    return (
        ...
        "▶️ Full YouTube videos (Premium only)\n"
...
def handle_links(message):
    ...
    platform = detect_platform(link)
    # ALL YouTube videos/Shorts are Premium-only. Admin and explicitly granted special YouTube users bypass this gate.
    if platform == "youtube" and not is_premium(uid) and not is_admin(uid) and not users.get(uid,{}).get("youtube_30m",False):
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("💎 OPEN PREMIUM", callback_data="premium_menu"))
        bot.send_message(message.chat.id, "❌ <b>YouTube is Premium-only.</b>\n\n💎 Open Premium to unlock YouTube downloads, higher quality, faster priority and extra platforms.", reply_markup=kb); return
    ...
    for uid,u in list(users.items()):
        try:
            if balance_is_locked(uid):
                ...
                av=available_asset_amount(uid); bl=blocked_amount(uid); text=f"💰 <b>Available Balance:</b> {format_asset(cur_code(uid),av)}\n🔒 <b>Blocked Amount:</b> {format_asset(cur_code(uid),bl)}\n\n{html.escape(custom)}"
...
def admin_see_all_balance(m):
    ...
    for i,(uid,u) in enumerate(users.items(),1):
        ...
        av=available_asset_amount(uid); blocked=blocked_amount(uid)
        port=portfolio_usd_value(uid)
        lines.append(f"{i}. {username} | <code>{uid}</code> | 🤖 {u.get('bot_id','N/A')}\n   💰 {format_asset(cur_code(uid),av)} | 🔒 {format_asset(cur_code(uid),blocked)} | 💼 ${port:,.2f} | {locked}")
...
def open_premium_start(m):
    if not is_admin(m.from_user.id): return
    msg=bot.send_message(m.chat.id, "Send User ID or BOT ID and number of months.\nExample: 123456789 6")
    bot.register_next_step_handler(msg, open_premium_process)

def open_premium_process(m):
    if not is_admin(m.from_user.id): return
    try:
        target, months_s=(m.text or '').split()[:2]
        months=int(months_s)
        if months<=0 or months>120: raise ValueError
        uid=resolve_user_input(target)
        if not uid: bot.send_message(m.chat.id,"❌ User not found."); return
        now=datetime.now(timezone.utc)
        old=users[uid].get('premium_until')
        try:
        ...
        bot.send_message(int(uid),f"🎉 <b>Downloader Bot Premium opened by Admin</b>\n\n⏰ Expires: <b>{local_datetime_text(uid, until)}</b>\n📥 Platforms: <b>{len(premium_platform_names())}</b>\n⚡ No artificial speed throttle\n▶️ Full YouTube is unlocked.")
        send_premium_email(uid,'Downloader Bot Premium opened by Admin',months,0,until)
    except Exception:
        bot.send_message(m.chat.id,"❌ Format error. Example: 123456789 6")
