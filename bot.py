...
# Dedicated managed-bot Creator. This bot must have Telegram's Bot Management Mode
# enabled in the @BotFather Mini App. Users create their own managed downloader bots
# from Telegram's native creation UI; no BotFather token is typed by the user.
...
FREE_MAX_MINUTES_DEFAULT = int(os.getenv("FREE_MAX_MINUTES", "12"))
...
FREE_YOUTUBE_MAX_MB_DEFAULT = int(os.getenv("FREE_YOUTUBE_MAX_MB", "0"))
...
# A small router lets the existing downloader code serve both the main bot and
# user-created downloader bots without duplicating the large download pipeline.
# Normal handlers use the main bot; managed-bot workers set the context variable
# before calling shared download functions.
...
class _BotRouter:
    def __getattr__(self, name):
        active = _ACTIVE_BOT.get()
        target = active or _main_bot
...
MANAGED_POWERED_BY = "Powered by: @Downloadvedioytibot"
...
creator_sessions = {}
managed_wallet_pending = {}

def _creation_open():
    return bool(get_setting("bot_creation_enabled", True))

def _create_button_open():
    # Separate switch: existing downloader bots continue working even when
    # creation is closed, while this switch controls only the public
    # "Create Your Own Bot" button shown inside those bots.
    return bool(get_setting("managed_create_button_enabled", True))
...
def _managed_premium_active(bot_id=None, owner_id=None):
    """Premium belongs to the specific created Downloader Bot, not the main account."""
    bid=str(bot_id or "").strip()
    if not bid:
        return False
    d=managed_bots_col.find_one({"bot_id":bid},{"premium_until":1}) or {}
...
def _managed_premium_until(doc):
...
def _active_powered_text():
    # Premium removes the Powered-by advertisement from the specific managed bot.
    meta=_ACTIVE_MANAGED_META.get() or {}
    if (not _managed_powered_by_open()) or _managed_premium_active(meta.get("bot_id"),meta.get("owner_id")):
        return ""
...
def send_html_email(to_email, subject, html_body, text_body=None):
    """Send transactional email through Resend with both HTML and plain text.

    The plain-text part improves compatibility with mailbox filters and clients;
    the code still cannot force Gmail to place a message in Inbox when DNS/domain
    reputation or authentication is not healthy.
    """
...
def user_menu(show_admin=False):
    ...
    if _song_search_is_open():
        kb.add("🔎 Search Song")
    if _creation_open() and _create_button_open():
...
def admin_menu():
    ...
    kb.add("🟢 OPEN CREATE BUTTON", "🔴 CLOSE CREATE BUTTON")
    ...
    kb.add("💳 WALLET REVOKE FEE")
    kb.add("✏️ SET CREATE CAPTION")
    kb.add("🤖 See All Bots", "📢 Broadcast All Bots")
...
def profile_handler(m):
    ...
    # The admin badge is global: one custom Telegram emoji/badge is shown for
    # every verified user.  We keep the old per-user sticker field for
    # backwards compatibility, but the profile now reads the canonical
    # MongoDB setting so the same badge is shown to everyone.
    badge = get_setting("admin_profile_badge", {}) or {}
    badge_id = str(badge.get("custom_emoji_id") or "").strip()
    badge_alt = str(badge.get("alt") or badge.get("text") or "💠").strip() or "💠"
    if verified:
        if badge_id:
            status_str = f"Verified<tg-emoji emoji-id=\"{html.escape(badge_id)}\">{html.escape(badge_alt)}</tg-emoji>"
        else:
            status_str = f"Verified{html.escape(badge_alt)}"
    else:
...
def verified_users_list(m):
    ...
    for uid in verified_list[:30]:
        ...
        badge = get_setting("admin_profile_badge", {}) or {}
        badge_id = str(badge.get("custom_emoji_id") or "").strip()
        badge_alt = str(badge.get("alt") or badge.get("text") or u_data.get("sticker", "N/A"))
        sticker = (f'<tg-emoji emoji-id="{html.escape(badge_id)}">{html.escape(badge_alt)}</tg-emoji>' if badge_id else html.escape(badge_alt))
        email = u_data.get('email', '')
        phone = u_data.get('phone', '')
...
def sticker_admin_start(m):
    """Set one global admin badge for every verified user's profile.

    The admin no longer enters a Telegram ID.  They simply send the custom
    emoji (optionally followed by text).  Telegram supplies the real
    custom_emoji_id in the message entity; that ID is persisted in MongoDB
    and reused in every profile message.
    """
    if not is_admin(m.from_user.id):
        return
    try:
        msg = bot.send_message(
            m.chat.id,
            "🏷️ <b>SET GLOBAL PROFILE BADGE</b>\n\n"
            "Send the exact Telegram Premium/custom emoji you want to use. "
            "You do <b>not</b> need to send a User ID.\n\n"
            "Example: send the custom emoji alone, or send it followed by "
            "<code>Verified</code>.\n\n"
            "The bot will save the real Telegram <b>custom emoji ID</b> and "
            "show the same badge on profiles of all verified users.",
            parse_mode="HTML"
        )
        bot.register_next_step_handler(msg, sticker_admin_process)
    except Exception as e:
        print("Sticker admin start error:", e)

def _extract_custom_emoji_from_message(m):
    """Return (custom_emoji_id, alt, label) from a Telegram message."""
    ...
    for ent in entities:
        ...
        try:
            ...
            # Telegram entity offsets/lengths are UTF-16 units.  For the
            # normal one-codepoint emoji this is equivalent to slicing; use
            # the first visible character as a safe fallback if needed.
            alt = text[offset:offset+length] if text else "💠"
        except Exception:
            alt = "💠"
        alt = alt or "💠"
        label = text
        try:
            label = (text[:offset] + text[offset+length:]).strip(" |-|–—")
        except Exception:
            pass
        return str(eid), alt, label
    return "", "", text.strip()
...
def sticker_admin_process(m):
    if not is_admin(m.from_user.id):
        return
    try:
        emoji_id, alt, label = _extract_custom_emoji_from_message(m)
        raw_text = str(getattr(m, "text", "") or "").strip()

        if not emoji_id:
            # Keep a simple-emoji fallback, but never ask for a user ID.
            # A custom Telegram emoji is preferred because it preserves the
            # exact Premium emoji ID.
            if not raw_text:
                bot.send_message(m.chat.id, "❌ Please send the Telegram custom emoji/badge.")
                return
            set_setting("admin_profile_badge", {
                "custom_emoji_id": "",
                "alt": raw_text.split()[0] if raw_text.split() else "💠",
                "text": raw_text,
                "updated_at": datetime.now(timezone.utc).isoformat(),
            })
            # Keep legacy per-user fields synchronized.
            for uid in list(users):
                users[uid]["sticker"] = raw_text
                save_user(uid)
            bot.send_message(
                m.chat.id,
                "✅ <b>Global profile badge updated.</b>\n\n"
                "It is now used for all verified users.\n"
                "⚠️ This message did not contain a Telegram custom-emoji ID, "
                "so the text emoji was saved as a fallback.",
                parse_mode="HTML"
            )
            return
        ...
        # Synchronize the legacy per-user field without requiring a user ID.
        for uid in list(users):
            users[uid]["sticker"] = label or alt
            users[uid]["sticker_custom_emoji_id"] = emoji_id
            users[uid]["sticker_custom_emoji_alt"] = alt
            save_user(uid)

        badge_html = f'<tg-emoji emoji-id="{html.escape(emoji_id)}">{html.escape(alt)}</tg-emoji>'
        bot.send_message(
            m.chat.id,
            "✅ <b>Global profile badge saved.</b>\n\n"
            f"Badge: {badge_html}\n"
            f"Custom Emoji ID: <code>{html.escape(emoji_id)}</code>\n\n"
            "👥 The same badge is now used for <b>all verified users</b>. "
            "No Telegram ID is required.",
            parse_mode="HTML"
        )
    except Exception as e:
        print("Sticker admin process error:", repr(e))
        try:
            bot.send_message(m.chat.id, "❌ Could not save the profile badge. Please send the custom emoji again.")
        except Exception:
            pass
    ...
    for params in variants:
        for attempt_no in range(1,4):
            try:
                ...
                # Retry transient provider/server errors; move to the next
                # response variant after the retry budget is exhausted.
                if r.status_code >= 500 and attempt_no < 3:
                    time.sleep(0.8 * attempt_no)
                    continue
                break
                ...
                if attempt_no < 3:
                    ...
                    continue
                break
        if last_error and any(x in str(last_error).lower() for x in ("quota","authentication","subscription","not found")):
            break
...
def _ensure_download_limit_defaults_v2():
    """Migrate old download defaults to the current policy once.
    Admin-selected non-default values are left untouched.
    """
    if get_setting("download_limits_defaults_v3",False): return
    try:
        ...
        old_free_min=get_setting("free_max_minutes",None)
        old_free_yt_mb=get_setting("free_youtube_max_mb",None)
        ...
        # YouTube policy: free users get up to 12 minutes; Shorts are free too.
        # Remove the old 49 MB YouTube ceiling so duration, not file size, is the
        # access rule. 0 means unlimited at the downloader layer.
        if old_free_min in (None, 10): set_setting("free_max_minutes",12)
        if old_free_yt_mb in (None, 49): set_setting("free_youtube_max_mb",0)
        set_setting("download_limits_defaults_v3",True)
    except Exception as e:
...
def _safe_send_file(chat_id, path, caption="", reply_markup=None, platform=None, link=None):
    ...
    with open(path, "rb") as f:
        if _is_image_file(path):
            sent = bot.send_photo(chat_id, f, caption=caption, reply_markup=reply_markup)
        elif _is_video_file(path):
            sent = bot.send_video(chat_id, f, caption=caption, supports_streaming=True, reply_markup=reply_markup)
        elif _is_audio_file(path):
            sent = bot.send_audio(chat_id, f, caption=caption, reply_markup=reply_markup)
        else:
            sent = bot.send_document(chat_id, f, caption=caption, reply_markup=reply_markup)
    if _ACTIVE_MANAGED_META.get():
        meta=_ACTIVE_MANAGED_META.get() or {}
        # Exactly one system Powered-by message follows each delivered media item.
        # Premium suppresses system promotional messages for this managed bot.
        if _managed_powered_by_open() and not _managed_premium_active(meta.get("bot_id"),meta.get("owner_id")):
            try:
                bot.send_message(chat_id, MANAGED_POWERED_BY, parse_mode="HTML")
            except Exception as e:
                print("Managed bot powered-by message failed:", repr(e))
    return sent
...
def download_media(chat_id, link, message_id, quality=None):
    ...
    try:
        ...
        # YouTube: try yt-dlp first with its full client/format retry ladder.
        # If YouTube blocks yt-dlp (for example, "Sign in to confirm you're not a bot"),
        # automatically switch to RapidAPI and make several provider/URL/format retries.
        # This keeps yt-dlp as the normal path while giving the bot a second independent
        # resolver when YouTube blocks the Railway server IP.
        ...
        # YouTube: use the configured RapidAPI resolver FIRST. This is the
        # primary path for Shorts/videos because Railway IPs can be challenged
        # by YouTube. yt-dlp remains a deep fallback, not the only resolver.
        if not media and platform == "youtube" and RAPIDAPI_YT_KEY:
            vid=_extract_youtube_video_id(link)
            rapid_links=[link]
            if vid:
                rapid_links += [f"https://www.youtube.com/watch?v={vid}", f"https://www.youtube.com/shorts/{vid}"]
            for rapid_link in dict.fromkeys(rapid_links):
                try:
                    media=_rapid_download(rapid_link,tmp,quality,max_seconds,chat_id=chat_id)
                    if media:
                        provider="rapidapi-youtube"
                        print("RapidAPI YouTube primary path succeeded")
                        break
                except Exception as rapid_exc:
                    print("RapidAPI YouTube primary attempt failed:",repr(rapid_exc))
                    media=[]
        ...
        if not media:
            ...
            try:
                ...
                print("yt-dlp exhausted all attempts:",repr(yt_exc))
                media=[]
                # YouTube-only emergency fallback: RapidAPI.  Never require a
                # RapidAPI call when yt-dlp already succeeded.
                if platform=="youtube" and RAPIDAPI_YT_KEY:
                    ...
                    seen_rapid=set()
                    rapid_errors=[]
                    for rapid_link in rapid_links:
                        if rapid_link in seen_rapid or media: continue
                        seen_rapid.add(rapid_link)
                        # _rapid_download itself tries multiple direct formats and
                        # retries video/audio downloads and FFmpeg merging.
                        try:
                            media=_rapid_download(rapid_link,tmp,quality,max_seconds,chat_id=chat_id)
                            if media:
                                provider="rapidapi-youtube"
                                print("RapidAPI YouTube fallback succeeded after yt-dlp failure")
                                break
                        except Exception as rapid_exc:
                            rapid_errors.append(repr(rapid_exc))
                            print("RapidAPI YouTube fallback attempt failed:",repr(rapid_exc))
                    if not media:
                        print("All YouTube fallback providers failed. yt-dlp:",repr(yt_exc),"RapidAPI:",rapid_errors[-5:])
                # Keep the original yt-dlp error visible if no provider produced media.
                if not media and platform!="youtube":
                    raise
        ...
        if not source_title and platform=="youtube" and provider=="rapidapi-youtube" and RAPIDAPI_YT_KEY:
            try:
                rapid_meta=_rapidapi_youtube_details(_extract_youtube_video_id(link))
                source_title=_music_clean_text(rapid_meta.get("title") or rapid_meta.get("name")) if isinstance(rapid_meta,dict) else ""
                source_artist=_music_clean_text(rapid_meta.get("artist") or rapid_meta.get("author") or rapid_meta.get("channelTitle")) if isinstance(rapid_meta,dict) else ""
            except Exception: pass
        sent=0
        for path in media[:50 if platform=="tiktok" else 20]:
            ...
            try:
                ...
                if _ACTIVE_MANAGED_META.get() and _create_caption_open() and not _managed_premium_active(_active_managed_meta.get("bot_id"),_active_managed_meta.get("owner_id")):
                    promo=_create_caption_text()
                    if promo:
                        try: bot.send_message(chat_id,promo,parse_mode="HTML",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Create Now",url=_creator_bot_url())]] if _creator_bot_url() else []))
                        except Exception: pass
            finally:
                upload_stop.set()
        ...
        meta=_ACTIVE_MANAGED_META.get() or {}
        managed_bid=str(meta.get("bot_id") or "")
        if managed_bid and sent:
            managed_bots_col.update_one({"bot_id":managed_bid},{"$inc":{"download_count":int(sent)},"$set":{"last_download_at":datetime.now(timezone.utc)}})
    except Exception as e:
...
def _jamendo_song_search(query, limit=100):
    """Fast Jamendo search for tracks that are explicitly downloadable.

    Requests are made in parallel so one slow/unreachable Jamendo endpoint cannot
    make the Telegram bot look frozen for minutes.
    """
    ...
    def call(extra):
        try:
            ...
            timeout=min(max(5,int(JAMENDO_TIMEOUT)),8)
            r=requests.get("https://api.jamendo.com/v3.0/tracks/",params=params,timeout=timeout)
            r.raise_for_status()
            body=r.json() or {}
    ...
    modes=[{"search":query},{"namesearch":query},{"artist_name":query}]
    words=[w for w in query.split() if len(w)>=2]
    if words:
        modes.append({"search":max(words,key=len)})

    raw=[]; seen_raw=set()
    with ThreadPoolExecutor(max_workers=min(4,len(modes))) as pool:
        futures=[pool.submit(call,m) for m in modes]
        for fut in futures:
            try:
                for item in fut.result(timeout=10) or []:
                    if not isinstance(item,dict): continue
                    iid=str(item.get("id") or "")
                    if iid and iid not in seen_raw:
                        seen_raw.add(iid); raw.append(item)
            except Exception as e:
                print("Jamendo parallel search failed:",repr(e))

    # One autocomplete pass only when the first search was sparse.
    if len(raw)<8:
        try:
            ac_params={"client_id":JAMENDO_CLIENT_ID,"format":"json","prefix":query,"limit":8,"matchcount":"true","entity":"artists tracks albums"}
            ar=requests.get("https://api.jamendo.com/v3.0/autocomplete/",params=ac_params,timeout=min(max(5,int(JAMENDO_TIMEOUT)),8))
            ...
            for group in ac.get("results") or []:
                if isinstance(group,dict):
                    ...
                    if name and str(name).casefold()!=query.casefold(): names.append(str(name))
            if names:
                extra=call({"search":names[0]})
                for item in extra:
                    if not isinstance(item,dict): continue
                    iid=str(item.get("id") or "")
                    if iid and iid not in seen_raw:
    ...
    for item in raw:
        ...
        # Jamendo explicitly requires audiodownload_allowed for application downloads.
...
def _rapidapi_youtube_search(query, limit=8):
    """Search YouTube through the configured RapidAPI provider first.

    This avoids making Search Song depend on yt-dlp's YouTube search extractor,
    which can be blocked independently of media downloading.
    """
    if not RAPIDAPI_YT_KEY:
        return []
    q=_music_clean_text(query)
    if not q:
        return []
    endpoint=RAPIDAPI_YT_SEARCH_URL
    headers={
        "x-rapidapi-key":RAPIDAPI_YT_KEY,
        "x-rapidapi-host":RAPIDAPI_YT_HOST,
        "Accept":"application/json",
    ...
    for attempt in range(1,4):
        try:
            r=requests.get(endpoint,params={"keyword":q,"lang":"en-US","sortBy":"relevance"},headers=headers,timeout=RAPIDAPI_TIMEOUT)
            body=r.json() if r.text else {}
            if r.status_code in (200,201) and isinstance(body,dict):
                items=[]
                for d in _rapid_walk(body):
                    if not isinstance(d,dict): continue
                    vid=str(d.get("id") or d.get("videoId") or d.get("videoID") or "")
                    if not re.fullmatch(r"[A-Za-z0-9_-]{11}",vid):
                        u=str(d.get("url") or d.get("webpageUrl") or d.get("webpage_url") or "")
                        vid=_extract_youtube_video_id(u) or ""
                    if not re.fullmatch(r"[A-Za-z0-9_-]{11}",vid): continue
                    title=_music_clean_text(d.get("title") or d.get("name") or d.get("videoTitle"))
                    artist=_music_clean_text(d.get("artist") or d.get("channelName") or d.get("channelTitle") or d.get("uploader") or d.get("author")) or "Unknown artist"
                    if not title: continue
                    if not _song_is_relevant(q,title,artist," ".join(str(d.get(k) or "") for k in ("description","album"))):
                        continue
                    try: dur=int(float(d.get("duration") or d.get("durationSeconds") or d.get("lengthSeconds") or 0))
                    except Exception: dur=0
                    url=f"https://www.youtube.com/watch?v={vid}"
                    items.append({"id":vid,"title":title,"artist":artist,"duration":dur,"album":"","cover":d.get("thumbnail") or d.get("thumbnailUrl"),"download":url,"download_allowed":True,"license":"","source":"youtube","webpage_url":url,"_score":_song_similarity(q,title,artist,"")})
                seen=set(); out=[]
                for x in sorted(items,key=lambda z:z.get("_score",0),reverse=True):
                    if x["id"] in seen: continue
                    seen.add(x["id"]); x.pop("_score",None); out.append(x)
                return out[:int(limit)]
            if r.status_code in (401,403,404,429):
                raise RuntimeError(f"RapidAPI search HTTP {r.status_code}")
            last=RuntimeError(f"RapidAPI search HTTP {r.status_code}")
        except Exception as e:
            last=e
        if attempt<3:
            time.sleep(0.7*attempt)
    print("RapidAPI YouTube search failed:",repr(last))
    return []

def _youtube_song_search(query, limit=8):
    """Search YouTube with RapidAPI first, then yt-dlp as fallback."""
    query=_music_clean_text(query)
    if not query: return []
    try:
        rapid=_rapidapi_youtube_search(query,limit)
        if rapid:
            return rapid[:int(limit)]
    except Exception as e:
        print("RapidAPI song search failed:",repr(e))
    try:
        variants=_song_query_variants(query,4)
        opts={"quiet":True,"no_warnings":True,"skip_download":True,"extract_flat":"in_playlist","noplaylist":True}
        rows=[]; seen=set()
        with ThreadPoolExecutor(max_workers=min(4,len(variants))) as pool:
            ...
            for fut in futures:
                try:
                    for item in fut.result(timeout=15) or []:
                        url=item.get("download")
                        if not url or url in seen: continue
                        seen.add(url); item["_score"]=_song_similarity(query,item.get("title",""),item.get("artist","")); rows.append(item)
                except Exception as e: print("YouTube song variant failed:",repr(e))
        rows.sort(key=lambda x:x.get("_score",0),reverse=True)
        for x in rows: x.pop("_score",None)
        return rows[:int(limit)]
    except Exception as e:
        print("YouTube song search failed:",repr(e)); return []

def _youtube_song_search_one(query, limit, opts):
    out=[]
    try:
        # Search may be rate-limited by YouTube. Reuse the same authenticated
        # cookie/extractor configuration as the actual downloader when present.
        search_opts=dict(opts or {})
        cookie_args={}
        if YTDLP_COOKIES_FILE and os.path.isfile(YTDLP_COOKIES_FILE):
            cookie_args["cookiefile"] = YTDLP_COOKIES_FILE
        if cookie_args:
            search_opts.update(cookie_args)
        extra=_youtube_extractor_args()
        if extra:
            search_opts["extractor_args"] = extra
        ...
        for item in ((info or {}).get("entries") or []):
            ...
            if not url and vid and len(vid)==11: url=f"https://www.youtube.com/watch?v={vid}"
            title=_music_clean_text(item.get("title") or item.get("fulltitle"))
            artist=_music_clean_text(item.get("artist") or item.get("uploader") or item.get("channel")) or "Unknown artist"
            if not url or not title: continue
            if not _song_is_relevant(query,title,artist,""): continue
            try: dur=int(float(item.get("duration") or 0))
            except Exception: dur=0
            out.append({"id":vid or hashlib.sha1(url.encode()).hexdigest()[:16],"title":title,"artist":artist,"duration":dur,"album":"","cover":item.get("thumbnail"),"download":url,"download_allowed":True,"license":"","source":"youtube","webpage_url":url})
    except Exception as e:
        print("YouTube song search one failed:",repr(e))
    return out


def _song_search_all(query, limit=12):
    """Return Jamendo downloadable tracks first, then YouTube metadata fallback."""
    jam=_jamendo_song_search(query,min(12,int(limit)))
    if len(jam)>=min(5,int(limit)):
        return jam[:int(limit)]
    yt=_youtube_song_search(query,max(4,int(limit)-len(jam)))
    rows=jam[:]
    seen={(str(x.get("source")),str(x.get("id"))) for x in rows}
    for x in yt:
        key=(str(x.get("source")),str(x.get("id")))
        if key not in seen:
            rows.append(x); seen.add(key)
    return rows[:int(limit)]
    ...
    if str(x.get("source")) == "youtube":
        def _download_youtube_song_job(chat_id,status_id,song,uid):
            try:
                ...
                try: bot.edit_message_text(f"❌ <b>Music download failed.</b>\n\n<code>{html.escape(str(e)[:500])}</code>",chat_id,status_id,parse_mode="HTML")
                except Exception: pass
...
def _send_mp3_file(chat_id, path, title, artist, cover_path=None, caption=None, reply_markup=None):
    ...
    with open(path,"rb") as audio:
        ...
        try:
            ...
            bot.send_document(chat_id,audio,caption=caption or (_active_managed_caption() if _ACTIVE_MANAGED_META.get() else DOWNLOAD_CAPTION),reply_markup=reply_markup,parse_mode="HTML")
...
def convert_link_to_mp3(chat_id, link, status_message_id, local_source=None, local_cache_dir=None, source_title="", source_artist="", pending_token=None):
    ...
    try:
        if status_message_id:
            try: bot.edit_message_text("⬇️ <b>Downloading song...</b>", chat_id, status_message_id, parse_mode="HTML")
            except Exception: pass
        rapid_mp3=None
        direct_source=None
        ...
        # Cobalt is a second provider for YouTube audio. This is especially
        # useful when YouTube blocks the server IP used by yt-dlp with
        # "Sign in to confirm you're not a bot". It is only used when the
        # owner has configured COBALT_API_URL.
        if not rapid_mp3 and source_platform == "youtube" and _cobalt_enabled():
            try:
                cpaths=_cobalt_process(link,tmp_dir,quality="max",mode="audio",max_mb=_download_max_mb(chat_id,platform="youtube",link=link))
                audio=next((p for p in cpaths if _is_audio_file(p)),None)
                if audio:
                    if os.path.splitext(audio)[1].lower()==".mp3":
                        rapid_mp3=audio
                    else:
                        rapid_mp3=os.path.join(tmp_dir,"cobalt_youtube_music.mp3")
                        _ffmpeg_convert_to_mp3(audio,rapid_mp3,bitrate="192k",timeout=300)
            except Exception as e:
                print("Cobalt YouTube MP3 fallback:",repr(e))
        ...
        friendly = "❌ <b>Music download failed.</b>\n\nPlease try the song again."
        detail=str(e)
        if (not local_source) and ("Sign in to confirm" in detail or "not a bot" in detail):
            friendly = "❌ <b>YouTube music is not available right now.</b>\n\nPlease try the song again later."
        try:
            if status_message_id:
                bot.edit_message_text(friendly + f"\n\n<code>{html.escape(detail[:500])}</code>", chat_id, status_message_id, parse_mode="HTML")
            else:
                bot.send_message(chat_id, friendly, parse_mode="HTML")
        except Exception:
            try: bot.send_message(chat_id, friendly, parse_mode="HTML")
            except Exception: pass
    finally:
        try: action_stop.set()
        except Exception: pass
...
def _creator_keyboard(uid):
    rows=[]
    if _creation_open() and _create_button_open(): rows.append([{"text":"🤖 Create My Bot"}])
    rows += [
...
def _creator_my_bots(uid, chat_id):
    ...
    for d in rows[:20]:
        ...
        premium="💎 Premium" if _managed_premium_active(str(d.get("bot_id") or ""),uid) else "🆓 Standard"
        _creator_send(chat_id,
...
def _generate_wallet_link_code():
    return "".join(secrets.choice(string.digits) for _ in range(16))

def _managed_wallet_code(doc):
    """Return the permanent wallet-link code for this bot, creating it once if needed."""
    enc=str((doc or {}).get("wallet_link_code_enc") or "").strip()
    if enc:
        try:
            code=_decrypt_managed_token({"token_enc":enc})
            if re.fullmatch(r"\d{16}",str(code or "")): return str(code)
        except Exception: pass
    code=_generate_wallet_link_code()
    managed_bots_col.update_one({"bot_id":str(doc.get("bot_id"))},{"$set":{"wallet_link_code_enc":_encrypt_managed_token(code)}},upsert=False)
    return code

def _managed_wallet_start(m, bid, user_id=None):
    uid=str(user_id if user_id is not None else m.from_user.id); d=_managed_bot_doc(bid)
    ...
    if d.get("wallet_linked"):
        fee=float(get_setting("managed_wallet_revoke_fee_usd",0.25) or 0.0)
        kb=InlineKeyboardMarkup(); kb.add(InlineKeyboardButton("♻️ Revoke Wallet",callback_data=f"mwalletrevoke:{bid}"))
        bot.send_message(m.chat.id,f"💳 <b>Wallet Linked</b>\n\n💰 Shared balance: <b>{html.escape(money_text(uid,balance_usd_value(uid)))}</b>\n\nThis bot uses the same balance as @Downloadvedioytibot.\nRevoke fee: <b>${fee:.2f}</b>.\n\n🔒 The wallet remains linked until you revoke it.",reply_markup=kb)
        return
    code=_managed_wallet_code(d)
    managed_wallet_pending[uid] = {"bot_id":str(bid),"created_at":time.time()}
    try:
        _main_bot.send_message(int(uid),
            "🔐 <b>YOUR PERMANENT WALLET LINK CODE</b>\n\n"
            f"<code>{code}</code>\n\n"
            "💳 This code links this Downloader Bot to your main @Downloadvedioytibot wallet.\n"
            "♾️ It does <b>not expire</b>. The link remains active until you choose <b>Revoke Wallet</b>.\n\n"
            "🚨 <b>DO NOT SHARE THIS CODE WITH ANYONE.</b>\n"
            "It is part of your account's wallet access. Keep it private.\n\n"
            "Return to your Downloader Bot and send the 16-digit code.",parse_mode="HTML")
    except Exception:
        bot.send_message(m.chat.id,"❌ I could not send the wallet code to @Downloadvedioytibot. Please open the main bot and try again."); return
    bot.send_message(m.chat.id,
        "💳 <b>Link Shared Wallet</b>\n\n"
        "I sent your permanent 16-digit wallet-link code to <b>@Downloadvedioytibot</b>.\n\n"
        "📩 Copy it from the main bot and send it here.\n"
        "♾️ The code does not expire; once linked, the wallet stays connected until you revoke it.\n\n"
        "⚠️ <b>Never share the code with another person.</b>")
...
def _managed_wallet_process(m,bid):
    ...
    expected=_managed_wallet_code(d)
    if not re.fullmatch(r"\d{16}",card) or card!=expected:
        bot.send_message(m.chat.id,"❌ Invalid wallet link code. Open @Downloadvedioytibot and copy the permanent 16-digit code for this bot."); return
    managed_wallet_pending.pop(uid,None)
    ...
    fee=float(get_setting("managed_wallet_revoke_fee_usd",0.25) or 0.0)
    kb=InlineKeyboardMarkup(); kb.add(InlineKeyboardButton("♻️ Revoke Wallet",callback_data=f"mwalletrevoke:{bid}"))
    bot.send_message(m.chat.id,f"✅ <b>Wallet Linked Successfully</b>\n\n💰 Shared balance: <b>{html.escape(money_text(uid,balance_usd_value(uid)))}</b>\n💳 Revoke fee: <b>${fee:.2f}</b>\n\nThis bot and @Downloadvedioytibot now use the same account balance.\n\n🔒 <b>The wallet stays linked until you revoke it.</b>",reply_markup=kb)

def _managed_wallet_revoke_callback(call,bid):
    uid=str(call.from_user.id); d=_managed_bot_doc(bid)
    if not d or str(d.get("owner_id"))!=uid:
        bot.answer_callback_query(call.id,"Only the bot owner can revoke this wallet.",show_alert=True); return
    if not d.get("wallet_linked"):
        bot.answer_callback_query(call.id,"Wallet is not linked.",show_alert=True); return
    fee=max(0.0,float(get_setting("managed_wallet_revoke_fee_usd",0.25) or 0.0))
    kb=InlineKeyboardMarkup(); kb.add(InlineKeyboardButton("✅ Confirm Revoke",callback_data=f"mwalletconfirm:{bid}"),InlineKeyboardButton("❌ Cancel",callback_data=f"mwalletcancel:{bid}"))
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id,f"♻️ <b>Revoke Shared Wallet?</b>\n\nFee: <b>${fee:.2f}</b>\nCurrent shared balance: <b>{html.escape(money_text(uid,balance_usd_value(uid)))}</b>\n\nThe wallet link will be removed from this bot. Your main balance itself is not deleted.",reply_markup=kb)

def _managed_wallet_revoke_confirm(call,bid):
    uid=str(call.from_user.id); d=_managed_bot_doc(bid)
    if not d or str(d.get("owner_id"))!=uid or not d.get("wallet_linked"):
        bot.answer_callback_query(call.id,"Wallet is already unlinked.",show_alert=True); return
    fee=max(0.0,float(get_setting("managed_wallet_revoke_fee_usd",0.25) or 0.0))
    if fee>0:
        asset=usd_to_asset(cur_code(uid),fee)
        if available_asset_amount(uid)<asset:
            bot.answer_callback_query(call.id,"Insufficient balance for the revoke fee.",show_alert=True); return
        ledger_debit(uid,asset,"managed_wallet_revoke_fee",{"bot_id":str(bid),"fee_usd":fee})
    managed_bots_col.update_one({"bot_id":str(bid)}, {"$set":{"wallet_linked":False,"wallet_owner_id":None,"wallet_revoked_at":datetime.now(timezone.utc)},"$unset":{"wallet_link_code_enc":""}})
    bot.answer_callback_query(call.id,"Wallet revoked.")
    bot.send_message(call.message.chat.id,f"✅ <b>Wallet Revoked</b>\n\nFee charged: <b>${fee:.2f}</b>\nYour @Downloadvedioytibot balance remains safe and unchanged except for the revoke fee.")

def _managed_wallet_revoke_cancel(call):
    bot.answer_callback_query(call.id,"Cancelled.")
    try: bot.delete_message(call.message.chat.id,call.message.message_id)
    except Exception: pass

def _main_create_stars_invoice_link(title,description,payload,stars):
    if not TOKEN: return None,"BOT_TOKEN is not configured"
    try:
        r=requests.post(f"https://api.telegram.org/bot{TOKEN}/createInvoiceLink",json={
            "title":title[:32],"description":description[:255],"payload":payload[:128],
            "currency":"XTR","prices":[{"label":title[:32],"amount":int(stars)}]
        },timeout=20)
        data=r.json()
        if data.get("ok"): return data.get("result"),None
        return None,str(data.get("description") or "createInvoiceLink failed")
    except Exception as e: return None,str(e)

def _managed_premium_menu(uid, chat_id, bid):
    uid=str(uid); bid=str(bid); d=_managed_bot_doc(bid)
    if not d or str(d.get("owner_id"))!=uid:
        bot.send_message(chat_id,"🔐 <b>Premium is owner-only.</b>\n\nOnly the owner of this Downloader Bot can activate Premium."); return
    until=_managed_premium_until(d)
    active=bool(until and until>datetime.now(timezone.utc))
    prices=get_premium_prices()
    kb=InlineKeyboardMarkup(row_width=1)
    for months in ("1","3","9","12"):
        kb.add(InlineKeyboardButton(f"💎 {months} Month — ${prices[months]:.2f}",callback_data=f"mprem:{bid}:{months}"))
    status=f"🟢 Active until <b>{html.escape(local_datetime_text(uid,until))}</b>" if active else "⚪ Not active"
    bot.send_message(chat_id,
        "💎 <b>Downloader Bot Premium</b>\n\n"
        f"Bot: <b>@{html.escape(str(d.get('username') or ''))}</b>\nStatus: {status}\n\n"
        "Premium removes the separate <b>Powered by</b> message and the creation promotion after downloads.\n"
        f"🌐 Platforms: <b>{len(premium_platform_names())}</b>\n⚡ Faster/priority processing\n🎥 Higher quality\n▶️ Full YouTube access\n\n"
        "Choose a plan. You can pay from your shared Wallet or with Telegram Stars.",reply_markup=kb)
...
def _managed_premium_callback(call):
    ...
    if data.startswith('mprem:'):
        parts=data.split(':')
        if len(parts)!=3: return
        bid,months=parts[1],parts[2]; d=_managed_bot_doc(bid)
        if not d or str(d.get('owner_id'))!=uid:
            bot.answer_callback_query(call.id,'Only the bot owner can purchase Premium.',show_alert=True); return
        ...
        until=_managed_premium_until(d); active=bool(until and until>datetime.now(timezone.utc))
        rows=[]
        if not active:
            price_asset=usd_to_asset(cur_code(uid),price); available=available_asset_amount(uid)
            wallet_text=f"💳 Wallet — ${price:.2f}" if available>=price_asset else f"💳 Wallet — ${price:.2f} (insufficient)"
        else:
            wallet_text=f"💳 Extend from Wallet — ${price:.2f}"
        rows.append([InlineKeyboardButton(wallet_text,callback_data=f"mpremwallet:{bid}:{months}")])
        stars_rate=max(1,int(get_setting("stars_per_usd",100) or 100)); stars=max(1,int(round(price*stars_rate)))
        rows.append([InlineKeyboardButton(f"⭐ Telegram Stars — {stars} Stars",callback_data=f"mpremstars:{bid}:{months}:{stars}")])
        rows.append([InlineKeyboardButton("❌ Cancel",callback_data=f"mpremcancel:{bid}")])
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id,f"💎 <b>{months} Month Premium</b>\n\nPrice: <b>${price:.2f}</b>\n\nChoose payment method:\n\n💳 <b>Wallet</b> — uses the same balance as @Downloadvedioytibot.\n⭐ <b>Telegram Stars</b> — opens a payment invoice issued by @Downloadvedioytibot, so the Stars payment goes to the main bot.",reply_markup=InlineKeyboardMarkup(rows)); return
    if data.startswith('mpremwallet:'):
        parts=data.split(':')
        if len(parts)!=3: return
        bid,months=parts[1],parts[2]; price=get_premium_prices().get(months); d=_managed_bot_doc(bid)
        if not d or str(d.get('owner_id'))!=uid or price is None: bot.answer_callback_query(call.id,'Invalid Premium request.',show_alert=True); return
        price_asset=usd_to_asset(cur_code(uid),price)
        if available_asset_amount(uid)<price_asset:
            bot.answer_callback_query(call.id,'Insufficient shared balance.',show_alert=True)
            bot.send_message(call.message.chat.id,f"❌ <b>Insufficient balance</b>\n\nNeed: <b>{html.escape(money_text(uid,price))}</b>\nAvailable: <b>{html.escape(money_text(uid,balance_usd_value(uid)))}</b>\n\n💡 Add balance or earn through referrals in @Downloadvedioytibot."); return
        now=datetime.now(timezone.utc); old=_managed_premium_until(d) or now; until=max(now,old)+timedelta(days=30*int(months))
        ledger_debit(uid,price_asset,'managed_bot_premium_purchase',{'price_usd':price,'months':months,'bot_id':bid,'payment':'wallet'})
        managed_bots_col.update_one({'bot_id':bid},{'$set':{'premium_until':until.isoformat(),'premium_source':'wallet','premium_updated_at':now}})
        premium_logs_col.insert_one({'user_id':uid,'bot_id':bid,'months':int(months),'price':price,'until':until.isoformat(),'time':now.isoformat(),'type':'managed_bot_purchase_wallet'})
        bot.answer_callback_query(call.id,'✅ Premium activated!')
        bot.send_message(call.message.chat.id,f"🎉 <b>Premium Activated</b>\n\n🤖 @{html.escape(str(d.get('username') or ''))}\n⏱️ {months} month(s)\n💵 Paid from Wallet: ${price:.2f}\n⏰ Expires: <b>{html.escape(local_datetime_text(uid,until))}</b>"); return
    if data.startswith('mpremstars:'):
        parts=data.split(':')
        if len(parts)!=4: return
        bid,months,stars=parts[1],parts[2],parts[3]; d=_managed_bot_doc(bid); price=get_premium_prices().get(months)
        if not d or str(d.get('owner_id'))!=uid or price is None: bot.answer_callback_query(call.id,'Invalid Premium request.',show_alert=True); return
        payload=f"managedprem:{bid}:{uid}:{months}:{stars}"
        url,err=_main_create_stars_invoice_link(f"Downloader Premium {months}M",f"Premium for @{str(d.get('username') or 'downloader')}",payload,int(stars))
        if err or not url:
            bot.answer_callback_query(call.id,'Could not create Stars invoice.',show_alert=True); return
        bot.answer_callback_query(call.id,'Opening Telegram Stars payment…')
        bot.send_message(call.message.chat.id,f"⭐ <b>Telegram Stars Payment</b>\n\nAmount: <b>{stars} Stars</b>\n\nThe payment invoice is issued by <b>@{html.escape(_main_bot_username())}</b>. After successful payment, Premium will be activated for this Downloader Bot.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⭐ Pay with Telegram Stars',url=url)]])); return
    if data.startswith('mpremcancel:'):
        bot.answer_callback_query(call.id,'Cancelled.')
        try: bot.delete_message(call.message.chat.id,call.message.message_id)
        except Exception: pass
        return
...
def _creator_wallet_card(uid, chat_id, regenerate=False):
    ...
    _creator_send(chat_id,
        "💳 <b>Creator Wallet</b>\n\n"
        f"Wallet code: <code>{masked}</code>\n"
        f"Balance: <b>{html.escape(money_text(uid,balance_usd_value(uid)))}</b>\n\n"
        "⚠️ This is an internal virtual wallet identifier, <b>not a bank card</b>.\n"
        "For security, new bot links now receive a fresh 16-digit code directly in @Downloadvedioytibot.\n"
        "Never share your wallet code with anyone.")
...
def _creator_admin_text(uid, chat_id, text):
    ...
    if text=="💳 Wallet Revoke Fee":
        current=float(get_setting("managed_wallet_revoke_fee_usd",0.25) or 0.0)
        _creator_set_session(uid,{"state":"admin_wallet_fee"}); _creator_send(chat_id,f"💳 <b>Wallet Revoke Fee</b>\n\nCurrent: <b>${current:.2f}</b>\n\nSend amount, e.g. <code>0.25</code>. Send <code>0</code> for FREE."); return
    if text=="🟢 Open Verify Create Bot":
        set_setting("create_bot_verify_required",True); _creator_send(chat_id,"🟢 <b>Verify-before-create OPEN</b>",reply_markup=_creator_admin_keyboard()); return
    ...
    sess=_creator_session(uid); state=sess.get("state")
    if state=="admin_wallet_fee":
        try:
            fee=max(0.0,float(text)); set_setting("managed_wallet_revoke_fee_usd",round(fee,2)); _creator_clear_session(uid); _creator_send(chat_id,f"✅ Wallet Revoke fee set to <b>${fee:.2f}</b>.",reply_markup=_creator_admin_keyboard())
        except Exception: _creator_send(chat_id,"❌ Invalid amount. Use <code>0.25</code> or <code>0</code>.",reply_markup=_creator_admin_keyboard())
        return
...
def _managed_reply_keyboard(bid, uid):
    d=_managed_bot_doc(bid) or {}
    owner=str(d.get("owner_id") or "")==str(uid)
    kb=ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add("💰 BALANCE")
    # Wallet, Premium and Bot Admin are private owner controls.
    if owner:
        kb.add("💳 Link Wallet","💎 PREMIUM")
        kb.add("👑 BOT ADMIN PANEL")
    if _creation_open() and _create_button_open(): kb.add("🤖 Create Your Own Bot")
    return kb

def _refresh_managed_bot_menus(notice=""):
    for d in managed_bots_col.find({"active":True,"suspended":{"$ne":True}}):
        bid=str(d.get("bot_id") or "")
        mb=managed_bot_objects.get(bid)
        if not mb: continue
        for target in (d.get("users") or []):
            try:
                if notice: mb.send_message(int(target),notice,reply_markup=_managed_reply_keyboard(bid,target))
                else: mb.send_message(int(target),"🔄 Menu updated.",reply_markup=_managed_reply_keyboard(bid,target))
            except Exception: pass

def _managed_admin_keyboard(bid):
    return InlineKeyboardMarkup(row_width=2).add(
        InlineKeyboardButton("📊 Statistics",callback_data=f"madmin:stats:{bid}"),
        InlineKeyboardButton("👥 Users",callback_data=f"madmin:users:{bid}"),
        InlineKeyboardButton("📢 Broadcast",callback_data=f"madmin:broadcast:{bid}"),
        InlineKeyboardButton("⚙️ Settings",callback_data=f"madmin:settings:{bid}"),
        InlineKeyboardButton("💎 Premium",callback_data=f"madmin:premium:{bid}"),
    )

def _managed_admin_panel(m,bid):
    uid=str(m.from_user.id); d=_managed_bot_doc(bid)
    if not d or str(d.get("owner_id"))!=uid:
        bot.send_message(m.chat.id,"🔐 Owner-only Admin Panel."); return
    bot.send_message(m.chat.id,f"👑 <b>BOT ADMIN PANEL</b>\n\n🤖 @{html.escape(str(d.get('username') or ''))}\n📥 Downloads: <b>{int(d.get('download_count',0) or 0)}</b>\n👥 Users: <b>{len(d.get('users') or [])}</b>\n🟢 Status: {'Active' if d.get('active',True) and not d.get('suspended') else 'Paused'}",reply_markup=_managed_admin_keyboard(bid))

def _managed_admin_callback(call,bid,action):
    uid=str(call.from_user.id); d=_managed_bot_doc(bid)
    if not d or str(d.get("owner_id"))!=uid:
        bot.answer_callback_query(call.id,"Owner-only.",show_alert=True); return
    if action=="stats":
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id,f"📊 <b>Bot Statistics</b>\n\n🤖 @{html.escape(str(d.get('username') or ''))}\n📥 Total downloads: <b>{int(d.get('download_count',0) or 0)}</b>\n👥 Users: <b>{len(d.get('users') or [])}</b>\n💎 Premium: {'Active' if _managed_premium_active(bid,uid) else 'Inactive'}")
    elif action=="users":
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id,f"👥 <b>Bot Users</b>\n\nUsers who interacted: <b>{len(d.get('users') or [])}</b>\n\nUser IDs are stored privately for bot management and broadcasts.")
    elif action=="broadcast":
        bot.answer_callback_query(call.id)
        msg=bot.send_message(call.message.chat.id,"📢 <b>Broadcast</b>\n\nSend the message you want to send to users of this bot.")
        bot.register_next_step_handler(msg,lambda x:_managed_admin_broadcast_step(x,bid))
    elif action=="settings":
        bot.answer_callback_query(call.id)
        kb=InlineKeyboardMarkup(row_width=2)
        kb.add(InlineKeyboardButton("🟢 Resume" if d.get('suspended') else "🔴 Pause",callback_data=f"madmin:toggle:{bid}"),InlineKeyboardButton("💳 Wallet",callback_data=f"mwallet:{bid}"))
        kb.add(InlineKeyboardButton("🤖 Creator",url=_creator_bot_url() or "https://t.me"))
        bot.send_message(call.message.chat.id,"⚙️ <b>Bot Settings</b>\n\nPause stops this bot's worker; Resume starts it again. Wallet opens the shared wallet flow.",reply_markup=kb)
    elif action=="premium":
        bot.answer_callback_query(call.id)
        _managed_premium_menu(uid,call.message.chat.id,bid)
    elif action=="toggle":
        if d.get('suspended'):
            managed_bots_col.update_one({'bot_id':bid},{'$set':{'suspended':False,'active':True,'updated_at':datetime.now(timezone.utc)}})
            nd=managed_bots_col.find_one({'bot_id':bid}); _managed_bot_start_instance(nd)
            bot.answer_callback_query(call.id,"Bot resumed.")
        else:
            managed_bots_col.update_one({'bot_id':bid},{'$set':{'suspended':True,'active':False,'updated_at':datetime.now(timezone.utc)}})
            mb=managed_bot_objects.pop(bid,None)
            managed_bot_threads.pop(bid,None)
            if mb:
                try: mb.stop_polling()
                except Exception: pass
            bot.answer_callback_query(call.id,"Bot paused.")
        bot.send_message(call.message.chat.id,"⚙️ <b>Settings updated.</b>")

def _managed_admin_broadcast_step(m,bid):
    uid=str(m.from_user.id); d=_managed_bot_doc(bid)
    if not d or str(d.get('owner_id'))!=uid: return
    text=str(m.text or m.caption or '').strip()
    if not text:
        bot.send_message(m.chat.id,"❌ Send a text message."); return
    mb=managed_bot_objects.get(bid) or _managed_bot_start_instance(d)
    sent=failed=0
    if mb:
        for target in d.get('users') or []:
            try: mb.send_message(int(target),text,parse_mode="HTML"); sent+=1
            except Exception: failed+=1
    bot.send_message(m.chat.id,f"✅ <b>Broadcast complete</b>\n\n📨 Sent: <b>{sent}</b>\n❌ Failed: <b>{failed}</b>")
    ...
    with managed_bot_lock:
        ...
        try:
            ...
            def _start(m):
                ...
                owner=(str(meta.get("owner_id") or "")==str(m.from_user.id))
                kb=_managed_reply_keyboard(bid,m.from_user.id)
                inline=[]
                if owner:
                    inline=[[InlineKeyboardButton("💳 Link Wallet",callback_data=f"mwallet:{bid}")],[InlineKeyboardButton("💎 Premium",callback_data=f"mopenprem:{bid}")],[InlineKeyboardButton("👑 BOT ADMIN PANEL",callback_data=f"mbotadmin:{bid}")]]
                if _creation_open() and _create_button_open(): inline.append([InlineKeyboardButton("🤖 Create Your Own Bot",callback_data=f"mcreate:{bid}")])
                bot.send_message(m.chat.id,
                    "🚀 <b>Welcome to your Downloader Bot</b>\n\n"
                    "Send a supported video link and I will download it quickly.\n\n"
                    "🌐 <b>Platforms:</b> TikTok • Instagram • Facebook • YouTube • Pinterest • Snapchat • X/Twitter + more\n"
                    "🎵 MP3 is available from video downloads.\n\n"
                    "💎 Premium removes system promotional messages and unlocks Premium download features.",
                    parse_mode="HTML",reply_markup=kb)
                if inline:
                    bot.send_message(m.chat.id,"Choose an option:",reply_markup=InlineKeyboardMarkup(inline))
            ...
            def _create(m):
                _ctx();
                if not _creation_open() or not _create_button_open(): bot.send_message(m.chat.id,"🔒 <b>Bot creation is currently unavailable.</b>\nExisting downloader bots continue working."); return
                url=_creator_bot_url()
            ...
            def _premium(m):
                ...
                _managed_premium_menu(str(m.from_user.id),m.chat.id,bid)
            ...
            def _madmin(call):
                ...
                if len(parts)>=3: _managed_admin_callback(call,parts[2],parts[1])

            def _mopenadmin(call):
                _ctx()
                data=str(call.data or "")
                bid2=data.split(":",1)[1] if ":" in data else bid
                if not _managed_bot_is_owner(bid2,call.from_user.id):
                    bot.answer_callback_query(call.id,"Owner-only.",show_alert=True); return
                bot.answer_callback_query(call.id)
                bot.send_message(call.message.chat.id,"👑 <b>BOT ADMIN PANEL</b>",reply_markup=_managed_admin_keyboard(bid2))

            def _mwallet(call):
                _ctx();
                data=str(call.data or "")
                if data.startswith("mwalletrevoke:"): _managed_wallet_revoke_callback(call,data.split(":",1)[1])
                elif data.startswith("mwalletconfirm:"): _managed_wallet_revoke_confirm(call,data.split(":",1)[1])
                elif data.startswith("mwalletcancel:"): _managed_wallet_revoke_cancel(call)
                elif data.startswith("mwallet:"):
                    _managed_wallet_start(call.message,bid,user_id=call.from_user.id)

            def _mopenprem(call):
                _ctx(); _managed_premium_menu(str(call.from_user.id),call.message.chat.id,bid)

            def _mcreate(call):
                _ctx();
                if not _creation_open() or not _create_button_open(): bot.answer_callback_query(call.id,"Creation is closed.",show_alert=True); return
                url=_creator_bot_url(); bot.answer_callback_query(call.id)
                if url: bot.send_message(call.message.chat.id,"🤖 <b>Create Your Own Bot</b>",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Open Creator Bot",url=url)]]))

            def _madmin_text(m):
                _ctx(); _managed_admin_panel(m,bid)
            ...
            mb.message_handler(func=lambda m:m.text=="💎 PREMIUM")(_premium); mb.message_handler(func=lambda m:m.text=="💰 BALANCE")(_balance); mb.message_handler(func=lambda m:m.text=="💳 Link Wallet")(_wallet) ; mb.message_handler(func=lambda m:m.text=="🤖 Create Your Own Bot")(_create); mb.message_handler(func=lambda m:m.text=="👑 BOT ADMIN PANEL")(_madmin_text)
            def _wallet_code(m):
                _ctx()
                uid=str(m.from_user.id)
                pending=managed_wallet_pending.get(uid)
                if not pending or str(pending.get("bot_id"))!=bid:
                    return
                _managed_wallet_process(m,bid)

            mb.message_handler(func=lambda m:bool(m.text and str(m.from_user.id) in managed_wallet_pending and str(managed_wallet_pending.get(str(m.from_user.id),{}).get("bot_id"))==bid))(_wallet_code)
            ...
            mb.callback_query_handler(func=lambda c:c.data.startswith(("mprem:","mpremwallet:","mpremstars:","mpremcancel:")))(_cbpremium)
            mb.callback_query_handler(func=lambda c:c.data.startswith(("mwallet:","mwalletrevoke:","mwalletconfirm:","mwalletcancel:")))(_mwallet)
            mb.callback_query_handler(func=lambda c:c.data.startswith("mopenprem:"))(_mopenprem)
            mb.callback_query_handler(func=lambda c:c.data.startswith("mcreate:"))(_mcreate)
            mb.callback_query_handler(func=lambda c:c.data.startswith("mbotadmin:"))(_mopenadmin)
            mb.callback_query_handler(func=lambda c:c.data.startswith("madmin:"))(_madmin)
            def _run():
...
def _creator_poll_loop():
    if not CREATOR_BOT_TOKEN:
        print("ℹ️ CREATOR_BOT_TOKEN is not configured; managed-bot Creator is disabled."); return
    offset=0
...
def _refresh_all_user_menus(notice=""):
    # Reply keyboards cannot be edited globally without sending a new keyboard.
    # Re-send the current main menu and existing managed-bot menus so creation
    # visibility changes take effect immediately.
    if not notice: return
    for uid in list(users.keys()):
        try: _main_bot.send_message(int(uid),notice,reply_markup=localized_user_menu(uid))
        except Exception: pass
    try: _refresh_managed_bot_menus(notice)
    except Exception as e: print("Managed menu refresh error:",repr(e))
...
@bot.message_handler(func=lambda m: m.text == "🟢 OPEN CREATE BUTTON")
def admin_open_create_button(m):
    if not is_admin(m.from_user.id): return
    set_setting("managed_create_button_enabled",True)
    bot.send_message(m.chat.id,"🟢 <b>CREATE BUTTON OPEN</b>\n\nThe Create Your Own Bot button is visible again inside existing downloader bots.",reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "🔴 CLOSE CREATE BUTTON")
def admin_close_create_button(m):
    if not is_admin(m.from_user.id): return
    set_setting("managed_create_button_enabled",False)
    bot.send_message(m.chat.id,"🔴 <b>CREATE BUTTON CLOSED</b>\n\nThe Create button is hidden. Existing downloader bots continue working.",reply_markup=admin_menu())
...
def admin_open_powered_by(m):
    ...
    bot.send_message(m.chat.id,"🟢 <b>POWERED BY OPEN</b>\n\nManaged downloader bots will show <code>Powered by: @Downloadvedioytibot</code> after delivered media unless that bot has Premium.",reply_markup=admin_menu())
...
def admin_close_powered_by(m):
    ...
    bot.send_message(m.chat.id,"🔴 <b>POWERED BY CLOSED</b>\n\nManaged downloader bots will no longer show the Powered-by message. Premium status is unchanged.",reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "💳 WALLET REVOKE FEE")
def admin_wallet_revoke_fee(m):
    if not is_admin(m.from_user.id): return
    current=float(get_setting("managed_wallet_revoke_fee_usd",0.25) or 0.0)
    msg=bot.send_message(m.chat.id,f"💳 <b>Wallet Revoke Fee</b>\n\nCurrent fee: <b>${current:.2f}</b>\n\nSend a new amount, e.g. <code>0.25</code>. Send <code>0</code> to make Revoke FREE.")
    bot.register_next_step_handler(msg,admin_wallet_revoke_fee_step)

def admin_wallet_revoke_fee_step(m):
    if not is_admin(m.from_user.id): return
    try:
        fee=max(0.0,float((m.text or "").strip()))
        set_setting("managed_wallet_revoke_fee_usd",round(fee,2))
        bot.send_message(m.chat.id,f"✅ Wallet Revoke fee set to <b>${fee:.2f}</b>.\n\nUse 0 to make it free.",reply_markup=admin_menu())
    except Exception:
        bot.send_message(m.chat.id,"❌ Invalid amount. Example: <code>0.25</code> or <code>0</code>.",reply_markup=admin_menu())
...
def start_handler(message):
    ...
    if len(args)>1 and args[1].lower().startswith("verifycreate"):
        if user_is_verified(uid):
            bot.send_message(message.chat.id,"✅ <b>Your account is already verified.</b>\n\nReturn to the Creator Bot and press <b>Create My Bot</b>.")
        else:
            kb=InlineKeyboardMarkup(); kb.add(InlineKeyboardButton("🔐 VERIFY ACCOUNT",callback_data="start_verify_flow"))
            bot.send_message(message.chat.id,"🔐 <b>Verify before creating a bot</b>\n\nChoose a verification method below. After verification, return to the Creator Bot.",reply_markup=kb)
        return
    if uid not in users:
...
def broadcast_send(m):
    if not is_admin(m.from_user.id):
        return
    text = m.text
    sent,failed=broadcast_users(lambda uid: bot.send_message(int(uid), text), list(users.keys()))
    try:
...
def successful_payment_handler(message):
    ...
    if payload.startswith("managedprem:"):
        try:
            parts=payload.split(":")
            if len(parts)>=5:
                bid,owner_id,months,stars=parts[1],parts[2],parts[3],int(parts[4])
                if owner_id!=uid:
                    bot.send_message(message.chat.id,"❌ Premium payment owner mismatch. Contact admin."); return
                ...
                if not d or str(d.get("owner_id"))!=uid or price is None:
                    bot.send_message(message.chat.id,"❌ Premium request is no longer valid."); return
                if currency != "XTR" or stars != expected_stars:
                    bot.send_message(message.chat.id,"❌ Payment verification failed: Stars amount or currency does not match the invoice."); return
                if charge_id and premium_logs_col.find_one({"type":"managed_bot_purchase_stars","telegram_payment_charge_id":charge_id}):
                    bot.send_message(message.chat.id,"ℹ️ This Stars payment was already processed."); return
                now=datetime.now(timezone.utc); old=_managed_premium_until(d) or now; until=max(now,old)+timedelta(days=30*int(months))
                # The invoice was issued by the MAIN bot, so the Stars revenue belongs to @Downloadvedioytibot.
                # Internally we also record the USD-equivalent value in the shared account ledger for reporting.
                usd=stars/max(1,float(get_setting("stars_per_usd",100) or 100))
                managed_bots_col.update_one({"bot_id":bid},{"$set":{"premium_until":until.isoformat(),"premium_source":"telegram_stars","premium_stars":stars,"premium_updated_at":now}})
                premium_logs_col.insert_one({"user_id":uid,"bot_id":bid,"months":int(months),"price":price,"stars":stars,"usd_equivalent":usd,"until":until.isoformat(),"time":now.isoformat(),"type":"managed_bot_purchase_stars","telegram_payment_charge_id":charge_id})
                log_activity(uid,"managed_bot_premium_stars",{"bot_id":bid,"months":int(months),"stars":stars,"usd_equivalent":usd})
                bot.send_message(message.chat.id,f"🎉 <b>Premium Activated</b>\n\n🤖 @{html.escape(str(d.get('username') or ''))}\n⭐ Paid: <b>{stars} Stars</b>\n⏱️ {months} month(s)\n⏰ Expires: <b>{html.escape(local_datetime_text(uid,until))}</b>\n\n✅ Payment verified by @Downloadvedioytibot.")
        except Exception as e:
            print("Managed Stars premium error:",repr(e))
            bot.send_message(message.chat.id,"❌ Premium payment was received but activation failed. Please contact admin.")
        return
...
if __name__ == "__main__":
    _creation_commands_refresh()
    threading.Thread(target=_managed_bots_startup, daemon=True).start()
