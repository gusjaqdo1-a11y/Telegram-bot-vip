...
WHATSAPP_OTP_TTL = 600  # 10 minutes (matches D7 Verify expiry)
...
def verify_d7_whatsapp(phone_number, code):
    ...
    otp_id=None
    # Prefer the exact active session for this user/phone.
...
def expire_whatsapp_session(uid, created_at):
    ...
    try:
        bot.send_message(
            ...
            "The WhatsApp verification code is valid for 10 minutes. "
...
def process_verification_whatsapp(m):
    ...
    msg = bot.send_message(
        m.chat.id,
        f"📲 <b>WhatsApp OTP request accepted</b>\n\n"
...
def resend_whatsapp_code_callback(call):
    ...
    msg = bot.send_message(
        call.message.chat.id,
        "📲 <b>New WhatsApp OTP request accepted.</b>\n\n"
...
def _rapidapi_youtube_details(video_id):
    ...
    r = requests.get(RAPIDAPI_YT_DETAILS_URL, params=params, headers=headers, timeout=RAPIDAPI_TIMEOUT)
    ...
    if r.status_code >= 400:
        ...
        raise RuntimeError(f"RapidAPI YouTube HTTP {r.status_code}: {detail}")
...
def _rapid_formats(data):
    """Normalize the API's simplified/raw video and audio objects."""
    out=[]; seen=set()
...
def _rapid_download(link,tmp_dir,quality,max_seconds,chat_id=None):
    ...
    if not shutil.which("ffmpeg"):
        # Do not pretend the two streams are one file when ffmpeg is absent.
        return [video_path]
