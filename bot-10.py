...
def download_media(chat_id, link, message_id):
    ...
    if platform == "unknown":
        bot.edit_message_text("❌ Unsupported or invalid link. Supported: TikTok, Facebook, Pinterest, Snapchat, Instagram, X/Twitter and YouTube.", chat_id, message_id)
        return
    ...
    try:
        # First inspect without downloading. This lets us enforce YouTube duration.
        ...
        # Some image-first extractors don't have a video format; yt-dlp will pick their best image.
        ...
        # Ignore sidecar metadata files if an extractor produced them.
        ...
        for path in media_files[:20]:
            # MUSIC is offered only for video files.  The callback stores only the source URL,
            # so the large downloaded video does not need to remain on disk after sending.
            reply_markup = None
            if _is_video_file(path):
                music_token = uuid.uuid4().hex[:24]
                music_pending[music_token] = {
                    "uid": uid_str,
                    "link": link,
                    "created": time.time()
                }
...
def convert_link_to_mp3(chat_id, link, status_message_id):
    ...
    try:
        ...
        mp3_files = _collect_downloaded_files(tmp_dir)
        mp3_files = [p for p in mp3_files if _is_audio_file(p)]
        if not mp3_files and prepared:
            candidate = os.path.splitext(prepared)[0] + ".mp3"
            if os.path.isfile(candidate):
                mp3_files = [candidate]
        if not mp3_files:
            raise RuntimeError("MP3 file was not created. Make sure FFmpeg is installed on the server.")

        path = mp3_files[0]
        if os.path.getsize(path) > MAX_UPLOAD_MB * 1024 * 1024:
            raise RuntimeError("The MP3 is too large for the configured Telegram upload limit.")
        ...
        try:
            bot.edit_message_text(f"❌ MP3 conversion failed.\n\n{str(e)[:700]}", chat_id, status_message_id)
        except Exception:
            pass
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def music_callback_handler(call):
    ...
    if not data:
        try:
            bot.answer_callback_query(call.id, "❌ This MUSIC button has expired. Download the video again.", show_alert=True)
        except Exception:
            pass
        return
    ...
    # Consume the token so repeated taps cannot queue unlimited conversions.
    music_pending.pop(token, None)
    try:
