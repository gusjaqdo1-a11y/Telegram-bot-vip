...
def cur_code(uid):
    code=lang_of(uid); return LANGUAGES.get(code,LANGUAGES["en"])["currency"]

def fx_rate(uid):
    code=cur_code(uid); base=next((v["rate"] for v in LANGUAGES.values() if v["currency"]==code),1.0)
...
START_MESSAGE_DEFAULT = """🎬 Welcome to Video Downloader Bot!

Copy a video link from a supported platform and send it here.

🎵 Convert videos to MP3
💎 Premium gives higher quality, faster downloads and extra platforms.
...
def send_waforge_whatsapp(phone_number, user_id):
    ...
    try:
        r = requests.post(WAFORGE_OTP_SEND_URL, json={"to": phone_number}, headers=headers, timeout=30)
        try: body = r.json()
        except Exception: body = {"raw": r.text}
        request_id = body.get("id") or body.get("request_id") or body.get("uuid")
        if r.status_code in (200,201,202) and request_id:
...
def verify_waforge_whatsapp(phone_number, code):
    ...
    headers = {"Authorization": f"Bearer {WAFORGE_API_KEY}", "Content-Type": "application/json", "Accept": "application/json"}
    try:
        r = requests.post(WAFORGE_OTP_VERIFY_URL, json={"to": phone_number, "code": str(code)}, headers=headers, timeout=30)
        try: body = r.json()
        except Exception: body = {"raw": r.text}
        return r.status_code == 200 and bool(body.get("verified")), body
    except Exception as e:
        return False, str(e)
...
def admin_menu():
    ...
    kb.add("🎁 TRIAL PREMIUM", "👑 PREMIUM USERS")
...
def localized_user_menu(uid):
    ...
    kb=ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add(d["balance"],d["withdraw"]); kb.add(d["ref"],d["id"]); kb.add(d["premium"],d["music"]); kb.add(d["profile"],d["topup"]); kb.add(d["history"],d["lang"]); kb.add(d["trial"]); kb.add(d["customer"],d["ai"])
...
def setlang_callback(call):
    ...
    if pending:
        ref_user=next((u for u,d in users.items() if d.get("ref")==pending and u!=uid),None)
        if ref_user:
            users[ref_user]["balance"]=users[ref_user].get("balance",0)+0.2; users[ref_user]["invited"]=users[ref_user].get("invited",0)+1
            ...
            save_user(ref_user); log_activity(ref_user,"referral",{"referred_user":uid})
...
def profile_handler(m):
    ...
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("📜 History", callback_data="profile_history"), InlineKeyboardButton("🌐 Change Language", callback_data="profile_language"))
    if not verified:
...
def process_verification_code(m):
    ...
    if code_input == data["code"]:
        ...
        email_verify_pending.pop(uid, None)
        try:
            bot.send_message(m.chat.id, "✅ Congratulations! Your account is now Verified. You can check your profile status.", reply_markup=localized_user_menu(str(m.from_user.id)))
        except: pass
    else:
        try:
            kb = InlineKeyboardMarkup()
...
def process_verification_whatsapp(m):
    ...
    if not ok:
        bot.send_message(m.chat.id, "❌ WaForge could not send the WhatsApp OTP. Check WAFORGE_API_KEY and your connected number.")
        print("WaForge OTP error:", detail)
        return
    now = time.time()
    whatsapp_verify_pending[uid] = {"phone": phone_input, "request_id": request_id, "time": now, "last_resend": now}
    kb = InlineKeyboardMarkup()
    kb.row(InlineKeyboardButton("🔄 Resend WaForge", callback_data="resend_whatsapp_code"), InlineKeyboardButton("❌ Cancel", callback_data="cancel_verify_process"))
    msg = bot.send_message(m.chat.id, f"📲 WaForge sent a 6-digit OTP to {phone_input}. Enter the code:", reply_markup=kb)
...
def resend_whatsapp_code_callback(call):
    ...
    if not ok:
        bot.answer_callback_query(call.id, "Could not send WaForge OTP.", show_alert=True); return
    data["request_id"] = request_id; data["last_resend"] = time.time()
...
def process_whatsapp_code(m):
    ...
    if not data:
        bot.send_message(m.chat.id,"❌ WhatsApp verification session expired."); return
    code=(m.text or "").strip()
    if not re.fullmatch(r"\d{6}",code):
        msg=bot.send_message(m.chat.id,"⚠️ Enter the 6-digit code:"); bot.register_next_step_handler(msg,process_whatsapp_code); return
    ok, detail=verify_waforge_whatsapp(data["phone"],code)
    if not ok:
        msg=bot.send_message(m.chat.id,"❌ Incorrect or expired code. Try again:"); bot.register_next_step_handler(msg,process_whatsapp_code); return
    users[uid]["verified"]=True; users[uid]["phone"]=data["phone"]; users[uid]["whatsapp_verified"]=True; users[uid]["sticker"]=users[uid].get("sticker") or "🌟"
    save_user(uid); whatsapp_verify_pending.pop(uid,None)
    bot.send_message(m.chat.id,"✅ WaForge WhatsApp verification successful. Your account is now verified.",reply_markup=localized_user_menu(uid))
...
def start_handler(message):
    ...
    if uid not in users:
        users[uid]={"username":message.from_user.username or "","balance":0.0,"blocked":0.0,"ref":random_ref(),"bot_id":random_botid(),"invited":0,"banned":False,"verified":False,"quick_access":False,"youtube_30m":False,"premium_until":None,"premium_warning_sent":False,"trial_used":False,"referral_50_rewarded":False,"joined_date":datetime.now().strftime("%Y-%m-%d"),"month":now_month(),"language":None,"gender":None,"city":None,"pending_ref":args[1] if len(args)>1 else None}
        save_user(uid)
...
def check_membership(user_id):
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            bot.send_message(
                user_id,
                get_setting("start_message", START_MESSAGE_DEFAULT),
...
def balance_handler(m):
    ...
    try:
        bot.send_message(m.chat.id, f"💰 Available Balance: <b>{money_text(uid, bal)}</b>\n💵 USD: ${bal:.2f}\n⏳ Blocked Amount: <b>{money_text(uid, blocked)}</b>")
    except: pass
...
def successful_payment_handler(message):
    ...
    if payload.startswith("balance_topup:"):
        ...
        users[uid]["balance"] = round(float(users[uid].get("balance", 0)) + usd, 8)
        save_user(uid)
...
def show_premium_menu(chat_id):
    ...
    if is_premium(uid):
        ...
        if not users[uid].get("trial_used", False): kb.add(InlineKeyboardButton("🎁 1 DAY FREE TRIAL", callback_data="premium_trial"))
        ...
        try:
            ...
            for uid,data in list(users.items()):
                ...
                if dt<=now:
                    ...
                    try: bot.send_message(int(uid),"⏰ <b>Your Premium has expired.</b>\n\n💎 Open Premium to renew.")
                    except Exception: pass
...
def open_premium_process(m):
    ...
    try:
        ...
        bot.send_message(int(uid),f"🎉 <b>Premium opened by Admin</b>\n\n⏰ Expires: {until.strftime('%Y-%m-%d %H:%M UTC')}\n💎 You now have high quality, faster downloads, full YouTube and extra platforms.")
        send_premium_email(uid,'Premium opened by Admin',months,0,until)
    except Exception:
...
def premium_trial_callback(call):
    ...
    if not users.get(uid,{}).get("verified"):
        bot.answer_callback_query(call.id,"❌ Verify your account first.",show_alert=True); return
    if users[uid].get("trial_used"):
        bot.answer_callback_query(call.id,"❌ Trial already used.",show_alert=True); return
    until=grant_premium_days(uid,1,"trial_1_day"); users[uid]["trial_used"]=True; save_user(uid)
    bot.answer_callback_query(call.id,"🎁 Trial activated!")
    bot.send_message(call.message.chat.id,f"🎁 <b>1 DAY PREMIUM TRIAL</b>\n\n⏰ Expires: {until.strftime('%Y-%m-%d %H:%M UTC')}")
    send_premium_email(uid,"Your 1-Day Premium Trial is Active",0,0,until)
...
def admin_trial_step(m):
    ...
    try:
        ...
        bot.send_message(int(uid),f"🎁 <b>Premium renewed by admin</b>\n\n⏰ Expires: {until.strftime('%Y-%m-%d %H:%M UTC')}")
        send_premium_email(uid,"Premium Trial Renewed",0,0,until)
...
def localized_action_dispatch(m):
    ...
    if action=="trial":
        if not users[uid].get("verified"): bot.send_message(m.chat.id,"🔐 Verify your account first."); return
        if users[uid].get("trial_used"): bot.send_message(m.chat.id,"❌ Trial already used."); return
        until=grant_premium_days(uid,1,"trial_1_day"); users[uid]["trial_used"]=True; save_user(uid); bot.send_message(m.chat.id,f"🎁 <b>1 DAY PREMIUM TRIAL ACTIVATED!</b>\n\n⏰ {until.strftime('%Y-%m-%d %H:%M UTC')}"); send_premium_email(uid,"Your 1-Day Premium Trial is Active",0,0,until)
