...
def promo_user_menu(m):
    ...
    if not bool(get_setting("promo_system_enabled",False)) or not public:
        bot.send_message(m.chat.id,"🎟 <b>PROMO CODE</b>\n\nNo promo code is available right now.",reply_markup=localized_user_menu(uid),parse_mode="HTML")
        return
    x=public[0]
...
def promo_user_enabled():
    return bool(get_setting("promo_system_enabled", False)) and bool(_promo_public_codes())
...
def _grant_premium_seconds(uid,seconds,reason="promo_code"):
    ...
    users[uid]["premium_until"]=until.isoformat(); users[uid]["premium_warning_sent"]=False
    users[uid]["premium_source"]="promo"
    save_user(uid)
    try: premium_logs_col.insert_one({"user_id":uid,"seconds":seconds,"until":until.isoformat(),"time":now.isoformat(),"type":reason})
    except Exception: pass
...
def promo_admin_callback(call):
    ...
    if action=="open":
        set_setting("promo_system_enabled",True); _refresh_all_user_menus("🔄 Promo system updated."); bot.answer_callback_query(call.id,"Promo system OPEN")
    elif action=="close":
...
def _jamendo_song_search(query, limit=100):
    ...
    # 3) A small partial-word ladder. Keep this deliberately short so song
    #    searches return quickly instead of making 10+ sequential HTTP calls.
    words=[w for w in query.split() if len(w)>=2]
    for w in sorted(set(words),key=len,reverse=True)[:2]:
        modes += [{"search":w},{"namesearch":w}]

    for mode in modes:
        add_batch(call(mode))
        if len(raw)>=max(int(limit)*2,80): break

    # 4) Only use autocomplete when the first pass is sparse.
    if len(raw)<10:
        try:
            ac_params={"client_id":JAMENDO_CLIENT_ID,"format":"json","prefix":query,"limit":10,"matchcount":"true","entity":"artists tracks albums"}
...
def _song_auto_search_should_handle(m):
    ...
    if not text or text.startswith("/") or not _song_search_is_open() or not get_setting("song_auto_search_enabled",False) or is_admin(m.from_user.id): return False
