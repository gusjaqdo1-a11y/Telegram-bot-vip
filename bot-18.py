...
def start_handler(message):
    ...
    if uid not in users:
        users[uid]={"username":message.from_user.username or "","balance":0.0,"blocked":0.0,"ref":random_ref(),"bot_id":random_botid(),"invited":0,"banned":False,"verified":False,"quick_access":False,"youtube_30m":False,"premium_until":None,"premium_warning_sent":False,"trial_used":False,"trial_pending":False,"referral_50_rewarded":False,"joined_date":datetime.now().strftime("%Y-%m-%d"),"month":now_month(),"language":None,"currency":"USD","gender":None,"city":None,"pending_ref":args[1] if len(args)>1 else None}
        save_user(uid)
...
def activate_pending_trial(uid, chat_id=None):
    ...
    if not u.get("trial_pending") or u.get("trial_used") or not u.get("verified"):
        return False
    u["trial_pending"]=False
    until=grant_premium_days(uid,1,"trial_1_day")
    u["trial_used"]=True; save_user(uid)
    if chat_id is not None:
        try:
            bot.send_message(chat_id, f"🎁 <b>1 DAY PREMIUM TRIAL ACTIVATED!</b>\n\n⏰ Expires: {until.strftime('%Y-%m-%d %H:%M UTC')}")
        except Exception: pass
...
def premium_trial_callback(call):
    uid=str(call.from_user.id)
    if users.get(uid,{}).get("trial_used"):
        bot.answer_callback_query(call.id,"❌ Trial already used.",show_alert=True); return
    if not users.get(uid,{}).get("verified"):
        users[uid]["trial_pending"]=True; save_user(uid)
        kb=InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🔐 VERIFY ACCOUNT", callback_data="start_verify_flow"))
        bot.answer_callback_query(call.id,"Verify once and your trial will activate automatically.",show_alert=True)
        bot.send_message(call.message.chat.id,"🔐 <b>Verification required</b>\n\nAfter verification, your 1-Day Premium Trial will activate automatically. You will NOT need to press the trial button again.",reply_markup=kb)
        return
    users[uid]["trial_pending"]=True; save_user(uid)
...
def localized_action_dispatch(m):
    ...
    if action=="trial":
        if users[uid].get("trial_used"):
            bot.send_message(m.chat.id,"❌ Trial already used."); return
        if not users[uid].get("verified"):
            ...
            bot.send_message(m.chat.id,"🔐 <b>Verification required</b>\n\nVerify once and your 1-Day Premium Trial will activate automatically. You will NOT need to press the trial button again.",reply_markup=kb); return
        users[uid]["trial_pending"]=True; save_user(uid); activate_pending_trial(uid,m.chat.id)
