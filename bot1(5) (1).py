...
def _support_start_keyboard():
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("📩 Contact admin directly",callback_data="support:new"))
    return kb
...
def _support_begin(customer_bot,message):
    ...
    kb=ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=True,row_width=1)
    kb.add(KeyboardButton("📱 Share phone number",request_contact=True)); kb.add(KeyboardButton("⏭️ Skip phone"))
...
def _finish_support_ticket(customer_bot,message,uid):
    ...
    if not ticket: return
    ticket["code"]=_support_ticket_code()
    ...
    for aid in SUPPORT_ADMIN_IDS:
        for sender in (customer_bot,bot):
            try: sender.send_message(int(aid),text,parse_mode="HTML")
            except Exception as e: print("Support admin notify error:",repr(e))
    lang=ticket.get("language","en")
    ...
    customer_bot.send_message(message.chat.id,done.get(lang,done["en"]),reply_markup=ReplyKeyboardRemove(),parse_mode="HTML")
...
if customer_ai_bot:
    ...
    @customer_ai_bot.callback_query_handler(func=lambda c:c.data=="support:new")
    def customer_ai_support_callback(call):
        uid=str(call.from_user.id)
        if uid not in users:
            customer_ai_bot.answer_callback_query(call.id,"Please start Customer Support first.",show_alert=True); return
        customer_ai_bot.answer_callback_query(call.id); _support_begin(customer_ai_bot,call.message)
