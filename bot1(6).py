...
def download_media(chat_id, link, message_id, quality=None):
    ...
    try:
        ...
        if platform=="youtube" and RAPIDAPI_YT_KEY:
            try:
                # RapidAPI is the primary YouTube downloader. Pass the original
                # URL first, then retry once with a canonical watch URL if the
                # provider rejects a Shorts/youtu.be variant.
                media=_rapid_download(link,tmp,quality,max_seconds,chat_id=chat_id); provider="rapidapi-youtube"
            except Exception as e:
                print("RapidAPI YouTube first attempt failed:",repr(e))
                vid=_extract_youtube_video_id(link)
                if vid:
                    try:
                        canonical=f"https://www.youtube.com/watch?v={vid}"
                        media=_rapid_download(canonical,tmp,quality,max_seconds,chat_id=chat_id); provider="rapidapi-youtube"
                    except Exception as e2:
                        print("RapidAPI YouTube canonical retry failed:",repr(e2))
