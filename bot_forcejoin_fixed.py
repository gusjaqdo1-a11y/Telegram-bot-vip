...
def _member_is_joined(member):
    """Return True for every Telegram status that means the user is in the channel."""
    return getattr(member, "status", "") in {
        "member", "administrator", "creator", "restricted"
    }


def _send_start_home(user_id):
    """Send the normal /start onboarding message and menu, with no join notification."""
    try:
        bot.send_message(
            int(user_id),
...
def check_membership(user_id):
    ...
    If the user is already a channel member, immediately continue with the normal
    /start home.  Do not send a warning, 'join verified' notification, or a
    separate confirmation message to members.
    """
    touch_user(user_id)
    try:
        member = bot.get_chat_member(CHANNEL_USERNAME, int(user_id))
        if _member_is_joined(member):
            # Remember a successful check so an already-joined user can continue
            # smoothly after transient Telegram API errors.
            try:
                users[str(user_id)]["channel_joined"] = True
                users[str(user_id)]["channel_joined_at"] = datetime.now(timezone.utc).isoformat()
                save_user(str(user_id))
            except Exception:
                pass
            return _send_start_home(user_id)

        # Not a member: only the join controls are shown.
        send_join_message(user_id)
        return False
    except Exception as e:
        print("FORCE JOIN CHECK ERROR:", repr(e))
        # If this account had a previously successful membership check, don't
        # lock the user out because Telegram temporarily failed the API request.
        if users.get(str(user_id), {}).get("channel_joined"):
            return _send_start_home(user_id)
        send_join_message(user_id)
        return False
...
def send_join_message(user_id):
    """Show only the required join controls while access is blocked."""
    kb = InlineKeyboardMarkup(row_width=2)
    kb.row(
...
def confirm_join(call):
    ...
    try:
        ...
        if _member_is_joined(member):
            # No toast/notification such as 'Join verified'.  The old join
            # prompt is removed and the regular /start home is shown instead.
            try:
                bot.answer_callback_query(call.id)
            except Exception:
                pass
            ...
            try:
                uid = str(user_id)
                users.setdefault(uid, {})["channel_joined"] = True
                users[uid]["channel_joined_at"] = datetime.now(timezone.utc).isoformat()
                save_user(uid)
            except Exception:
                pass

            _send_start_home(user_id)
            ...
            if user_id in pending_links:
                ...
                if link:
                    try:
                        ...
                        print("PENDING LINK AFTER JOIN ERROR:", repr(e))
        else:
            # Keep the join controls available; only a callback alert is used
            # for an unsuccessful confirmation attempt.
            try:
        ...
        print("CONFIRM JOIN CHECK ERROR:", repr(e))
        try:
            bot.answer_callback_query(
                call.id,
                "❌ Please join the channel first.",
