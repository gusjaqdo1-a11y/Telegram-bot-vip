...
def _rapid_url(d):
    # The RapidAPI response has changed field names between provider revisions.
    # Accept all common direct-media URL fields, while ignoring manifests/thumbnails.
    if not isinstance(d,dict):
        return None
    for k in (
        "url","downloadUrl","download_url","downloadUrlHttps","download_url_https",
        "videoUrl","video_url","audioUrl","audio_url","playbackUrl","playback_url",
        "streamUrl","stream_url","contentUrl","content_url","mediaUrl","media_url",
        "directUrl","direct_url","link"
    ):
        v=d.get(k)
        if isinstance(v,str) and v.startswith(("http://","https://")):
            low=v.lower()
            if not any(x in low for x in (".jpg",".jpeg",".png",".webp","thumbnail")):
                return v
    # Some versions wrap the direct URL one level deeper.
    for k in ("download","media","source","stream","file"):
        v=d.get(k)
