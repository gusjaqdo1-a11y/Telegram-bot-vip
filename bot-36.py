...
WHATSAPP_OTP_TTL = 300  # 5 minutes
...
def send_d7_whatsapp(phone_number, user_id):
    """WhatsApp OTP through D7 Verify (same OTP lifecycle as SMS).

    D7 Verify supports SMS and WhatsApp on the same send-otp endpoint; the
    channel is selected explicitly here so there is no dependency on WaForge.
    """
    if not D7_TOKEN:
        return False, "D7_TOKEN is not configured", None
    headers={"Authorization":f"Bearer {D7_TOKEN}","Content-Type":"application/json","Accept":"application/json"}
    payload={
        "originator":os.getenv("D7_WHATSAPP_ORIGINATOR",os.getenv("D7_ORIGINATOR","SignOTP")),
        "recipient":phone_number,
        "content":"Your Downloader Bot Verification Code is: {}",
        "expiry":600,"data_coding":"text","channel":"WhatsApp",
        "otp_code_length":6,"otp_type":"numeric","retry_delay":60,"retry_count":5
    }
    try:
        r=requests.post(D7_VERIFY_SEND_URL,json=payload,headers=headers,timeout=30)
        try: body=r.json()
        ...
        if r.status_code in (200,201,202):
            otp_id=body.get("otp_id") or body.get("request_id") or body.get("id")
            if otp_id: return True,body,str(otp_id)
        return False,f"HTTP {r.status_code}: {body}",None
    except Exception as e: return False,str(e),None

def verify_d7_whatsapp(phone_number, code):
    """Verify a D7 WhatsApp OTP using the exact otp_id saved for this number."""
    if not D7_TOKEN: return False,"D7_TOKEN is not configured"
    otp_id=None
    for data in whatsapp_verify_pending.values():
        if data.get("phone")==phone_number and data.get("otp_id"):
            otp_id=str(data.get("otp_id")); break
    if not otp_id: return False,"WhatsApp OTP session is missing or expired"
    headers={"Authorization":f"Bearer {D7_TOKEN}","Content-Type":"application/json","Accept":"application/json"}
    try:
        r=requests.post(D7_VERIFY_CHECK_URL,json={"otp_id":otp_id,"otp_code":str(code)},headers=headers,timeout=30)
        try: body=r.json()
        except Exception: body={"raw":r.text}
        ok=r.status_code==200 and str(body.get("status","")).upper() in {"APPROVED","SUCCESS","VERIFIED"}
        return ok,body
...
def sms_admin_manager(m):
    ...
    if m.text == "🟢 Open SMS":
        ...
        if not D7_TOKEN:
            try:
                bot.send_message(m.chat.id, "❌ WhatsApp cannot be opened because D7_TOKEN is missing.")
            except: pass
            return
        set_setting("whatsapp_verify_enabled", True)
        try:
            bot.send_message(m.chat.id, "🟢 WhatsApp verification is now OPEN. Users can request OTP via D7 Verify WhatsApp.")
        except: pass
...
def start_verify_flow(call):
    try:
        ...
        whatsapp_enabled = get_setting("whatsapp_verify_enabled", False) and bool(D7_TOKEN)
        if sms_enabled:
            kb = InlineKeyboardMarkup()
            kb.row(
...
def handle_verify_choice(call):
    ...
    try:
        ...
        if choice == "gmail":
            ...
            if not get_setting("whatsapp_verify_enabled", False) or not D7_TOKEN:
...
# ----- WHATSAPP VERIFICATION LOGIC (D7 Verify) ----- #
...
def process_verification_whatsapp(m):
    ...
    if not re.fullmatch(r"\+[1-9]\d{9,14}", phone_input):
        ...
        bot.register_next_step_handler(msg, process_verification_whatsapp)
        return
    ok, detail, request_id = send_d7_whatsapp(phone_input, uid)
    if not ok:
        bot.send_message(
            m.chat.id,
            "❌ <b>D7 could not send the WhatsApp OTP.</b>\n\n"
            "Please try again in a moment.",
        )
        print("D7 WhatsApp OTP error:", detail)
        return
...
def resend_whatsapp_code_callback(call):
    ...
    ok, detail, request_id = send_d7_whatsapp(data["phone"], uid)
    if not ok:
        print("D7 WhatsApp resend error:", detail)
        bot.answer_callback_query(call.id, "Could not send D7 WhatsApp OTP.", show_alert=True)
        return

    now = time.time()
    data["otp_id"] = request_id
    data["last_resend"] = now
    data["time"] = now
    bot.answer_callback_query(call.id, "✅ New D7 WhatsApp OTP sent")
    msg = bot.send_message(
        call.message.chat.id,
...
def process_whatsapp_code(m):
    ...
    if time.time() - data.get("time", 0) >= WHATSAPP_OTP_TTL:
        whatsapp_verify_pending.pop(uid, None)
    ...
    ok, detail = verify_d7_whatsapp(data["phone"], code)
    if not ok:
        print("D7 WhatsApp verify error:", detail)
...
def _run_ytdlp_download(link, tmp_dir, platform, fmt, base_opts, max_duration, uid=None, quality=None):
    """Direct yt-dlp downloader. YouTube does not use RapidAPI or another downloader API."""
...
def download_media(chat_id, link, message_id, quality=None):
    ...
    try:
        media=[]
        provider="yt-dlp"
        # YouTube uses direct yt-dlp only; no Railway/RapidAPI downloader is used.
...
def rapidapi_status_handler(m):
    if not is_admin(m.from_user.id): return
    # Kept for compatibility with old admin keyboards, but YouTube no longer
    # depends on RapidAPI. Report the actual local downloader status instead.
    bot.send_message(m.chat.id, f"✅ YouTube downloader: direct yt-dlp\n🎞 FFmpeg: {'AVAILABLE' if _youtube_ffmpeg_available() else 'NOT FOUND'}\n☁️ External YouTube API: DISABLED")
...
if __name__ == "__main__":
    ...
    print(f"🟢 D7 WhatsApp configured: {bool(D7_TOKEN)} | D7 SMS configured: {bool(D7_TOKEN)}")
